...
| FR-365 | 已完成 FlowRun 节点会话原生 Fork | DONE | 流程完成时保留节点源会话只读，但放开与普通会话一致的原生 Fork；取消流程／取消 Attempt 继续拒绝所有会话写入。 |
...
| 切片 | 风险 | 状态 | 范围 |
| --- | --- | --- | --- |
| FIX-01 | 会话列表在 ASGI worker 内同步读取多个 OpenHands Conversation 状态；供应商限流或 Runtime 慢响应会耗尽 API worker，进而使普通首屏请求长期 pending | DONE | Agent Workspace 和 FlowRun 节点会话列表只返回受控数据库投影与明确 `unknown` 状态，不再从列表路由读取 Runtime；当前选中会话继续经既有受限 Runtime-read lane 对账。分页、排序、未读、写入权限与能力投影保持，且新增“列表不得触发 Runtime”回归。 |
| FIX-02 | FIX-01 将未选中会话状态统一为 `unknown`，导致后台运行会话的侧栏图标消失 | DONE | 保持会话列表零 Runtime 调用；新增独立活动快照接口，经低优先级有界读取通道单次调用 OpenHands running search，并将授权范围内的原生 Conversation ID 映射为 binding ID。前端独立轮询该投影，选中会话仍以精确 readiness／事件状态为准。 |
| FIX-03 | 长时间没有正式事件的运行会话被误呈现为“正在思考”，而原生 `error`／`stuck` 终态仍被普通未读蓝点覆盖 | DONE | 活动快照只读地投影运行中会话的正式事件空窗与原生 `error`／`stuck` 状态；侧栏以红色感叹号优先显示异常，终态异常抑制蓝点的视觉呈现但不改写 `unread`，运行中异常仍可同时显示正常蓝色未读点。会话内改为中性的“后台长时间未产生可确认进展。可暂停后继续以重新建立调用。”提示；不暂停、重试、重建或持久化任何会话状态。 |
| FIX-04 | 同一红色异常标记无法区分“仍在运行但无进展”与已异常终止 | DONE | 仍处于 native running 的长期无进展会话令红色感叹号以克制心跳动画显示；原生 `error`／`stuck` 终态保持静态。动画只反映既有只读活动投影，并尊重系统“减少动态效果”偏好。 |
| FIX-05 | 已读的历史异常仍长期显示红色感叹号，且运行中异常与蓝色未读点并排 | DONE | 红色感叹号现在是服务端 `unread` 的异常同位样式：未读且异常时替代蓝点并固定在相同位置；进入会话沿用既有已读写回后立即消失。运行中长期无进展时为心跳样式，原生终态异常保持静态；不新增持久状态或会话控制。 |
| FIX-06 | 用户手动标记未读被异常投影误显示为红色感叹号，且删除按钮与红标重叠 | DONE | `AgentConversationBinding.unread_origin` 区分 `MANUAL` 与 `SYSTEM`：手动未读始终为蓝点并可覆盖既有系统异常来源；只有系统来源的未读异常显示红色感叹号。历史未读保持普通蓝点。侧栏悬停显示删除按钮时同时隐藏红标；不改变 OpenHands、Runtime 或会话控制。 |
| FIX-07 | 已打开会话的“后台长时间无响应”提示只有文字，没有运行中心跳感叹号 | DONE | 会话内正式 `possibly_stuck` 投影现在显示红色心跳感叹号；它仅在原生会话仍运行且实时连接正常时出现，后续正式事件、连接恢复提示或终态会自动移除。不改变未读、OpenHands 或 Runtime 控制。 |
...
| FR-376 | 输入区任务摘要行内收拢与详情层级 | DONE | 任务标签紧随工作区路径并缩至 20px；详情浮层在标签上方以独立层级显示，避免贴住会话边界或被输入区遮挡。 |
| FR-377 | 会话历史浏览与最新回复浮层恢复 | DONE | 历史分页不再吞掉用户上滑；离开底部后保留阅读位置并显示动态“跳到最新”浮层。 |
...
| FR-386 | 协作注释归属 OpenHands 消息元信息 | DONE | 注释随当前用户消息原生持久化并从消息回显；移除 FlowWeave 注释表 CRUD，不校验模型 marker。 |
...
| FR-444 | 暂停后附件重思考归属校验 | DONE | 重思考首条含附件消息时，以会话共享项目根而非冻结子目录校验同一 binding 的私有附件；跨会话附件继续拒绝。 |
...
| FR-451 | Chat 模型协议与思考强度冻结 | DONE | API Key Chat Completions 模型显式绑定 OpenHands `api_mode=chat`，并在创建、切换与 fallback profile 中传入冻结的 `reasoning_effort`（含显式空值）；切换后校验 Runtime 实际思考档位，避免从 Responses 模型继承或由 SDK 默认补成 `high`。 |
| FR-452 | 模型切换 Registry 身份收敛 | DONE | OpenHands `switch_llm` 的 LLM registry 为 first-write-wins；主模型 usage_id 从供应商级身份收敛为 Runtime 会话密钥 HMAC 的完整有效配置身份，避免同供应商切换模型、思考强度、协议、地址或轮换凭据时复用旧 LLM。切换后仍严格校验供应商、模型、地址、协议和思考强度。 |
| FR-453 | 默认思考强度持久化一致性 | DONE | FlowWeave 的空思考强度表示采用固定 OpenHands 1.47 默认值；Runtime LLM payload 解析为可跨 `exclude_none` 持久化往返的正式 `high`，避免切换后立即回读为 high 并被误判漂移。显式选择的其他档位保持不变。 |
| FR-454 | 逐步运行同节点独立记录 | DONE | 中性画布中再次选择同一节点保存配置会建立独立的逐步记录与首个 Attempt，不会被已有待启动／运行记录拒绝；仅 `FLOW_TRANSITION` 下游占位可被配置或流转复用。 |
...
目标：从 Git 提交详情选择文件打开中央 Diff 页签后，重新激活文件页签时，右侧 Git 侧栏必须保留此前选择的同一仓库提交详情与文件树，不能因侧栏条件卸载重新显示提交历史列表。显式返回提交历史、选择另一提交或切换到另一仓库时，仍须按原有语义清理或替换详情。

完成：已选提交 ID 与所属仓库路径提升到既有工作区作用域状态；Git 侧栏重新挂载时从该状态恢复详情。关闭中央 Diff 页签只清除局部已选 Diff 文件，保留提交身份；仓库改变时局部文件选择仍会清空，避免跨仓库复用路径。

### FR-372 提交／改动审查层级文件树 — DONE
...
### FR-376 输入区任务摘要行内收拢与详情层级 — DONE

依赖：`FR-375`。

目标：实时任务摘要必须作为工作区路径后的紧凑行内状态，而非输入区右下角的独立块；标签高度不得再接近会话边界。悬停／聚焦详情必须保持在标签上方且有足够堆叠层级，不受输入区布局遮挡。

完成：输入区底部先展示工作区路径，再紧随 20px 任务标签；移除了将标签推至右端的自动边距，收紧底部间距与路径图标。任务详情浮层改为从标签左上方展开，并由 composer 底部和浮层的明确堆叠层级覆盖邻近输入区。继续只读取既有 OpenHands `TaskTrackerObservation` 快照，未新增持久化、私有协议或运行时状态。

验收结果：Web TypeScript typecheck、ESLint、production build、Playwright 定向产品流用例收集、`git diff --check` 与任务状态唯一性通过。完整浏览器用例仍需本地 API／认证服务，当前未启动，故仅收集并静态校验相关断言；不修改 API、数据库、Runtime Provider 或 OpenHands。

### FR-377 会话历史浏览与最新回复浮层恢复 — DONE

依赖：`FR-27`。

目标：流式输出和后台会话历史分页不能阻止用户上滑、拖动滚动条或键盘浏览较早记录。用户离开最新消息后，后续输出不得抢回滚动位置，页面必须显示可识别的动态“跳到最新”浮层；历史仍在分页读取时应明确提示加载状态。

完成：历史分页不再使滚动事件失效；会话视口的真实位置统一决定是否继续自动跟随。滚轮、触控拖拽、滚动条和键盘上行都可解除贴底，点击浮层或主动回到末行后才重新开启。浮层提升层级和对比度，生成中继续使用脉冲与三点动画；历史分页期间在会话顶部显示“正在载入更早的会话记录”。

### FR-378 运行中上划的滚动竞争消除 — DONE

依赖：`FR-377`。

目标：流式渲染、内容尺寸观察和压缩状态更新不得启动或续接平滑滚动，从而与用户上划、触控拖拽或滚动条操作竞争。用户开始向上浏览时，尚未执行的自动滚动帧和浏览器平滑动画必须立刻停止。

完成：自动贴底统一改为可取消的即时位置更新，仅“跳到最新”按钮保留用户主动触发的平滑过渡。会话视口在滚轮和触控捕获阶段先取消残余动画、设置阅读锁，再允许浏览器处理滚动；流式输出、ResizeObserver 和压缩状态只在阅读锁未解除时更新底部位置。

### FR-379 会话历史异步分页与原始跳转样式恢复 — DONE

依赖：`FR-377`、`FR-378`。

目标：打开会话继续先展示最新窗口，随后异步读取更早历史；模型运行、未完成轮次和 input-readiness 轮询不得阻止这一只读分页。页面只能在真实历史请求期间显示加载提示，分页游标循环或请求失败不得留下永久“正在载入”状态。生成中的“跳到最新”控件恢复为最初无外圈波纹、无整体震动的简洁三点样式。

完成：将“存在下一页”与“正在加载”拆分为独立状态；最新窗口绘制 100ms 后立即后台分页，无论当前会话是否运行。分页期间按 scope 显示加载提示，完成或失败均清理；同一游标重复被明确拒绝并显示错误，失败游标不自动无限重试。跳转控件恢复 34px 原始视觉基线，仅三点保留呼吸动画。

### FR-380 会话真实底部定位稳定性 — DONE

依赖：`FR-377`、`FR-378`、`FR-379`。
...
目标：所有入口都必须传入 `/runtime/workspace/project/<record-id>`；不得再传物理 mount、Attempt ID 或工作目录子路径作为 OpenHands workspace root。嵌套导入记录继续用自身的记录 ID 作为逻辑根，同时复用父 FlowRun 的唯一 Runtime allocation。历史 Attempt-private Runtime 仅保持既有只读兼容，不迁移、不合并或重新创建。

完成：新建共享 Attempt、节点会话创建／重载、locator 恢复、附件上传和工作目录选择全部使用记录根，物理 `project` mount、Attempt 与逻辑子目录不再进入 OpenHands workspace root。记录身份始终由服务端解析，嵌套记录仍使用自身记录 ID 并继续解析父 FlowRun allocation。只对已有 Attempt-private Runtime 保留原冻结路径的只读兼容；未迁移、合并或重建其历史文件。
...
### FR-401 记录 ID 与 Runtime owner 身份分离 — DONE

依赖：`FR-400`。

目标：共享 Runtime 的物理 owner 仅用于 allocation／网络／能力物化；工作区根必须使用产品记录 ID。连续嵌套记录使用自身 ID，非连续记录使用其 NodeRun 记录 ID；不同记录不得因共享 FlowRun Runtime 而共用目录。

完成：保留 `project/<record-id>` 的规范 OpenHands 根和全部入口的统一传递，但恢复 `flow_run_record_id()` 的产品记录解析：嵌套连续记录使用自身 ID，非连续记录使用 NodeRun 记录 ID。Runtime owner 继续只用于共享 allocation／网络／能力物化，不能进入 workspace root。新增回归覆盖两类记录身份及其规范路径，确保同一 Runtime 下不同记录隔离。
...
### FR-422 流式工具活动在补读窗口中闪退 — DONE

依赖：`FR-407`。

目标：同一 OpenHands 工具 Action 在浏览器流式事件中已显示后，若 REST 最新页只返回其部分投影或暂未
覆盖该 Action，运行中的工作过程不得变为空卡片，也不得把工具状态退回泛化的“正在思考”。

完成：前端以 OpenHands 正式 event ID 合并 REST 与流式事件；同 ID 的最新 REST 字段覆盖已持久字段，
而流式投影已经获得的 command、thought 与 details 在 REST 还未携带时保留。最新页读取也与当前已加载
分支按身份累积，避免有界 cursor 读取清除本轮已显示活动；不按事件顺序或文本推断关联。

验收：新增 Agent 工作台 Playwright 回归断言，模拟同一 `TerminalAction` 的不完整 REST 投影在流式
工具卡已显示后到达，确认工具说明与“正在运行 pwd”持续可见。

### FR-423 Agent 会话 Runtime 投递短事务与无阻塞连接审计 — DONE

依赖：无（会话消息可靠性交付审计发现的独立根因修复）。

目标：运行中的 OpenHands standard Agent 可以原生追加正式用户消息，但 FlowWeave 不得在该 Runtime
HTTP 调用期间持有 `agent_conversation_bindings` 的 `FOR UPDATE` 行锁。否则一个可能等待当前 Agent
步骤完成的投递会阻塞同一会话的读取、WebSocket 重连和后续投递。会话读取也不得仅为连接审计更新该
同一 binding 行。

完成：Agent Workspace 和 FlowNode 的普通消息入口均改为无锁读取已授权 binding；引用仍按正式 native
event ID 解析，附件、工作区引用、模型选择、OpenHands 原生 `send_message` 和 native running-turn append
语义均不变。Runtime 返回后才继续写入既有附件、活动时间和 FlowNode wakeup 投影。Agent Workspace
`get_conversation`、FlowNode `get_node_session_view` 与 `get_node_conversation` 不再修改
`last_connected_at` 或 flush，因此读取／realtime 握手不再为了连接审计竞争 binding 行。新增回归断言：
运行中追加与 FlowNode END_BLOCKED 唤醒投递都不请求 `lock=True`；会话读取保留既有连接审计时间。
...
目标：FlowNode 会话的运行中追加必须与 Agent Workspace 保持相同的可靠性边界：本地授权、Attempt／binding
lineage、附件和工作区引用在短事务中冻结；运行中的 OpenHands `input-readiness`、正式 event ID 引用解析和
`POST /events` 仅在不使用 FlowWeave 数据库的 Runtime 段执行；成功后再以短事务围栏检查同一 native
conversation 并投影附件／活动时间及 END_BLOCKED 唤醒。若 Runtime 返回超时或服务端错误，必须返回
`AGENT_MESSAGE_DELIVERY_AMBIGUOUS`，不得按历史或最新消息推测并自动补发。

验收：受影响 Python Ruff format/check、`py_compile`、无数据库依赖的 Runtime dispatch 回归、`git diff --check`
与任务状态唯一性；数据库回归仅在 Testcontainers 可用时执行。

完成：FlowNode `/messages` 在运行中先通过 `run_sync` 提交并冻结授权、lineage、附件／工作区引用和
Runtime handle；随后 `run_blocking` 仅调用 OpenHands readiness、按正式 event ID 读取引用并追加正式 user
event；确认返回后才以新的短事务检查原 conversation locator，写入附件投影、活动时间与 END_BLOCKED
...
### FR-444 暂停后附件重思考归属校验 — DONE

依赖：`FR-26`、`FR-432`。

目标：用户在新会话首条消息上传附件、暂停后编辑该消息并选择“重新思考”时，附件必须仍可由同一会话重新发送；任何其他会话的私有附件仍必须拒绝。冻结工作目录不能把会话共享项目根目录中的首条草稿附件误判为外部附件。

完成：重思考路径继续以当前 binding ID 验证附件的不可伪造 owner，但根目录改为 Runtime handle 的会话共享项目根，与草稿上传、首条 bootstrap 和普通消息路径一致。新增回归覆盖冻结子目录后的首条图片附件暂停／重思考，确认附件路径和原生图片内容均会再次发送。

验收：受影响 Python 文件 `py_compile` 与 `git diff --check` 通过；新增定向 pytest 已准备，但本机 Python 开发依赖缺失且 Docker Unix socket 不可用，Testcontainers 数据库 fixture 无法启动，未伪记为通过。未修改数据库、OpenHands、Runtime Provider、Docker 或远端环境。
...
### FR-437 Responses 不完整终态关联诊断 — DONE

依赖：`FR-436`。

目标：当正式 OpenHands Conversation／Agent Error 终态在 FlowWeave 投影为失败时，必须保留足以关联同一
Conversation、原生事件、Runtime、state leaf 和响应终止形态的服务端诊断事实，以区分浏览器投影、
FlowWeave adapter、OpenHands／LiteLLM Responses 流终态与上游请求级差异。诊断不得新增 Conversation 事实、
重试、续写、模型切换或 OpenHands 私有协议；不得把用户输入、模型输出、事件 detail、prompt、凭据、endpoint
或原始 ID 写入日志。

完成：OpenHands Runtime adapter 在正式 REST event page 与 active branch 读取路径中，对每个
`ConversationErrorEvent`／`AgentErrorEvent` 仅记录一次有界、进程内去重的诊断。日志只携带 SHA-256 截短
关联摘要、白名单错误码／classification、native execution status 和安全终态 signature；已识别
`ResponseIncompleteEvent`、缺少 completed response 与其他非预期 terminal event。所有事件文本仅在内存中用于
signature 分类，绝不传给 logger。

验收：新增无 Docker fixture 的单元回归，模拟含假 Bearer token 的 `ResponseIncompleteEvent` detail，确认重复读取
只产生一条日志，且日志保留 `LLMNoResponseError`、`responses_incomplete_event` 与 retryable 分类，却不含原始
event／parent／response／leaf ID 或假 token。受影响 Python Ruff check／format、`py_compile`、两项定向 pytest
与 `git diff --check` 通过。未修改数据库、OpenHands 源码、Runtime Provider、Docker 或持久数据。

### FR-438 Conversation／Fork 模型绑定一致性诊断 — DONE

依赖：`FR-437`。

目标：当同一模型的新 Conversation 成功，而既有 Conversation 及其原生 Fork 重复出现 Responses 不完整终态时，
必须区分 FlowWeave 选择已更新但 OpenHands 仍使用旧 LLM，以及 OpenHands 已使用目标 LLM、但异常事件树／Agent
持久状态随原生 Fork 继承。诊断不得读取或记录消息正文、API Key、OAuth token、endpoint 明文、原始 provider／
Conversation ID，也不得改变模型、Fork、重试、事件树或会话历史。

完成：OpenHands adapter 在 Conversation 创建、`switch_llm` 后正式 state 回读、原生 Fork 父子状态返回和正式错误
终态读取处写入脱敏诊断。模型名仅在严格安全字符白名单内明文显示；provider、base URL、Conversation、父
Conversation 与 Runtime 仅记录 SHA-256 截短摘要。日志逐项标明 provider/model/base URL/API mode/reasoning 是否
匹配；Fork 同时记录 `fork_inheritance` 和 FlowWeave 保存配置的 `fork_binding`。错误终态附加当时 active model、
API mode、reasoning、当前 token 使用、窗口和累计 usage；日志异常被隔离，不得影响正式会话读取。

验收：无 Docker fixture 的定向回归覆盖 switch 成功、旧 LLM 漂移拒绝、Fork 父子 LLM 继承／保存配置匹配，以及
`ResponseIncompleteEvent` 错误时 active model/reasoning 投影；断言 endpoint、provider 原值、密钥、event／response
原始 ID 和假 Bearer token 均不进入日志。受影响 Python Ruff check／format、`py_compile`、四项定向 pytest 与
`git diff --check` 通过。未修改数据库、OpenHands 源码、Runtime Provider、Docker 或持久数据。

### FR-439 会话投递／Fork 全链路脱敏诊断 — DONE

依赖：`FR-437`、`FR-438`。

目标：对今天异常最可能经过的 Agent Workspace／FlowNode 消息入口、running 快速追加、idle 重绑与主动
压缩、异常 Fork 恢复、原生 Fork、正式 user event append 以及 Responses 不完整终态建立同一套可关联诊断。
日志只能记录截短 SHA-256 摘要、安全枚举、字符／事件计数、精确 execution status、上下文用量与策略决策；
不得记录用户消息、模型输出、事件 detail、API Key、OAuth token、endpoint 明文或原始 Conversation／Runtime／
event／response 身份。诊断读取失败不得改变发送、Fork、重试、历史或终态。

完成：Agent Workspace 和 FlowNode 在 running／idle 分流后、正式发送前记录 readiness、active HEAD 的有界
事件数、叶子类型、user／assistant／error 数量、当前模型/API mode/reasoning、上下文与 condenser 阈值，以及
是否重绑模型、执行异常 Fork 恢复或主动压缩。OpenHands user event append 对已接受与结果不确定分别记录
匿名 cursor 来源、请求长度／图片数或安全错误码；Fork 记录父子有界事件量、指标重置、上下文和 LLM 继承
匹配。`ResponseIncompleteEvent` 额外输出白名单 `incomplete_reason`；固定 OpenHands 未暴露结构化原因时明确为
`not_exposed`，不再把缺失原因猜成模型、网关或长度限制。

验收：受影响 Python Ruff check／format、`py_compile`、完整无 Docker `test_openhands.py`（154 passed）、
`git diff --check` 与任务状态唯一性通过。回归覆盖不完整原因白名单、重复终态去重、投递状态／事件计数、
append 接受／歧义和诊断读取失败隔离，并确认消息、输出、Bearer、endpoint、provider 与所有原始身份不进入
日志。三项 Conversation 投递定向测试因本机 Docker socket 缺失，在 Testcontainers PostgreSQL fixture 初始化
时失败且未进入断言，未记为通过。未修改数据库、迁移、OpenHands 源码、Runtime Provider 或持久数据。
...
### FR-384 引用与注释的统一上下文呈现 — DONE

依赖：`FR-382`、`FR-383`。

目标：会话文本和文件内容的既有“添加到会话”必须统一为可带评论的精确锚点；用户不应在一次性引用和协作注释之间作选择。发送区继续采用既有会话／文件引用 chip 的紧凑呈现，点击后可查看原文、定位及编辑评论。
...
### FR-386 协作注释归属 OpenHands 消息元信息 — DONE

依赖：`FR-385`。

目标：移除 FlowWeave 对协作注释的独立存储，使会话和文件锚点、评论与 ID 成为当前用户消息的一部分；历史呈现从同一 OpenHands 原生 MessageEvent 投影，服务端不校验或关联模型回复 marker。

完成：前端选区只维护发送前草稿，发送时将注释随消息封套提交；首条消息草稿刷新后仍恢复其注释，会话历史从同一封套还原注释并据已知 ID 宽松渲染回复 marker。会话与文件锚点共用这套草稿机制；两个宿主均移除 `/annotations` CRUD、ORM 模型与删除逻辑；旧表不再访问，且不做破坏性删表。已发送消息中的注释按原生消息冻结，评论只能在发送前编辑。

验收：受影响 Python 编译与 Ruff、Web TypeScript typecheck、注释封套往返断言、唯一 Alembic head（仍为 `0115_agent_annotations`）及 `git diff --check`。数据库 pytest 仍需要本机 Docker Testcontainers，当前环境不可用。

### FR-387 协作注释草稿交互修正 — DONE

依赖：`FR-386`。

目标：发送前注释应立即成为本地草稿，评论可留空；详情必须是悬浮层而非撑高 Composer；会话定位应定位到选中引文而非整条消息；文件选区的“添加到会话”仅在真实回调存在时出现并可靠创建草稿。

完成：新建会话／文件注释不再弹出必填评论框，直接创建带空评论的草稿并自动打开详情浮层；评论编辑允许保存空值。详情卡片以 Composer 上方的独立 Popover 渲染，不改变输入框高度。会话定位在目标 native event 内按已保存 quote 建立 DOM Range、滚动到该范围并短暂选择高亮，找不到精确文本时回退消息卡片定位。文件预览仅将真实注释回调传入选区组件，并阻止操作按钮的 `mouseup` 回流到选区捕获逻辑。没有新增 API、数据库、消息校验、模型 marker 校验或任何 OpenHands 改动。

验收：Web TypeScript typecheck、受影响 Web ESLint、生产构建与 `git diff --check`。

### FR-388 协作引用草稿删除 — DONE

依赖：`FR-387`。

目标：发送前的会话／文件引用 chip 应与其他 Composer 上下文一致，支持明确移除；删除不得影响已发送的 OpenHands 原生消息。

完成：每个协作引用 chip 新增可访问的移除按钮，删除对应本地草稿注释；若该 chip 的详情或评论编辑已展开则同步关闭。未发送的消息继续使用更新后的注释封套，已发送消息保持冻结。

验收：Web TypeScript typecheck、受影响 Web ESLint、生产构建与 `git diff --check`。

### FR-389 Web 镜像离线运行层修复 — DONE
...
### FR-337 FlowRun 供应任务耗尽后的受控恢复 — DONE

依赖：`FR-336`。

目标：

- Worker 启动与周期恢复必须能识别已耗尽的 `PROVISION_FLOW_RUN_RUNTIME` 任务：仅当最后错误为
  `SANDBOX_BACKEND_UNAVAILABLE`、FlowRun 仍可执行、没有 active generation，且同一持久 Sandbox／generation
  仍为 `RUNNING`／`PROVISIONING` 意图时，恢复该任务。
- 每个任务只允许一次额外重投递预算；在 task payload 中留下恢复标记，二次耗尽保持 `DEAD`，禁止无限重试。
- 不恢复 `DRAFT`／`COMPLETED`／`CANCELLED` 或已物理删除的 FlowRun，不恢复确定性错误，也不将历史
  `FAILED`／`DELETED` generation 伪装为安全的同 generation 重试。
- 恢复必须使用 `SKIP LOCKED` 与既有 task row，清除 lease 并重置 attempts，但不新建物理 Sandbox、generation
  或 OpenHands Conversation。

验收：受影响 Python 文件可解析／静态检查、`git diff --check`、任务状态唯一性；定向 pytest 若本机 Docker
可用则运行，否则如实记录 Testcontainers 前置条件阻断。生产部署后只读确认不存在符合条件的积压任务，
且已删除 Run 的历史 `DEAD` task 未被复活。
...
### FR-339 Diff 跳转的规范文件坐标与逐层树定位 — DONE

依赖：`FR-338`。

目标：

- Diff 跳转不得直接把事件中携带的展示路径作为预览路径；必须以当前受权目录分页接口返回的文件路径为准。
- 懒加载目录树必须顺序确认每一层目录，再展开并请求下一层；不存在或过期路径必须停止，不能遗留在展开集合并持续请求 404。
- Git 仓库显示前缀与工作目录同名时，须去除重复前缀，避免形成 `<working-directory>/<label>/...` 的错误坐标。

完成：工作区源路径归一化会去除重复的工作目录标签；新增待导航状态，按根目录到目标文件逐层验证目录分页条目、
展开已确认目录，并仅在最终文件由接口返回后设置预览、行高亮与树选中。目标行出现后自动滚动至可见区域。
...
### FR-342 根工作区 Git 历史展示修复 — DONE

依赖：`FR-341`。

目标：

- 根工作区或当前工作目录本身是 Git 仓库时，选中该目录或其子文件必须展示该仓库的提交历史。
- 继续在嵌套仓库场景选择最深匹配仓库，不放宽后端的工作目录／仓库路径授权边界。

完成：前端仓库选择不再排除恰好等于 `details.root` 或 `details.working_directory` 的仓库。此前这条
容器路径过滤导致根工作区的 `hq-interface` 虽被后端正确发现却永远无法挂载 Git 侧栏；现在与子工作区一致，
按选中路径的最深包含仓库显示历史。
...
- 用户在连续运行草稿中配置并冻结额度失败备用模型后，保存其他配置或重新打开草稿时必须保留已选顺序，不能显示为“未配置”。
...
- 添加区始终展示供应商、模型、思考程度和添加按钮；没有可用思考程度档位时显示默认值。
- 列表必须显示实际供应商名称、优先级和可操作的拖拽把手、移除控件。
- 保存动作使用“保存”，不再把临时编辑操作表述为“冻结”。

完成：上方添加区将供应商、模型、思考程度和单一“添加备用模型”按钮并排展示；思考程度没有支持档位时仍显示“默认”。列表按运行时尝试顺序展示，只能从左侧把手拖拽重排，避免误触调整；模型的默认思考程度仍随所选模型保存。
...
- 历史 Environment、Snapshot、节点和会话可以保留 OpenHands 版本、source commit/ref 等 provenance 作为审计信息，但不得以其存在、完整性或与运行中 Server 的相等性作为打开、创建、恢复或执行的准入条件。
- 保留对损坏的 FlowWeave 对象身份、缺少必需协议字段、路由、工具或能力规格漂移的明确拒绝；这些是实际功能兼容性诊断，不能被基线升级掩盖。
...
### FR-365 已完成 FlowRun 节点会话原生 Fork — DONE

依赖：FR-344（终态节点会话只读恢复）。

目标：

- FlowRun 为 `COMPLETED` 时，节点源会话仍禁止消息、改名、模型切换、暂停／继续、删除和执行恢复，但允许从一条完成回复调用与普通 Agent 会话相同的 OpenHands 原生 Fork。
...
- 将私有生产环境上不停累积的 FlowWeave platform/web rollback image 与可回收 BuildKit cache 纳入可审计的
...
完成：维护工具从私有本地配置读取目标，默认 dry-run；其候选
...
- 操作前后确认所有 Container 引用 image、named volume、持久 Workspace、远端
...
完成（2026-09-12）：在私有生产环境以已提交的 OPS-03 工具重新预检后，
...
### FR-155 执行记录 Command／Shift 多选删除 — DONE

依赖：`FR-154`。

目标：FlowRun 工作台的单节点运行与自动运行记录支持 Command/Control 切换选择及 Shift 连续范围选择。多选只
用于批量删除；“拷贝”始终只针对最后点击的单条记录。单节点多选必须继续沿用现有运行中不可删除保护，任一
选中记录不满足删除条件时不得批量删除；自动记录则保持原有逐条删除的服务端状态语义。删除后清空选区。

完成：两类记录均以 Command/Control 切换和 Shift 区间选择展示选区，删除按钮会显示选中数量并依次删除
...
### FR-164 手动暂停中断事件展示收口 — DONE

依赖：`FR-163`。

目标：OpenHands 在用户手动暂停工具调用时会写入用于补齐工具调用历史的合成 `AgentErrorEvent`；该事件不得被
误显示为“本轮没有生成回复”或模型配置错误，也不应额外显示暂停提示。真实 Conversation／Agent 错误继续保留
原有失败展示，OpenHands 正式事件与恢复语义不得改写。

完成：Runtime 适配器补齐 `AgentErrorEvent.error` 和正式 classification 投影；Web 仅对固定 OpenHands 明确标记
“工具调用因会话暂停而中断”的合成事件静默隐藏，暂停状态仍由原生 execution status 驱动底部“已暂停”和继续
按钮。新增适配器投影断言，并在既有 Agent Workspace 浏览器场景加入暂停合成事件与无错误卡片断言。

验收：适配器定向 pytest（2 passed）、Python `py_compile`、Ruff、Web ESLint、TypeScript typecheck／production
...
### FR-172 节点完成事件回投、记录工作区清理与失败态可见性 — DONE

依赖：`FR-171`。

目标：当节点 Attempt 已因 Runtime 失败进入 `END_BLOCKED` 后，仍持续监听同一 OpenHands Conversation；若正式活动分支随后以 FinishAction 完成，必须把该完成事件重新投影到既有 Artifact 准备、结束门禁与流转状态机，而不能因为观察时 native status 已为 `finished` 丢弃。不得把浏览器候选卡片当作 Artifact 真相，也不得取消事件订阅或伪造会话事件。

删除逐步运行记录时，事务提交后递归删除其受管 `workspace_ref` 记录根及全部后代文件／目录；删除目标必须是受管理根中的普通目录，拒绝符号链接、文件、工作区根与范围外路径。画布上的 `START_BLOCKED`／`END_BLOCKED` 必须优先于自动配置与选中态渲染为红色，并维持失败状态文字。

完成：`END_BLOCKED` 的长期原生事件订阅现在一旦读到活动分支中的后续 FinishAction，会先以 CAS 清除旧失败投影，再将同一正式事件交给既有 Artifact 准备、登记与结束门禁流程；无需浏览器确认候选输出。逐步运行记录删除在事务提交后仅递归清理经 `workspace_root` 约束和普通目录校验的记录根，空路径或已不存在路径可安全忽略，符号链接／文件／工作区根／范围外路径均拒绝。失败节点的状态优先于自动配置和选中状态，选中失败节点继续使用红色边框与标识。

### FR-173 Agent 工作区级联删除、用户根附件与队列直发 — DONE

依赖：`FR-172`。

目标：删除 Agent 工作区分组时，必须永久删除该分组所有冻结版本关联的会话、会话私有附件及目录版本，不能再以“会话引用中”冻结目录。当前用户级项目根
`/runtime/workspace/project/users/<user-id>` 的图片粘贴与普通附件上传必须通过同一受限附件协议，继续拒绝其他会话或其他用户根伪造的路径。会话运行中，Enter
继续加入浏览器顺序队列；Command/Ctrl+Enter 必须暂停当前原生回合并在其正式就绪后优先发送该条引导消息，不能被当成普通排队或悄然新建会话。

完成：工作目录删除按所有历史版本显式定位对应 Agent Conversation binding，并依次调用既有原生删除和私有附件清理后物理删除目录记录图；确认文案同步为不可恢复的级联删除。附件路径合同扩展为用户根，并在上传返回和正式发送时同时核对不透明 owner UUID 与实际 Runtime 根；浏览器投影的 sandbox 图片路径使用同一用户根语法。Composer 现在区分 Enter 与 Command/Ctrl+Enter：前者保留队列，后者请求安全暂停并在 OpenHands `input_readiness` 就绪后优先发送；暂停请求失败会恢复草稿及附件，不丢失输入。

### FR-174 OpenHands 原生运行中引导消息 — DONE

依赖：`FR-173`。

目标：纠正 Command/Ctrl+Enter 的“立即引导”语义。会话正在运行时，不得调用 OpenHands `/interrupt`、取消当前 LLM／Tool 步骤或等待暂停；应通过固定 OpenHands 1.44.0 正式 events 接口追加同一条 user event，并由 OpenHands 在当前步骤结束后的下一次模型调用中消费。普通 Enter 继续仅使用浏览器内顺序队列。一级 Agent Workspace 与 FlowRun 节点共享 Workbench 必须保持一致；运行中投递不得执行仅空闲时安全的换模、分叉恢复或上下文压缩。显式“暂停当前 Agent”仍使用原生 interrupt。
...
### FR-176 定时任务记录母版与 Cron 调度 — DONE

依赖：`FR-175`。

目标：定时任务必须只能从一条已满足启动条件的连续运行 FlowRun 记录创建，不论该记录是否已经启动；创建时冻结该记录的流程快照、运行环境、逐节点计划、输入、提示词、Agent 与门禁配置为执行母版。定时任务界面不得再配置流程、环境、节点或启动提示词，只负责名称和标准五段 Cron。每次触发均从冻结母版生成独立自动 FlowRun，并在定时任务名称目录下显示，与用户手动启动的 FlowRun 区分。删除一次生成记录不得静默删除其定时任务；母版记录被引用时不得删除。

完成：调度创建 API 与 Web 均只接受／展示已就绪连续运行记录和 Cron 表达式，服务端冻结源记录的 Snapshot 与完整自动化计划，后续触发不读取可变源配置。生成的自动 FlowRun 带 `schedule_id`／occurrence 身份并在现有 Flow→定时任务→FlowRun 目录内展示。调度器实现标准五段 Cron 的数值、范围、列表和步长，按 UTC 计算下一槽位；旧分钟间隔任务在迁移后暂停，避免以不完整配置继续执行。删除生成记录保持调度目录和母版；引用中的母版 FlowRun 受显式删除保护。

### FR-177 定时任务母版目录与执行记录详情 — DONE

依赖：`FR-176`。

目标：定时任务页固定按“流程 → FlowRun 母版 → 定时任务名称”三级目录展示，一个定时名称只对应一个已冻结母版。定时名称提供“查看详情”入口；详情以应用内弹窗按触发时间倒序、分页加载所有 occurrence，每条生成 FlowRun 下必须列出可点击进入的 NodeRun 执行记录。首页不得为所有定时任务预加载完整历史，且手动 FlowRun 不得混入定时任务详情目录。
...
分类必须保守区分网络、超时、不可用与鉴权、额度、策略、上下文超限、参数错误等故障；未知故障保持未知。
动作仅允许 Webhook、通知和创建待办这类用户可选的外部增强，严禁恢复会话、暂停 Attempt、替换 Runtime 或
改变 OpenHands 生命周期；并提供基于触发器版本、正式事件身份和动作位置的稳定幂等键。此切片只实现无框架
领域匹配与分类，不接入数据库、Outbox、Worker 或 Conversation 控制。
...
### FR-205 触发器版本与动作策略持久化 — DONE

依赖：`FR-204`。

目标：持久化用户拥有的事件触发器版本和有序动作链。触发器版本使用 `owner_user_id + trigger_key +
version_no` 唯一键，过滤条件作为不可变 JSON 快照；动作使用独立记录并按 `position` 唯一排序，动作类型由
平台允许集合约束。表之间保留显式业务标识但不创建数据库外键，遵循当前物理删除和租户治理边界；本切片不
提供 CRUD API、不投递 Outbox、不执行动作，也不修改 OpenHands Conversation。

验收：新增 ORM 映射和 Alembic `0100` upgrade/downgrade 脚本可解析，元数据导入完整，受影响 Pyright/Ruff/
...
只按当前租户读取并按动作位置投影。此切片不消费运行事件、不创建 Outbox、不发送外部请求，也不触发会话或
Runtime 控制。
...
Secret。未知来源和非法触发器快照 fail closed。本切片不发送 Webhook/通知、不执行会话或 Runtime 动作，且
...
并停止；后续外部通知适配器必须保持本节冻结的无会话副作用边界。

### FR-208 Agent／子 Agent 只读事件监控与自动补救退役 — DONE

依赖：`FR-207`。

目标：将用户关注点收敛为 OpenHands 正式 Agent 事件与 `TaskAction`／`TaskObservation` 子 Agent 事件的可见性。
REST 事件读取在不持久化 Conversation 状态的前提下，投影最近正式事件、距离最近事件的墙钟时间、活动子 Agent
的正式 `action_event_id`／`tool_call_id`、以及“可能长时间无新事件”的提示；它是帮助用户判断是否卡住的观测，
不是失败裁决，不能自动中断、重发消息、恢复 Conversation、替换 Runtime 或继续父会话。历史子 Agent watchdog
任务在 Worker 中退役为无副作用完成；读取 Runtime 暂不可用时返回明确不可读状态，不再借读取路径触发恢复。
触发器动作同时收紧为 `WEBHOOK`、`NOTIFY`、`CREATE_TASK` 三类外部增强，历史会话／Attempt／Runtime 变更动作
迁移后不可再创建或投递。

验收：新增只读监控纯函数的直接断言覆盖主 Agent／子 Agent 的 stale 与终态判定；受影响 Python Ruff、格式、
`py_compile`、Alembic head、Web TypeScript typecheck 与 `git diff --check` 通过。定向 pytest 仍由仓库统一
...
### FR-211 OpenHands 事件窗口、官方 cursor 增量与历史按需读取 — DONE
...
### FR-214 共享 PostgreSQL LISTEN 与 SSE 有界扇出 — DONE

依赖：`FR-209`。将 SSE 客户端与 PostgreSQL 连接解耦为每 stream-api 进程一条 LISTEN 及有界 fanout；慢消费者
丢弃并使用 cursor 重连，LISTEN 连接和客户端配额进入指标与数据库预算。

完成：`RunEventListener` 改为每个实际提供 SSE 的进程按需创建一条 autocommit PostgreSQL `LISTEN` 连接，
再按 FlowRun ID 扇出到有界本地队列。进程上限默认为 256 个订阅者、每订阅者 8 个通知；达到容量时返回可恢复的
...
### FR-217 进程级共享 HTTP transport 与连接池 — DONE

依赖：`FR-209`。为 OpenHands、Docker Controller 及相关出站调用复用有生命周期的 HTTP client，配置连接数、
keepalive、超时和关闭；控制通道与普通读取独立限额。

完成：新增进程级 `HttpTransportPool`，普通请求池限制 20 条连接/10 条 keepalive、30 秒 keepalive，控制与
流式请求池限制 4 条连接/2 条 keepalive、15 秒 keepalive；两类池均配置 connect/read/write/pool timeout。
Container 在生命周期结束时统一关闭 sync/async client。OpenHandsRuntime 的 API、工作区、上传和 ask-agent
...
完成边界：用户决定以已部署的基础回归和健康验证收口本切片。2026-09-08 已将 commit `7f5673f` 构建为
linux/amd64 平台镜像并发布至远端；migration 以 0 退出，API、stream-api、Runtime Provider 均 healthy，
Worker 为 Up，页面、Agent 深链、静态资源、指标端点与 FastGPT 登录入口均可用。

运行中验证：本机 Docker/Testcontainers、Redis/Valkey、Prometheus 抓取端及固定 OpenHands Runtime 仍不可用，
...
### FR-226 自动运行回归验收 — DONE

依赖：`FR-222`–`FR-225`。

目标：完成自动门禁、终态恢复、摘要列表轮询、按需详情和 Artifact 分页的端到端回归；验证历史 Artifact
不会因读路径、轮询或失败恢复被重复投影、重复写入或隐式删除。

验收：覆盖 Gate `PASS`／`FAIL`／`ERROR`、自动终态恢复、摘要响应边界、按需详情、分页 Artifact 作用域以及
历史 Artifact 保留；禁止本切片调用任何删除 Artifact、工作区或自动记录的写路径。

完成：补齐自动 Gate `PASS` 的持久推进断言，并回归 `FAIL` 的第三次修订与第四次转人工、`ERROR` 的受控
...
目标：为受历史重复 Artifact 影响的自动运行提供只读、可分页且作用域受限的审计报告，精确列出候选重复项、
引用关系、工作区影响和不可自动删除原因；仅生成待用户确认的清理计划，禁止该切片删除 Artifact、工作区、
Attempt 或 FlowRun。
...
### FR-231 Agent 会话状态提示与创建时间排序 — DONE

依赖：`FR-229`。会话列表必须以创建时间稳定排序，新消息、标题任务或连接时间更新都不得改变会话的位置。每个会话左侧状态位在 OpenHands 原生运行快照为运行中时显示旋转圆环；已观测到的后台会话完成后继续显示蓝色未读点，直至用户打开该会话。

完成：服务端列表和分页 cursor 改为 `created_at DESC, id DESC`，前端在已加载页面合并后按同一稳定键显示。运行圆环和未读点均位于会话行左侧状态位，互斥呈现；删除按钮仍仅在闲置会话悬停时显示。

### FR-232 历史连续运行门禁标识恢复 — DONE

依赖：`FR-221`。修复 FR-221 对旧冻结计划的安全阻断缺少用户恢复闭环的问题。复制任何旧连续运行计划时，新的可编辑草稿必须补齐缺失 Gate ID；已因 `AUTOMATIC_PLAN_GATE_ID_MISSING` 阻断的历史记录，只有仍为冻结计划、`WAITING_HUMAN`、零 NodeRun 且版本匹配时，才能显式补齐 ID 并重新投递启动任务。完整记录详情必须投影 FlowRun 级阻断代码及节点／门禁位置；工作台针对该精确阻断显示原因和“升级计划并重新启动”，不得将其他人工处理状态误导为可恢复。
...
### FR-236 门禁执行配置、供应商切换重试与失败文案收口 — DONE

依赖：`FR-235`。

目标：门禁详情必须显示该次评估实际冻结的门禁 Agent 供应商、模型和思考强度；历史评估缺少单次记录时只可回退到本 Attempt 的冻结门禁策略，不得猜测工作区或当前默认模型。对可重试的门禁技术异常，用户可在对应门禁详情中显式选择另一可用供应商／模型后重新执行当前阶段；旧评估和其原配置必须保留，新的执行记录必须携带新的冻结配置并写入独立审计事件。平台输出合同校验不得显示供应商切换入口。失败说明必须覆盖自动和人工触发的用户自定义门禁技术故障，不能将其误称为“平台自动门禁”；弹窗在所有视口保持与页面顶部的安全间距。

验收：平台定向回归覆盖单次配置投影、受限供应商切换重试、平台门禁拒绝切换和历史配置回退；Web 类型检查／构建覆盖详情中的配置及重试入口；受影响 Python／TypeScript 基础检查、`git diff --check` 与任务状态唯一性通过。完成后使用独立 Git commit 并停止。
...
### FR-242 门禁侧栏按门禁维度收口 — DONE

依赖：`FR-241`。

目标：门禁结果侧栏只展示每个 Gate 的紧凑入口（自定义 Gate 显示为“门禁 1／门禁 2”，平台确定性校验显示为
“平台交付与映射校验记录”），辅以最新状态和记录数。门禁定义与逐条执行记录均为内部详情：用户打开对应 Gate
后才显示冻结定义、按时间倒序分页的执行记录与单条详情入口。不得折叠、删除或重写 GateEvaluation。

验收：Web 类型检查、构建和定向浏览器回归覆盖紧凑侧栏、Gate 详情打开、定义查看、执行记录分页与单条详情；
`git diff --check`、任务状态唯一性通过。完成后独立提交并停止。

完成：门禁结果侧栏现在仅展示按 Gate 维度聚合的紧凑入口，显示最近一次状态、摘要和记录数；自定义 Gate 以
“门禁 1／门禁 2”呈现，平台校验保留“平台交付与映射校验记录”。点击入口后才打开对应 Gate 的冻结定义、
...
### FR-247 FlowRun 候选输出集与门禁流转语义 — DONE

依赖：`FR-246`。

目标：将一次正式完成事件的必填输出冻结为后台候选输出集；重复投影按完成事件身份幂等，END 门禁只审查当前集并记录门禁结论，Fork 修订时替代旧集。页面和读取接口只返回当前集；下游只使用最新且门禁通过集中的固定 Artifact ID，拒绝历史输出和无序同字段查询结果。门禁业务失败与执行异常保持独立状态，旧 Artifact/门禁记录继续作为审计保留。

验收：受影响 Python `py_compile`、Ruff、候选集状态单元测试、迁移脚本解析、`git diff --check` 与任务状态唯一性通过；集成测试因本机 Docker daemon 不可用未运行。本切片使用独立 Git commit。

### FR-248 FlowRun 列表执行概览去单节点化 — DONE

依赖：`FR-247`。

目标：FlowRun 列表不得将最后创建的 NodeRun 伪称为唯一“当前节点”，因为一个 Run 可同时保留多条节点执行记录、重试与人工等待。列表状态列只呈现 FlowRun／Runtime 状态；执行概览必须按全部 NodeRun 提供总数、未结束、已完成、失败、已取消和需人工处理计数，且“需人工处理”须汇总全部活动记录的最新 Attempt，不能只取最后一条记录。
...
### FR-250 FlowRun 当前容器资源用量 — DONE

依赖：`FR-249`。

目标：运行资源不能只显示配置上限，必须同时展示当前绑定容器的 CPU 实时使用百分比、内存已用字节数和容器可写层已用字节数。用量必须经 Runtime Provider 从 Docker 当前观测读取，不能复用历史 generation、节点或 Attempt 数据。Docker 宿主不支持可写层配额时，不得将目标配置误报为已生效上限，应明确显示“无硬上限”。
...
### FR-252 连续运行历史输入与输出按需展示 — DONE

依赖：`FR-247`、`FR-251`。

目标：连续运行详情继续不内嵌运行级或 Attempt 级 Artifact 数组；用户选中任一历史或已完成 Attempt 后，工作台必须通过既有受父 FlowRun 和精确 Artifact ID 作用域约束的分页元数据接口，展示其已冻结的输入绑定与当前候选输出集。不得将 Artifact 内容、`storage_key` 或完整 Artifact 集重新加入轮询详情，不得改写候选输出集、输入绑定、Artifact、Attempt 或 FlowRun。

验收：补充浏览器回归覆盖已完成连续记录的输入和输出元数据展示；Web typecheck／production build、`git diff --check`、Alembic head 与任务状态唯一性通过；无迁移。
...
### FR-256 FlowRun 列表异步容器资源加载 — DONE

依赖：`FR-255`。

目标：恢复运行资源列，但首个 `GET /flow-runs` 响应只能读取数据库投影并优先渲染记录。当前容器的 Docker usage、容器 ID 和宿主机挂载路径必须经单独、按 FlowRun 请求的只读接口异步加载，任何 Docker 慢响应或失败均不得延迟列表。
...
### FR-262 Hook 编辑弹窗标题顶部留白修复 — DONE

依赖：`FR-260`。

目标：在保留标题与底部操作栏固定、中段表单独立滚动的前提下，恢复 Hook 编辑弹窗标题相对于弹窗上沿的产品留白，避免“新建 Hook”紧贴弹窗顶部。

完成：常规视口的标题区顶部内边距提升到 30px；较矮视口仍保留 22px 的明确顶部留白，不再将标题压缩到弹窗边缘。
...
完成：编辑器加宽为适合六项回调事件同排展示的桌面布局；事件卡、Matcher 示例、执行方式、上传区和说明区均缩小内边距与排版密度。常用桌面宽度显示六项事件一行，并将工具 Matcher／执行方式及文件上传／运行语义分别成对横排，消除主要配置流的纵向滚动；宽度不足时有序回退为三列，极窄或极矮视口才保留中段滚动。
...
### FR-265 Hook 中等桌面宽度紧凑布局修复 — DONE

依赖：`FR-264`。

目标：使 Hook 紧凑桌面布局适配浏览器缩放后的中等有效宽度：六个事件仍在一行展示，标题顶部留白稳定；仅在真正窄屏时才回退为三列。

完成：移除不必要的三列事件回退：FlowWeave 的桌面最小宽度下，约 780px 编辑器已经可承载六个紧凑事件卡，因此浏览器缩放后的中等有效宽度也固定使用六项同排和双列主体布局；标题留白规则使用 `section.modal.hook-editor` 最终精确选择器。
...
### FR-277 门禁调整会话即时反馈与定向进入 — DONE

依赖：`FR-276`。

目标：修复“根据门禁结果调整并重试”成功创建原生 Fork 后仍停留在旧 `END_BLOCKED` 页面、用户只能进入节点会话后自行发现新会话的问题。命令响应必须显式返回当前 Attempt 与安全的目标 binding ID，幂等重放返回同一目标；Web 在确认后立即显示创建中状态，成功后直接进入该调整会话，失败则保留当前页面并显示错误，不按 Attempt ID 是否变化猜测结果。

验收：服务层定向回归覆盖首次创建与幂等重放的 binding 身份；Web 定向 Playwright 覆盖确认、提交中反馈与成功后带 binding ID 的节点会话路由；受影响 Python/Web 静态检查、production build、Alembic head、任务状态唯一性与 `git diff --check` 通过。本切片独立提交并停止。
...
### FR-279 FlowRun 终态节点会话只读进入与工作区操作 — DONE

依赖：无（终态会话可用性修复）。

目标：已完成或已取消的 FlowRun 必须允许用户从每个历史节点重新进入 Agent 会话；会话事件和内容只读，新建会话、发送消息、重命名、分叉、模型切换、确认和其他会话控制继续由服务端拒绝。右侧持久工作区不随会话只读而失效：用户仍可浏览、新建和删除受范围保护的文件，并可打开交互式终端。不得恢复流程执行、重新创建 OpenHands Conversation，或放宽既有路径、租户和 Runtime 授权边界。
...
### FR-281 Agent 会话过程输出同级呈现与长命令截断 — DONE

依赖：FR-280。

目标：将共享 Agent 会话 Surface 的中间文本输出和工具调用收敛为与最终回复同级、同字号的可折叠过程区域，移除活动列表的左侧竖线与额外缩进；工具调用摘要和长命令必须受会话内容宽度约束，以单行省略展示，展开后仍可查看完整命令和结果。不得改动 OpenHands 事件、会话传输、Runtime 或后端数据合同。
...
### FR-289 Agent 工作区 Git 提交详情审查 — DONE

依赖：FR-288。

目标：中间提交审查页签不再展示原始 Diff；其上部将受影响文件按可展开目录树展示，下部展示完整提交说明、作者／提交者、邮箱、时间和完整 SHA。两个区域必须可上下拖拽调整高度，且不得放宽工作目录、路径或 Git 对象授权边界。

完成：提交详情接口补充作者／提交者姓名、邮箱和 ISO 时间以及完整 message；中间审查页以目录树取代平铺文件清单，移除原始 Diff 请求和渲染。文件树与提交详情之间加入可上下拖拽分隔条；详情区显示完整 SHA、多行提交说明、作者和提交者元数据。

### FR-290 Agent 工作区 Git 历史目录范围回归 — DONE

依赖：FR-289。

目标：Git 历史只跟随用户最初在文件树中选中的目录或其子路径所属仓库；不得自动回退到工作目录根、授权范围内的首个仓库或跨仓库选择。未选中仓库路径时，侧栏应明确提示用户先选择目录。
...
### FR-292 Agent 工作区 Git 条件侧栏与结构化 Diff — DONE

依赖：FR-291。

目标：Git 侧栏只在当前文件树选中的目录或文件属于 Git 仓库时显示；普通文件与非 Git 路径必须让中间工作区填满原有宽度，不显示空态侧栏。侧栏继续仅按当前选中路径的最深仓库匹配，不能恢复工作目录根或首个仓库回退。点击侧栏中的提交文件后，中间区域必须以带行号、删除／新增高亮的统一或并排 Diff 渲染文本补丁，而不是原始 `git diff` 输出。

完成：Git 仓库选择解析提升到工作区布局层，只有当前选中路径命中最深仓库时才挂载侧栏、预留右侧宽度并为文件视图启用 Git 网格；非 Git 路径和普通文件恢复全宽。提交文件 Diff 解析统一补丁 hunk，提供带行号的统一／并排视图、修改前后滚动对齐和新增／删除高亮；无可解析文本时保留明确空态。

### FR-293 Agent 工作区 Git 侧栏重复网格预留修复 — DONE

依赖：FR-292。

目标：在 Git 侧栏显示时，文件树、分隔条与文件预览仍只能使用三列；Git 侧栏由工作区容器的右侧预留宽度和绝对定位承载，文件视图不得再创建第四列，从而在文件预览与 Git 侧栏间留下空白。
...
### FR-296 Agent 工作区提交详情返回操作左置 — DONE

依赖：FR-295。

目标：提交详情侧栏的“返回提交历史”操作应位于标题左侧，符合从详情返回列表的阅读与导航直觉。
...
### FR-299 Agent 工作区 Git Diff 模式页签式横向布局 — DONE

依赖：FR-297。

目标：Git Diff 的“统一／并排”必须采用顶部“审查”页签的无描边、浅绿激活样式，并始终横向并排，不得在窄空间堆叠为纵向按钮。
...
### FR-301 Agent 工作区信息收口与输入区目录提示 — DONE

依赖：FR-300。

目标：右侧工作区概览不再显示“当前工作区”的绝对路径或 Runtime 容器身份；当前会话／草稿所属工作区应在输入框正上方仅显示名称，悬停时显示相对根工作区的 `./…` 路径。
...
### FR-303 Agent 工作区 Git 详情字段去重与文件树横向滚动 — DONE

依赖：FR-300。

目标：提交详情需保留短 Hash，但不重复显示与作者相同的提交者信息；深层目录中的文件名必须可完整辨认。

完成：详情字段恢复短 Hash（完整 Hash 仅作悬浮提示），仅保留作者与作者时间；提交树默认展开全部目录以直接显示变更文件，按内容原宽度布局，超过右侧栏时提供横向滚动，不再使用省略号截断文件名。

### FR-304 Agent 输入区工作区标签位置微调 — DONE

依赖：FR-302。

目标：工作区标签整体应再向右移动到输入区上方的指定视觉区域，文件夹图标和目录名必须作为一个整体移动。
...
完成：将可点击复制的工作区标签移动至 composer 之后，并以紧凑间距贴近发送框下沿；composer 与标签作为主面板底部悬浮层，不再创建 Grid 行或预留空白。会话内容可一直延伸到发送框边界，只有在发送框本身的覆盖范围内才被遮挡。当前会话运行期间，如已有来自成功正式工具结果的文件修改，发送框上方会显示可点击的紧凑增删行摘要，点击直接进入改动审查；会话停止、完成或暂停后自动隐藏。
...
### FR-310 Runtime 子进程密钥隔离与多层日志脱敏 — DONE
...
完成：固定 `1.47.0` 的 `sanitized_env()` 契约已锁定，Agent 驱动子进程会剥离
`OH_SECRET_KEY`、`SESSION_API_KEY` 与所有 `OH_SESSION_API_KEYS_*`。FlowWeave 在此上增加
独立第二边界：Runtime Provider handler 对日志记录脱敏，OpenHands event/Tool/模型投影递归屏蔽
敏感字段、`env`/`environment`/`headers` 容器、URL 凭据和可识别 API-key 字面量（含 `sk-oh-*`）。
...
完成：授权 Relay 的连接内投影现在识别 OpenHands `item_started`、`delta`、`item_aborted` 及
durable/transient envelope 的正式 `item_id`、attempt 与 order：只有已启动 slot 的递增 text delta
可到达浏览器，reasoning、无启动身份、过期 attempt 与重复/乱序 order 全部丢弃；较高 attempt 发送
短暂 reset 而非把重试内容追加到旧回复。matching durable event 或 abort 会关闭 slot，Relay 正常结束
也会关闭未完成 slot。该状态仅存在于一次授权 Relay 连接和浏览器内存，不写入 Conversation、cursor、
数据库或审计；旧 `/sockets/events` 的无身份 StreamingDeltaEvent 继续作为兼容 UX 通道。
`/sockets/session` 的实际接入、seq replay 与背压仍由 FR-313 独立实施。

### FR-313 Session Socket 授权 Relay 适配 — DONE
...
完成：三条浏览器 WebSocket 路由仍只连接 FlowWeave 授权 Relay；浏览器只在当前订阅 closure
保留已安全投影的最大 durable `seq`，重连时作为上游正式排他 `after_seq` 传回，页面卸载即丢弃，
不写入 browser storage、Conversation、locator、数据库或审计。OpenHands adapter 统一改接
`/sockets/session/{conversation_id}`，Provider relay 和直连路径均保留 Runtime 派生 session key
在受保护边界认证。Relay 忽略 sync、只安全投影 durable/transient 内层正式 Event，并以
`durable_cursor` 让浏览器仅更新内存锚点；未知或不安全帧不外泄。每个 replay 请求携带一次性
nonce，因此不会接入已越过其 replay 段的共享 Relay；live-only Relay 继续共享。Provider 总 hub
容量与慢消费者有界队列仍保护资源，Runtime 内遗留 Relay 的每会话清理上限提高至 16，避免一个
浏览器 replay 误中断另一浏览器的 live/replay 连接。
...
完成：固定 `1.47.0` 的 `EventLog.append()` 合同现额外锁定为先检查长度 marker、仅在 marker
不能证明同一长度时才目录扫描；普通单 writer append 不随 EventLog 历史线性退化。FlowWeave 不复制
该 marker、index 或 event cursor；Session Socket replay 保持由 OpenHands 逐页读盘。平台的按需
REST active-HEAD reconciliation 与遗留 fork branch recovery 共享 8 秒交互读 deadline，最多跨 8 个
原生 100-event 页面（800 个正式 Event）。超过预算返回明确 `RUNTIME_EVENT_HISTORY_BUDGET_EXHAUSTED`，
不会让一次恢复/状态读扫描无限长历史；显式历史分页与授权 Relay durable replay 仍为完整历史入口。
Provider Relay 的 hub、subscriber 和队列上限继续作为 fan-out 资源门禁，不新增平台 EventLog 或状态存储。

### FR-315 异步运行中用户消息与流式 idle 保活 — DONE
...
完成：固定 `30cf5832e` 源码契约锁定 `LocalConversation.arun()` 在异步 step 后重新比较
`last_user_message_id` 并继续消费运行中抵达的正式 user event，及 `EventService` 从 streaming delta
生产端以节流方式刷新 Server 的 idle clock。FlowWeave 不复制 run loop 或 heartbeat：普通 FlowRun 提问入口
现在与 Agent Workspace／节点会话一致，仅在正式 `running/executing` 状态直接提交原生
`POST /events` user message（`run=true`），不在活动 turn 内重绑模型、fork、压缩或中断；停止、暂停、
确认等非可续处理状态继续 fail closed。Adapter 对该原生 append 使用 3600 秒单请求等待边界，避免 Agent
持有 step lock 时短超时触发重复发送；仍只返回正式 cursor，不持久化 stream delta 或 idle 信号。
...
### FR-323 终态 FlowRun Runtime 停止与持久状态保留 — DONE

依赖：FR-322。

目标：FlowRun 完成或取消后，停止其 FlowRun/Node Attempt Agent Runtime 容器以回收 CPU、内存和网络，
但保留外置 Workspace、Conversation/Event、persistence 与 generation 审计；只有显式永久删除 Run 才能删除
这些持久资源。停止必须在既有取消任务完成后发生，以免与原生 cancel 并发。

完成：完成、自动完成或取消 FlowRun 时写入唯一 `STOP_FLOW_RUN_RUNTIMES` Runtime lane task。该任务先等待
所有 `CANCELLING/CANCEL_FAILED` Attempt settle，再将本 Run 的共享与 Attempt-owned Runtime Session fence 为
`STOPPED`，通过受管 Docker Provider drain 容器，并把 generation/ledger 投影为 `STOPPED`。外置 Workspace、
...
目标：Provider `ensure`/inspect 短暂 503 不能将仍实际运行的受管容器永久投影为 `ERROR`。恢复后必须重新
...
### FR-327 Background Task 积压批次追赶 — DONE

依赖：FR-325。

目标：终态任务的首次 retention 清理若达到批次上限，不能在存在历史积压时等待完整日常 cadence，
否则大表回收速度不足以抑制增长。

完成：若单次删除数等于配置 batch size，Worker 仅等待默认 60 秒的 backlog cadence 后领取下一小批；
直到删除数小于 batch size 后才恢复默认每日 cadence。删除筛选、事务边界、`SKIP LOCKED` 与 active lease
保护均不变。

### FR-328 历史终态 FlowRun Runtime 停止任务恢复 — DONE

依赖：FR-323。

目标：FR-323 发布前已经完成或取消的 FlowRun 不会自然产生
`STOP_FLOW_RUN_RUNTIMES` task，导致其仍受控的 Runtime 容器长期占用 CPU、内存和网络。Worker 启动与
维护恢复必须从终态 FlowRun/未停止 Runtime Session 这一持久事实幂等回补停止任务；不得触及
`ACTIVE`/`WAITING_HUMAN` Run、删除 Workspace／OpenHands persistence／allocation，或以停止旧容器替代正式
生命周期。

完成：`recover_runtime_tasks()` 现先以 `FOR UPDATE SKIP LOCKED` 扫描
`COMPLETED`/`CANCELLED` 且仍有非 `STOPPED`/`DELETING` Runtime Session 的 Run，并复用既有的
`stop-flow-run-runtimes:<run-id>` 唯一 key 回补停止任务。启动恢复与周期 maintenance 均经同一 delivery
恢复入口执行，重复扫描不会创建第二条任务；`ACTIVE`/`WAITING_HUMAN` Run 和停止／删除中的 Session 不在候选
...
目标：Agent Workspace 与 FlowNode 的 token 压缩保护使用 256,000 tokens 绝对阈值，不再按模型目录窗口的 80% 计算。新会话与新 Fork 必须通过 OpenHands 正式 `LLMSummarizingCondenser.max_tokens` 冻结该阈值；空闲发送、旧 FlowRun 问答和恢复入口必须只依据 OpenHands 当前主模型正式 usage，在达到阈值时先执行既有安全压缩验收。运行中追加不得并发修改 condenser；922,000 模型目录窗口、供应商配置、事件树和会话历史保持不变。
...
### FR-457 OpenHands 空响应自恢复与运行状态一致性 — DONE

依赖：FR-425、FR-436、FR-456。

目标：固定 OpenHands 1.47.0 在模型返回“无工具调用、无可见文本”时，会先持久化空 Agent Message，再以
`source=environment` 写入 corrective nudge 并继续 Agent loop。FlowWeave 不得把这两个非终态事件识别为最终
回复、停止左侧运行标记或把英文框架消息显示成普通对话正文。只有带可见正文的正式 Agent Message、
FinishAction 或 ERROR 才可成为事件树终态证据；纠正事件应显示为低干扰的中文自动恢复状态。底部动作、左侧
会话行和工作过程继续以 OpenHands 精确 `execution_status` 为运行事实，不修改 OpenHands、事件树或恢复循环。

验收：固定 OpenHands 1.47.0 `response_dispatch` 源码取证；Web TypeScript typecheck、受影响 ESLint、
production build、定向 Agent 工作台 Playwright、`git diff --check`、Alembic head 与任务状态唯一性通过；本切片
使用独立 Git commit。

完成：新增共享的 OpenHands MessageEvent 分类，只把来源为 Agent／Assistant 且含可见正文的消息作为正式回复。
空 Agent Message 不渲染且不结束当前轮；固定 `source=environment` corrective nudge 作为工作过程显示中文自动恢复
状态，其余 environment 消息继续作为非终态过程信息呈现。Agent 工作台的完成判定、回复收尾和 FlowRun 门禁审查
...
### FR-475 会话用量累计 Token 实时投影 — DONE

依赖：无。

目标：右侧既有“会话用量”只补充当前会话的 OpenHands 累计供应商 Token；不得新增用量字段、卡片、样式或底部 composer 内容，费用继续沿用已有持久化投影。

完成：右侧既有“累计 … Token”优先显示当前 `/context` 返回的 `cumulative_tokens`，缺失或瞬时零值时回退到既有持久化 usage summary。正式会话事件同步后使既有 context 查询失效并回读，因此供应商 usage 到达 OpenHands 后可更新右侧现有数值；未改变底部 Token／事件压缩指示器或任何视觉结构。
...
目标：将新建 Agent 会话冻结传给 OpenHands `LLMSummarizingCondenser` 的事件阈值从 `10,000` 收紧到 `1,000`，使当前 view 在超过 1,000 个事件时由 OpenHands 原生触发事件型压缩；不得改变 `256,000` token 阈值、既有会话已冻结配置、Conversation 事件、数据库或 OpenHands 源码。

完成：共享 Runtime 配置将 `CONDENSER_MAX_EVENTS` 设为 `1,000`。新建会话的 Runtime Agent spec 因而传入 `max_size=1,000`、`max_tokens=256,000`；OpenHands 继续以 `len(view) > max_size` 判定事件压力，故第 1,001 个 view 事件触发原生压缩。既有 Conversation 的 condenser 已由 OpenHands 持久化，保留原创建阈值，不进行隐式在线重配。

验收：定向 Runtime Agent spec pytest 尝试、受影响 Python Ruff／编译、`git diff --check`、Alembic head 与任务状态唯一性；未修改 OpenHands、数据库、Runtime Provider、Docker 或远端环境。
...
目标：连续运行与逐步运行的前端必须复用同一条记录选择、流程图状态、节点详情、会话回跳、输入／门禁／
产物查看与节点完成呈现链路。逐步运行只在一个已到达的后继节点上停留，保留用户配置及显式启动操作；
连续运行则在相同位置自动继续。除这个后继启动时机外，不得因运行方式产生不同的记录详情或节点交互。

范围：收敛 Workbench 的运行记录上下文、流程图和节点详情渲染分支，并补充逐步／连续记录的前端回归；
不修改 OpenHands、数据库迁移、Runtime Provider 或远端部署。

完成：已启动的连续／逐步记录现在统一解析为同一执行记录上下文，流程图使用相同的记录、当前节点和
Attempt，右侧详情也复用同一个 `AttemptPanel` 与会话返回上下文。输入、门禁、产物和节点完成状态不再因
...
### FR-481 普通 Agent 历史会话可写权限恢复 — DONE

依赖：FR-480。

目标：修复共享 Agent 会话组件启用逐会话 `write_available` 后，普通 Agent Workspace 会话响应缺失该字段、
导致所有历史会话被浏览器误判为只读的问题。普通工作区的详情、旧列表、分页列表及创建／Fork 等会话响应
统一显式返回工作区 Runtime 可写事实；Runtime `ACTIVE` 且工作区 `RUNNING` 时为可写，恢复中或维护态为只读。

范围：仅修改普通 Agent Workspace 会话 DTO 投影，并补定向服务端契约与浏览器回归；不恢复前端全局 Runtime
fallback，不修改 FlowRun 节点源会话只读／detached Fork 可写规则，不修改数据库、迁移、OpenHands、Runtime Provider、
Docker 或远端环境。

完成：普通 Agent Workspace 的详情、旧列表、分页列表及创建／Fork 等复用 `_dict` 的响应现在都显式返回
`write_available`。该字段仅由工作区 Runtime `ACTIVE` 与工作区 `RUNNING` 的共同事实决定；恢复中返回 `false`，
前端继续严格消费逐会话权限，不恢复会误放开 FlowRun 节点源会话的全局 Runtime fallback。定向浏览器回归验证
两个已完成历史会话切换后输入框均可编辑。

验收：Agent 历史会话定向 Playwright（1 passed）；Web TypeScript typecheck、受影响 ESLint、production build；
...
### FR-487 逐步／连续记录选择与操作栏一致性 — DONE

依赖：FR-479。

目标：逐步运行记录在左侧栏复用连续运行记录的选中态、Ctrl/Cmd 多选、Shift 范围选择、批量删除确认及
删除后即时隐藏；同时恢复直接启动与连续运行记录的删除操作。画布、右侧详情和三栏布局继续复用既有共享组件，
不改变两种模式的配置范围、启动时机或服务端状态机。

范围：仅修改 FlowRun Web 工作台与定向浏览器回归。逐步记录不新增连续运行专属的拷贝或导出入口；历史父
FlowRun 兼容记录保持可读且不可删除。不得修改 API、数据库、迁移、OpenHands、Runtime Provider、Docker
或远端环境。

完成：逐步记录栏现在复用连续运行的选中卡片、`Ctrl/Cmd` 多选、`Shift` 范围选择与批量删除工具栏；来自
会话返回的记录同样会恢复选中态。接受删除请求后，记录会立即从本地投影隐藏，批量请求仅恢复失败项；历史父
...
### FR-489 既有会话认证同步可用性与会话配置层级 — DONE

依赖：`FR-485`。

目标：

- 认证同步相关迁移尚未应用时，普通 Agent Workspace 和 FlowRun 节点会话的认证读取／同步接口必须返回
  不包含内部 SQL 的稳定、可行动错误，不能让前端收到未分类的 500。
- 既有会话的配置弹窗标题改为“会话配置”，以“能力／认证”为顶层导航；能力类别筛选继续作为能力页的
  二级导航。删除标题下的说明，避免两层导航和冗长说明挤占可用空间。
- 不改变 `FR-485` 冻结的同步语义：既有会话仍只在用户手动操作时同步，新会话继续默认注入当前认证。

验收：受影响 Python Ruff format/check、`py_compile`、认证同步 schema guard 定向 pytest、Web TypeScript
typecheck、定向 ESLint、Alembic 唯一 head、`git diff --check` 与任务状态唯一性；无 OpenHands 源码、Docker、
迁移实跑、远端或业务 E2E 操作。

完成：认证读取和手动同步入口会先检查认证同步所需的新增字段与表。缺少该 schema 时，仅将 PostgreSQL
`42703`／`42P01` 或 SQLite 的明确缺列／缺表错误转换为不含内部 SQL 的
`AGENT_CREDENTIAL_SYNC_SCHEMA_OUTDATED`（503）；其他数据库错误仍原样抛出。会话弹窗标题改为“会话配置”，
...
### FR-498 会话直接发送、暂停续答与队列原地编辑 — DONE

依赖：FR-429、FR-435、FR-440。

目标：已创建会话的普通发送无论当前原生会话处于运行、空闲还是暂停，均应同步投影为用户消息并异步提交给 OpenHands；
运行中消息明确标注为正在追加，暂停消息直接以新的 `run=true` 用户输入继续会话，不得要求用户先点击继续或将其误导为
排队。浏览器本地投递意图仍须在正式 HTTP cursor 返回前持久保留，但 WebSocket 到达的同文本文字不得作为确认或删除
依据。用户还应能在队列中原地修改尚未提交的消息，保留同一投递 ID、顺序及全部附件、引用和注释元数据。

范围：仅调整 Agent 工作台的浏览器本地投递投影、队列编辑交互与顶层 Agent 产品流回归。OpenHands 正式 cursor 仍是
成功确认的唯一事实；不修改 API、数据库、OpenHands、Runtime Provider、Docker 或远端环境。

完成：所有直接发送先在本地以用户消息投影，随后异步提交；运行中保留“正在追加到当前回复”状态，HTTP 返回正式 cursor
后才移除浏览器本地投递意图。WebSocket 的同文本事件只作为正式历史投影，不能凭文本删除本地意图。暂停会话的新消息同样
...
### FR-500 会话投递契约兼容与工作台连续定位交互 — DONE

依赖：FR-493、FR-497、FR-499。

目标：已冻结 Runtime 若未提供 OpenHands 当前 View `/context` 读取端点，仍须允许会话启动与消息投递；
当前 View Token 继续明确投影为未知，不能将一个用于可选观测的端点变成写入路径的阻断条件。Skill 组合弹出菜单在用户点击
弹窗内其他区域时应关闭，而不要求再次点击触发图标。用户消息定位条应根据指针在整条标尺中的连续位置，以每帧合并的波纹式
刻度反馈显示邻近项，不能再只在单个 tick 的进入／离开时跳变。

范围：仅修改 FlowWeave Runtime 契约兼容校验、Agent Workspace 会话配置菜单与 `ConversationSurface`
定位条的浏览器交互，并补充对应的纯逻辑／前端回归。`/context` 读取仍只服务于用量观测；不得修改消息请求、OpenHands
事件、数据库、Runtime Provider、Docker 或远端环境。
...
| 2026-09-25 | FIX-03 | 受影响 Python Ruff format/check、`py_compile`；Web TypeScript typecheck、受影响 ESLint、`git diff --check`；目标后端 pytest 与 Agent 工作台 Playwright 尝试 | PASS（静态）：活动快照通过正式 OpenHands `search?status=` 与运行中会话的正式事件窗口只读投影红色异常标记；未读继续是 binding 服务端字段，红色终态仅覆盖其视觉点，运行中异常仍可同时显示蓝点。后端 pytest 在 fixture 创建前被本机缺失 Docker socket 阻断；浏览器回归因本机 Vite 测试服务未启动而在 `page.goto` 前失败，均未记为通过。未修改 OpenHands Conversation、Runtime Provider、数据库、Docker 或远端环境。 |
| 2026-09-25 | FIX-04 | Web TypeScript typecheck、受影响 ESLint、`git diff --check` 与侧栏 E2E 断言更新 | PASS（静态）：运行中的“无可确认进展”红色感叹号具有心跳效果；`error`／`stuck` 的终态感叹号始终静态，减少动画偏好会停用心跳。未改变监测阈值、读取、未读投影或会话控制。 |
| 2026-09-25 | FIX-05 | Web TypeScript typecheck、受影响 ESLint、侧栏定向 Playwright（1 passed）、`git diff --check` 与任务状态唯一性 | PASS：红色异常提示现在只在 binding 为未读时出现，并与蓝色未读点同位互斥；运行中异常为心跳样式，终态异常为静态。浏览器回归确认进入运行中与终态异常会话都复用既有 `markConversationRead` 写回并消除红色标记；未修改 API、数据库、OpenHands、Runtime Provider、Docker 或远端环境。 |
| 2026-09-25 | FIX-06 | Python Ruff format/check、`py_compile`、Alembic 单头、Web TypeScript typecheck、受影响 ESLint、定向侧栏 Playwright（1 passed）、`git diff --check`；两类后端数据库回归尝试 | PASS（静态／浏览器）：`MANUAL` 未读始终显示蓝点，显式手动动作会覆盖既有 `SYSTEM` 异常来源；`SYSTEM` 未读异常仍显示同位红标，悬停删除按钮时红标隐藏。两类宿主 DTO/API 和迁移均已覆盖；定向后端 pytest 在 fixture 创建前被本机 Docker socket 缺失阻断，未进入断言。未改变 OpenHands、Runtime、会话控制或异常投影的只读性质。 |
| 2026-09-25 | FIX-07 | Web TypeScript typecheck、受影响 ESLint、`git diff --check`、定向 Agent 工作台 Playwright 运行至后续既有终态等待 | PASS（静态／目标断言）：会话内 `possibly_stuck` 状态行显示红色心跳感叹号；随后的空响应恢复状态不显示该图标。定向大场景在新断言之后、既有的“模型服务暂时不可用”终态等待处超时，未将此无关失败记为通过。未改变 OpenHands、Runtime、未读或会话控制。 |
...
| 2026-09-24 | FR-525 | 受影响 Python `py_compile`、Ruff format/check、`git diff --check`；Agent Workspace replacement 数据库回归已加入并尝试 | PASS（静态／受限数据库）：新增端点仅经 Agent Workspace 控制面 fence 活跃 generation，复用既有 `PROVISION_AGENT_WORKSPACE_RUNTIME` 任务与 Provider 生命周期，不调用 OpenHands 私有 reprovision 或 Docker。目标 pytest 在 collection 前被本机缺失 Docker socket 的全局 Testcontainers fixture 阻断，未进入断言；远端部署后必须以用户授权的目标 Workspace 读取 replacement 状态并重新读取 `/context`。 |
...
| 2026-09-24 | FIX-01 | 受影响 Python `py_compile`、Ruff format/check、列表 Runtime 调用静态断言、`git diff --check` 与任务状态唯一性；两组数据库定向 pytest 尝试；定向 Pyright | PASS（静态／隔离）：Agent Workspace 与 FlowRun 节点会话列表不再调用 `get_runtime`，列表状态明确为 `unknown`，当前选中会话仍走既有隔离读取路径。分页、排序、未读和写入权限的现有数据库回归已纳入目标；pytest 在 collection 前因本机 Docker socket 缺失、Testcontainers PostgreSQL 无法创建而阻断，未记为通过。Pyright 报告的 48 项均位于本切片未修改的既有未知类型与可能未绑定变量位置，未新增本切片诊断；Ruff 与编译通过。未修改 OpenHands、数据库、Runtime Provider、Docker 或远端环境。 |
...
| 2026-09-15 | FR-451 | 固定 OpenHands 1.47 LLM 对象序列化探针；受影响 OpenHands 适配器定向 pytest；Python Ruff format/check、`py_compile`；完整无 Docker `test_openhands.py`；`git diff --check` 与任务状态唯一性 | PASS：Chat Completions 新建与 Responses→Chat 切换 payload 显式使用 `api_mode=chat`，并准确传递 `reasoning_effort=None/high`；切换后若 Runtime 保留旧思考档位则以 `RUNTIME_LLM_BINDING_DRIFT` 拒绝。未修改 OpenHands 源码、数据库、迁移、会话事件或用户工作区。 |
...
| 2026-09-13 | FR-377 | Web TypeScript typecheck、ESLint、production build、Playwright 定向产品流、`git diff --check` 与任务状态唯一性 | PASS（静态／构建）：历史分页不再吞掉用户滚动；阅读锁仅在用户主动回到末行或点击浮层后才恢复自动跟随。生成中的浮层提升层级和对比度，保留脉冲与三点动画；顶部显示历史分页加载状态。产品流新增“流式输出期间上划阅读并跳回最新”的断言；完整执行在产品工作台前因本机 API／认证服务未启动而停留登录页，未执行该断言，未记为浏览器通过。未修改 API、数据库、Runtime Provider 或 OpenHands。 |
| 2026-09-13 | FR-376 | Web TypeScript typecheck、ESLint、production build、Playwright 用例收集、`git diff --check` 与任务状态唯一性 | PASS（静态／构建）：工作区路径现在位于实时任务标签之前，标签高度收紧为 20px，不再作为右下角独立块贴近会话底线。悬停／键盘聚焦仍显示同一 OpenHands 任务快照，浮层改由左上方展开并提升到 composer 上方。产品流断言覆盖 DOM 顺序、20px 高度和悬停详情；本机 API／认证服务未运行，未执行完整浏览器场景。未修改 API、数据库、Runtime Provider 或 OpenHands。 |
...
| 2026-09-12 | FR-312 | 固定 `30cf5832e` 的 `StreamContext`、Event Service、legacy/session socket 源码取证；新增 attempt/order/abort/durable-id/Relay-end 测试；`test_openhands.py`（116 passed）、Python Ruff format/check、`py_compile`、Web TypeScript typecheck、ESLint、Alembic head、`uv lock --check`、`git diff --check` 与任务状态唯一性 | PASS（静态／定向）：短暂流 slot 仅接受正式 stream identity，重试不会拼接旧文本，隐藏 reasoning 不能越过安全投影，abort、durable event 与 Relay 正常结束均清除浏览器短暂 slot；状态没有进入平台持久层。唯一 Alembic head 为 `0110_candidate_output_set_owner`，无 CURRENT，FR-313 为唯一 READY。Docker daemon 不可用，故真实 Runtime Relay、socket 断连/重试、镜像内 contract_check、session-socket backpressure/replay 及 Testcontainers 集成验证未执行且未记为通过。 |
...
| 2026-09-11 | FR-307 | Web TypeScript typecheck、ESLint、production build、`git diff --check` 与任务状态唯一性 | PASS：工作区标签位于发送框下方，composer/标签悬浮而不参与主面板 Grid 排版；会话内容延伸至发送框边界，不再在其上方预留空白。运行中的会话有成功正式工具结果产生文件修改时，发送框上方显示可点击的增删行摘要并直接进入改动审查，停止、完成或暂停后隐藏；相对路径复制、成功反馈与普通箭头光标保持不变；无 `CURRENT`。production build 仅报告既有大 chunk 提示。 |
