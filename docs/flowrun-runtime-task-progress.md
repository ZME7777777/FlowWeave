...
### FR-96 Agent 会话内核与默认宿主分层 — READY

依赖：`FR-95`。

目标：让 `agent_sessions` 只依赖中性会话 host context、locator 和 OpenHands bridge；Agent Workspace
仅作为默认 Runtime/工作目录/策略解析器及 API 适配。保持 `/agent` 全部功能和历史身份兼容，不改动
FlowRun、节点或 Attempt。
