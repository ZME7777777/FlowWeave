...
def test_annotation_rule_is_system_context_and_message_contains_no_reply_validation() -> None:
        ...
        annotations=(
            {
                ...
                "anchor": {"event_id": "native-event", "quote": "选中内容"},
                "comment": "用户评论",
            },
        ),
    )
    ...
    assert payload["collaboration_annotations"][0]["id"] == "annotation-1"
    assert "annotation_reply_instruction" not in payload
    display_content, references, workspace_references, annotations = (
        session_conversations.project_conversation_references(prompt)
    )
    assert display_content == "请开始修改"
    assert references == ()
    assert workspace_references == ()
    assert annotations == (
        {
...
def test_accepted_flow_node_keeps_source_read_only_but_allows_detached_conversations(
    ...
    with db_session_factory() as db:
        ...
        source = AgentConversationBinding(
            runtime_session_id=runtime_session_id,
            host_kind="FLOW_NODE",
            host_id=flow_run_id,
            conversation_scope_id=attempt_id,
            ...
            openhands_conversation_id=str(uuid4()),
            display_title="源会话",
            create_idempotency_key="completed-node-source",
            lifecycle="ACTIVE",
        )
        db.add(source)
        db.flush()
        assert flow_node_conversations._node_session_dict(db, source)["write_available"] is False
        with pytest.raises(DomainError) as source_write:
        ...
        assert source_write.value.code == "NODE_ATTEMPT_COMPLETED"
...
def test_idle_node_message_dispatch_rebinds_then_sends_without_database(
    ...
    class IdleRuntime:
        ...
        def conversation_context(self, _handle: RuntimeHandle) -> dict[str, int]:
            return {"used_tokens": 409_599, "condenser_max_tokens": 409_600}
...
def test_idle_node_message_dispatch_condenses_at_the_frozen_native_threshold(
    ...
    class ThresholdRuntime:
        ...
        def conversation_context(self, _handle: RuntimeHandle) -> dict[str, int]:
            return {"used_tokens": 409_600, "condenser_max_tokens": 409_600}
