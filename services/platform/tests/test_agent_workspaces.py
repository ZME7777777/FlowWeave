...
def test_agent_workspace_conversation_create_is_idempotent_and_uses_external_identity(
    ...
    with settings_context(settings), db_session_factory() as db, runtime_context(runtime):
        ...
        assert runtime.request.agent_spec.agent_context.system_message_suffix == (
            ...
            "仅在开始执行、计划或风险发生变化、完成关键阶段、需要用户决定或最终交付时，给出简短进展；"
            "不要在普通工具调用前逐条解释命令。\n"
            "独立且只读的检查可以在同一轮模型输出中并行发起；存在数据依赖、写入、Git 提交/推送、"
            "部署、"
            "权限确认或其他风险的动作必须串行，先读取并验证前一步结果。\n"
                ...
                events=(
                    ...
                    RuntimeEvent(
                        ...
                        payload={
                            "source": "user",
                            "content": "latest",
                            "parent_id": "earlier-answer",
                        },
                    ),
                    RuntimeEvent(
                        cursor="earlier-answer",
                        event_type="MESSAGE",
                        payload={
                            "source": "agent",
                            "content": "earlier",
                            "parent_id": "earlier-user",
                        },
                    ),
                    RuntimeEvent(
                        cursor="earlier-user",
                        event_type="MESSAGE",
                        payload={"source": "user", "content": "old", "parent_id": "__root__"},
                    ),
                ),
                cursor="latest-error",
            )
