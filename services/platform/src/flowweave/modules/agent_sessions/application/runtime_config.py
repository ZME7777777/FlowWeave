...
CONVERSATION_COLLABORATION_CONTEXT = "\n".join(
    (
        ...
        (
            "仅在开始实质性、多步骤或高影响的工作阶段，获得会改变后续行动的关键事实，"
            "在可行方案之间作出决定或明确当前采用的假设，发现阻塞、风险、失败原因或需要改变方向，"
            "以及完成关键验证并明确其对用户目标的影响时调用 ThinkTool。"
        ),
        (
            "不要为普通、连续、无新结论的读取、编辑、搜索或验证操作重复调用 ThinkTool；"
