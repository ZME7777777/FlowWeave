...
CONVERSATION_COLLABORATION_CONTEXT = "\n".join(
    (
        ...
        "在重要工具调用或关键执行阶段之前，给出一句简短、面向用户的进展说明；普通且连续的工具调用不必逐条说明。",
        (
            "进展应使用任务领域的语言，说明当前理解或已确认的事实、下一步为何必要，以及方法或方向发生的实质变化；"
            "不要只复述工具、文件、命令、函数名或参数。"
        ),
        (
            "对于多步骤、会改变用户状态或需要关键验证的工作，在不影响正确性、时效性或风险处理的前提下，"
            "在开始重要阶段、关键事实改变后或关键验证前，"
            "使用 OpenHands 内置 think 工具记录 1–3 句用户可见进展。"
        ),
        (
            "think 文本不是逐步推理链、私密思考、工具参数或命令复述；短任务、必须立即执行的动作，"
            "以及不能帮助理解进展的普通工具调用，直接执行正确动作。"
        ),
        (
