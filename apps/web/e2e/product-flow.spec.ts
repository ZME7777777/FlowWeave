...
test('top-level Agent workspace creates a direct conversation and restores its URL', async ({ page }) => {
  ...
  await page.getByLabel('消息投递队列').getByRole('menuitem', { name: '编辑' }).click();
  ...
  await composer.fill('第二条排队消息');
  await composer.press('Enter');
  await expect(page.getByLabel('消息投递队列').getByText('第二条排队消息')).toBeVisible();
  await expect(page.getByLabel('消息投递队列').locator('article')).toHaveCount(2);
  await page.getByLabel('消息投递队列').getByRole('button', { name: '更多排队消息操作 2' }).click();
  await page.getByLabel('消息投递队列').getByRole('menuitem', { name: '上移' }).click();
  await expect(page.getByLabel('消息投递队列').locator('article').first()).toContainText('第二条排队消息');
  await page.getByLabel('消息投递队列').locator('article').first().getByRole('button', { name: /移除排队消息/ }).click();
  await page.getByLabel('消息投递队列').getByRole('button', { name: /移除排队消息/ }).click();
