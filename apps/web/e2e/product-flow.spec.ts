...
test('top-level Agent workspace creates a direct conversation and restores its URL', async ({ page }) => {
  ...
  await page.route('**/api/v1/agent-workspaces/**', async route => {
    ...
    if (path.endsWith('/context')) {
      ...
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(forkContext ? {
        used_tokens: null, window_tokens: 922_000, cumulative_tokens: 0, condenser_max_tokens: 409_600,
        ...
        used_tokens: null, window_tokens: 922_000, cumulative_tokens: 12_716, condenser_max_tokens: 409_600,
  ...
  const sidebarStagedHeightBeforeResize = await sidebarStagedTree.evaluate(element => element.getBoundingClientRect().height);
  await page.mouse.move(sidebarResizerBox.x + sidebarResizerBox.width / 2, sidebarResizerBox.y + sidebarResizerBox.height / 2);
  await page.mouse.down();
  await page.mouse.move(sidebarResizerBox.x + sidebarResizerBox.width / 2, sidebarResizerBox.y + 50, { steps: 6 });
  await page.mouse.up();
  await expect.poll(() => sidebarStagedTree.evaluate(element => element.getBoundingClientRect().height)).toBeGreaterThan(sidebarStagedHeightBeforeResize + 20);
  ...
  await sidebarStagedTree.getByText('staged.ts', { exact: true }).click();
  const workingDiffFileTrees = page.locator('.agent-git-working-file-trees');
  const reviewStagedTree = workingDiffFileTrees.getByRole('navigation', { name: '暂存区' });
  const reviewUnstagedTree = workingDiffFileTrees.getByRole('navigation', { name: '未暂存' });
  await expect(reviewStagedTree.getByText('staged.ts', { exact: true })).toBeVisible();
  await expect(reviewUnstagedTree.getByText('local.ts', { exact: true })).toBeVisible();
  await expect(reviewUnstagedTree.getByText('new.ts', { exact: true })).toBeVisible();
  const reviewStagedHeightBeforeCollapse = await reviewStagedTree.evaluate(element => element.getBoundingClientRect().height);
  await reviewStagedTree.getByRole('button', { name: '收起暂存区' }).click();
  await expect(reviewStagedTree.getByText('staged.ts', { exact: true })).toBeHidden();
  await expect.poll(() => reviewUnstagedTree.evaluate(element => element.getBoundingClientRect().height)).toBeGreaterThan(reviewStagedHeightBeforeCollapse);
  await reviewStagedTree.getByRole('button', { name: '展开暂存区' }).click();
  await expect(workingDiffFileTrees.getByRole('separator', { name: '调整暂存区和未暂存区高度' })).toBeVisible();
  await reviewUnstagedTree.getByText('local.ts', { exact: true }).click();
  ...
  await expect(page.locator('.agent-context-progress.token')).toHaveAttribute('title', /OpenHands 当前 View 6,380 \/ 上下文窗口 512,000.*达到 80%（409,600 Token）时自动压缩/);
  ...
  await trackerCard.getByText('检查构建', { exact: true }).click();
  ...
  await expect(page.locator('.conversation-operation-group')).toHaveCount(0);
  ...
  const liveProgressSummary = liveProgressGroup.locator(':scope > summary');
  const liveProgressIcons = liveProgressSummary.locator('.conversation-progress-icons');
  await expect(liveProgressSummary.locator(':scope > svg.lucide-square-terminal')).toHaveCount(0);
  await expect(liveProgressIcons.locator(':scope > svg')).toHaveCount(1);
  await expect(liveProgressIcons.locator(':scope > svg').nth(0)).toHaveClass(/lucide-square-terminal/);
  ...
  await expect(liveProgressIcons.locator(':scope > svg')).toHaveCount(2);
  await expect(liveProgressIcons.locator(':scope > svg').nth(0)).toHaveClass(/lucide-square-terminal/);
  await expect(liveProgressIcons.locator(':scope > svg').nth(1)).toHaveClass(/lucide-square-terminal/);
  for (let index = 2; index < 6; index += 1) {
    agentStream!.send(JSON.stringify({
      type: 'event',
      event: { id: `live-tool-${index}`, event_type: 'TOOL_CALL', payload: { parent_id: `live-tool-${index - 1}`, action_id: `live-tool-${index}`, tool_call_id: `live-call-${index}`, llm_response_id: 'live-operation-batch-1', tool_name: 'terminal', event_name: 'TerminalAction', details: { command: `echo ${index}` }, timestamp: new Date().toISOString() } },
    }));
  }
  ...
  const liveFileSummary = liveFileProgressGroup.locator(':scope > summary');
  await expect(liveFileSummary.locator(':scope > svg.lucide-file-pen-line')).toHaveCount(0);
  await expect(liveFileSummary.locator('.conversation-progress-icons > svg')).toHaveCount(1);
  ...
  await liveProgressGroup.locator(':scope > summary').click();
  const completedStatusDetail = liveProgressGroup.locator('.conversation-tool-detail').filter({ hasText: '已运行 git status --short' });
  const completedPwdDetail = liveProgressGroup.locator('.conversation-tool-detail').filter({ hasText: '已运行 pwd' });
  await expect(completedStatusDetail).not.toHaveClass(/running/);
  await expect(completedPwdDetail).not.toHaveClass(/running/);
  await expect(completedStatusDetail.locator(':scope > summary b')).toHaveCSS('animation-name', 'none');
  await expect(completedPwdDetail.locator(':scope > summary b')).toHaveCSS('animation-name', 'none');
  await expect(page.locator('.conversation-turn-status')).toHaveText('OpenHands 会话连接正常，等待响应');
  ...
  await expect(queueMenu).toBeHidden();
  await queueMoreButton.click();
  await expect(queueMenu).toBeVisible();
  ...
  await composer.fill('已编辑的排队消息');
  await composer.press('Enter');
  await expect(page.getByLabel('消息投递队列').getByText('已编辑的排队消息')).toBeVisible();
  await page.getByLabel('消息投递队列').getByRole('button', { name: '更多排队消息操作 1' }).click();
  await page.getByLabel('消息投递队列').getByRole('menuitem', { name: '关闭队列模式' }).click();
  await expect(page.locator('.agent-composer-note')).toHaveText('队列已关闭 · 保留 1 条');
  await page.reload();
  await expect(page.locator('.agent-composer-note')).toHaveText('队列已关闭 · 保留 1 条');
  await expect(page.getByLabel('消息投递队列').getByRole('button', { name: '调整方向排队消息 1' })).toBeDisabled();
  await composer.fill('关闭后不应排队');
  await composer.press('Enter');
  await composer.press('Meta+Enter');
  await expect(composer).toHaveValue('关闭后不应排队');
  await expect(page.getByLabel('消息投递队列').locator('article')).toHaveCount(1);
  expect(sentMessages).toBe(sentBeforeQueue);
  modelIsResponding = false;
  await page.reload();
  await expect(page.getByLabel('消息投递队列').getByText('已编辑的排队消息')).toBeVisible();
  await expect.poll(() => sentMessages).toBe(sentBeforeQueue);
  modelIsResponding = true;
  await page.reload();
  await page.getByLabel('消息投递队列').getByRole('button', { name: '更多排队消息操作 1' }).click();
  await page.getByLabel('消息投递队列').getByRole('menuitem', { name: '启用队列模式' }).click();
  ...
  await expect(activeProcess.locator(':scope > summary .conversation-activity-spinner-slot')).toHaveClass(/active/);
...
test('returning to a conversation completed in the background immediately restores its final result', async ({ page }) => {
  const now = new Date().toISOString();
  let terminal = false;
  let terminalEventReads = 0;
  const conversation = {
    id: 'background-complete-conversation', display_title: '后台完成', lifecycle: 'ACTIVE',
    streaming_callback_ready: true, write_available: true, model_provider_id: null, model_name: null, reasoning_effort: null,
    execution_status: 'running', created_at: now, updated_at: now,
  };
  await page.route('**/api/v1/auth/me', route => route.fulfill({
    status: 200,
    contentType: 'application/json',
    body: JSON.stringify({ id: 'background-complete-user', username: 'tester', role: 'USER', is_super_admin: false }),
  }));
  await page.routeWebSocket('**/agent-workspaces/**/stream', () => undefined);
  await page.route('**/api/v1/agent-workspaces/**', async route => {
    const request = route.request();
    const path = new URL(request.url()).pathname;
    if (path.endsWith('/default')) return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ id: 'background-complete-workspace', display_name: 'Agent 工作区', desired_state: 'RUNNING', updated_at: now }) });
    ...
    if (path.endsWith('/conversations') && request.method() === 'GET') return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ items: [{ ...conversation, execution_status: terminal ? 'idle' : 'running' }], next_cursor: null }) });
    if (path.endsWith('/events')) {
      if (terminal) terminalEventReads += 1;
      return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({
        events: [
          { id: 'background-user', event_type: 'MESSAGE', payload: { source: 'user', parent_id: '__root__', content: '请在后台完成', timestamp: now } },
          ...(terminal ? [{ id: 'background-finish', event_type: 'COMPLETED', payload: { source: 'agent', parent_id: 'background-user', event_name: 'FinishAction', content: '后台任务已完成。', timestamp: now } }] : []),
        ],
        next_cursor: terminal ? null : 'background-user',
      }) });
    }
    ...
    if (path.endsWith('/workspace')) return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ root: '/runtime/workspace/project', scope: { kind: 'ROOT', display_name: '根工作区' }, working_directory: '/runtime/workspace/project', work_directory: null, files: [], repositories: [], runtime: { container_id: 'background-complete-runtime' }, ide: { workspace_path: '/runtime/workspace/project', gateway: { supported: false, status: '未配置', note: '' } } }) });
  ...
  await login(page);
  await page.goto('/agent/conversations/background-complete-conversation');
  await expect(page.getByRole('button', { name: '暂停当前 Agent' })).toBeVisible();
  await page.evaluate(() => {
  ...
  await expect.poll(() => terminalEventReads, { timeout: 1_500 }).toBeGreaterThan(0);
  await expect(page.getByText('后台任务已完成。'), { timeout: 1_500 }).toBeVisible();
  await expect(page.getByRole('button', { name: '发送消息' })).toBeVisible();
  await expect(page.getByText('正在同步上一轮结束状态…')).toHaveCount(0);
});

test('terminal reconciliation retries the latest event window when the durable reply arrives late', async ({ page }) => {
  const now = new Date().toISOString();
  let messageAccepted = false;
  let readinessReadsAfterSend = 0;
  let terminalLatestReads = 0;
  const conversation = {
    id: 'late-final-reply-conversation', display_title: '延迟落盘回复', lifecycle: 'ACTIVE',
  ...
  await page.route('**/api/v1/auth/me', route => route.fulfill({
    status: 200,
    contentType: 'application/json',
    body: JSON.stringify({ id: 'late-final-reply-user', username: 'tester', role: 'USER', is_super_admin: false }),
  }));
  await page.routeWebSocket('**/agent-workspaces/**/stream', () => undefined);
  await page.route('**/api/v1/agent-workspaces/**', async route => {
    const request = route.request();
    const path = new URL(request.url()).pathname;
    if (path.endsWith('/default')) return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ id: 'late-final-reply-workspace', display_name: 'Agent 工作区', desired_state: 'RUNNING', updated_at: now }) });
    if (path.endsWith('/runtime')) return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ state: 'ACTIVE', write_available: true, message: null, updated_at: now }) });
    ...
    if (path.endsWith('/events')) {
      const eventUrl = new URL(request.url());
      const cursor = eventUrl.searchParams.get('cursor');
      const terminalWindow = messageAccepted && readinessReadsAfterSend >= 2 && !cursor;
      if (terminalWindow) terminalLatestReads += 1;
      const replyAvailable = terminalWindow && terminalLatestReads >= 2;
      return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({
        events: replyAvailable ? [
          { id: 'late-final-user', event_type: 'MESSAGE', payload: { source: 'user', parent_id: '__root__', content: '等待最终回复', timestamp: now } },
          { id: 'late-final-assistant', event_type: 'MESSAGE', payload: { source: 'agent', parent_id: 'late-final-user', content: '无需刷新即可显示的最终回复。', timestamp: now } },
        ] : [],
        next_cursor: replyAvailable ? 'late-final-assistant' : cursor,
        history_cursor: null,
      }) });
    }
    if (path.endsWith('/input-readiness')) {
      if (!messageAccepted) return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ ready: true, execution_status: 'idle' }) });
      readinessReadsAfterSend += 1;
      return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(readinessReadsAfterSend === 1
        ? { ready: false, execution_status: 'running' }
        : { ready: true, execution_status: 'idle' }) });
    }
    if (path.endsWith('/messages') && request.method() === 'POST') {
      messageAccepted = true;
      return route.fulfill({ status: 202, contentType: 'application/json', body: JSON.stringify({ accepted: true, cursor: 'late-final-user' }) });
    }
    if (path.endsWith('/work-directories')) return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ root: { kind: 'ROOT', display_name: '根工作区', working_directory: '/runtime/workspace/project' }, items: [] }) });
    if (path.endsWith('/workspace')) return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ root: '/runtime/workspace/project', scope: { kind: 'ROOT', display_name: '根工作区' }, working_directory: '/runtime/workspace/project', work_directory: null, files: [], repositories: [], runtime: { container_id: 'late-final-reply-runtime' }, ide: { workspace_path: '/runtime/workspace/project', gateway: { supported: false, status: '未配置', note: '' } } }) });
    if (path.endsWith('/context')) return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ model_name: null, reasoning_effort: null }) });
    if (path.endsWith('/pending-confirmation')) return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ pending: false }) });
    if (path.endsWith('/capabilities')) return route.fulfill({ status: 200, contentType: 'application/json', body: '[]' });
    return route.fulfill({ status: 404, contentType: 'application/json', body: JSON.stringify({ error: { code: 'RESOURCE_NOT_FOUND', message: 'not found' } }) });
  ...
  await login(page);
  await page.goto('/agent/conversations/late-final-reply-conversation');
  const composer = page.getByLabel('发送 Agent 消息');
  await composer.fill('等待最终回复');
  await composer.press('Enter');

  await expect.poll(() => readinessReadsAfterSend).toBeGreaterThanOrEqual(2);
  await expect.poll(() => terminalLatestReads).toBeGreaterThanOrEqual(1);
  await expect(page.getByText('无需刷新即可显示的最终回复。'), { timeout: 3_000 }).toBeVisible();
  await expect(page.locator('.conversation-turn-status')).toHaveCount(0);
  await expect(page.getByRole('button', { name: '发送消息' })).toBeVisible();
});
...
test('terminal readiness without a formal result keeps reconciliation non-blocking', async ({ page }) => {
  ...
  terminal = true;
  await page.reload();
  // Native readiness owns whether new input is accepted. The transcript may
  // retain running presentation only during the bounded final-event recovery,
  // then settles without blocking the composer.
  await expect(page.getByRole('button', { name: '发送消息' })).toBeVisible();
  await expect(page.getByRole('button', { name: '正在同步会话结束' })).toHaveCount(0);
  await expect(page.locator('.conversation-turn-status')).toHaveCount(1);
  await expect(page.locator('.conversation-turn-status')).toHaveCount(0, { timeout: 3_000 });
