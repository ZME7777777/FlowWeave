...
function taskBoundaryEventIds(entries: ActivityEntry[]): ReadonlySet<string> {
  ...
  // Preserve the full formal lineage: a subagent may emit an ordinary terminal
  // or file action below the TaskAction rather than repeat runtime_task on each
  // child event.  Event ids and parent ids are OpenHands identities, so this
  // never guesses ownership from labels or display text.
...
function isAggregateableOperation(entry: ActivityEntry, taskBoundaryIds: ReadonlySet<string>): boolean {
  ...
  // A TaskAction/Observation is a formal subagent lifecycle boundary.  Do not
  // collapse it, or an action directly nested beneath it, into the parent's
  // command batch merely because their text happens to look alike.
