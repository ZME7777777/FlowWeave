...
import { ConversationSurface, type ConversationReference } from '../ConversationSurface';
...
function RuntimeTaskTab({ tasks, definitions, selectedTaskId, onSelect, onInterrupt, interrupting }: {
  ...
  return <section className="agent-subagent-tab" aria-label="子智能体记录">
    ...
    <RuntimeTaskRecord task={selectedTask} definitions={definitions} onInterrupt={onInterrupt} interrupting={interrupting}/>
  </section>;
}
...
type AgentCapabilityType = 'SKILL' | 'MCP' | 'PLUGIN' | 'CONTEXT' | 'AGENT_DEFINITION';
...
function ComposerCapabilityAutocomplete({
  draft, suggestions, disabled, placeholder, onDraftChange, onPaste, onDropFiles, onDropWorkspaceFiles, onSubmit, onDirectSubmit, onManageCapabilities, onNativeAction,
}: {
...
function CapabilityManager({ workspaceId, bindingId, conversationCapabilities, draftCapabilityIds, onClose, onCreateEnhancedConversation }: {
  ...
  const frozenCreationOnlyIds = useMemo(() => new Set(
    (conversationCapabilities ?? [])
      .filter(item => item.capability_type === 'CONTEXT' || item.capability_type === 'AGENT_DEFINITION')
      .map(item => item.id),
  ), [conversationCapabilities]);
  ...
  const capabilities = useMemo(() => (catalogQuery.data ?? []).filter(item =>
    ['SKILL', 'MCP', 'PLUGIN', 'CONTEXT', 'AGENT_DEFINITION'].includes(item.capability_type)
    && (item.is_latest || frozenCreationOnlyIds.has(item.id))
    && (!bindingId || !['CONTEXT', 'AGENT_DEFINITION'].includes(item.capability_type) || frozenCreationOnlyIds.has(item.id)),
  ), [bindingId, catalogQuery.data, frozenCreationOnlyIds]);
  const visible = useMemo(() => {
    const needle = query.trim().toLocaleLowerCase();
    return capabilities.filter(item =>
      (kind === 'ALL' || item.capability_type === kind)
      && (!bindingId || !['CONTEXT', 'AGENT_DEFINITION'].includes(item.capability_type) || kind === item.capability_type)
  ...
  const readonlyCreationOnly = Boolean(bindingId && (kind === 'CONTEXT' || kind === 'AGENT_DEFINITION'));
  ...
  return <div className="agent-capability-backdrop" role="presentation" onPointerDown={event => { if (event.target === event.currentTarget && !save.isPending) onClose(); }}>
    <section ref={dialog} className="agent-capability-manager" role="dialog" aria-modal="true" aria-labelledby="agent-capability-title" onKeyDown={trapFocus}>
      <header><div><span className="eyebrow">{bindingId ? 'CURRENT AGENT SESSION' : 'NEW AGENT SESSION'}</span><h2 id="agent-capability-title">能力</h2><p>{bindingId ? '可为当前会话注册新的已发布 Skill、MCP 或 Plugin。Context 和 Agent 仅在创建会话时冻结；可在对应标签查看已装配版本，但不能新增、编辑或删除。' : '新会话默认不挂载能力；在这里选择的版本只会冻结到即将创建的这一个会话。Context 会作为 OpenHands 系统级会话上下文，Agent 会作为 OpenHands 原生子 Agent 定义。'}</p></div><button ref={closeButton} type="button" aria-label="关闭插件管理" disabled={save.isPending} onClick={onClose}><X size={18}/></button></header>
      <div className="agent-capability-toolbar"><div className="agent-capability-tabs">{([['ALL', 'all'], ['PLUGIN', 'plugin'], ['MCP', 'MCP'], ['SKILL', 'skill'], ['CONTEXT', 'context'], ['AGENT_DEFINITION', 'agent']] as const).map(([value, label]) => <button type="button" key={value} className={kind === value ? 'active' : ''} onClick={() => setKind(value)}>{label}</button>)}</div><div className="agent-capability-toolbar-actions">{!readonlyCreationOnly && <button type="button" className="agent-capability-select-visible" disabled={!selectableVisibleIds.length} onClick={toggleVisible}>{allVisibleSelected ? '取消选择筛选结果' : `选择筛选结果 (${selectableVisibleIds.length})`}</button>}<label className="agent-capability-search"><Search size={15}/><input value={query} onChange={event => setQuery(event.target.value)} placeholder="搜索名称、说明或文件…"/></label></div></div>
      <div className="agent-capability-summary"><span>{readonlyCreationOnly ? <>已装配 <b>{kind === 'CONTEXT' ? frozenContextCount : frozenAgentCount}</b> 个 {kind === 'CONTEXT' ? 'Context' : 'Agent'}</> : <>{bindingId ? '已注册' : '已选择'} <b>{selectedIds.length}</b> 项</>}</span><span>{readonlyCreationOnly ? `${kind === 'CONTEXT' ? 'Context' : 'Agent'} 在创建会话时冻结，仅供查看，不能新增、编辑、取消或删除。` : bindingId ? '可继续注册新能力；已注册能力已锁定，不能取消或删除。' : '选择只作用于本次新会话，不会改变工作区或其他会话。'}</span></div>
...
function AgentSessionWorkbenchContent({ onNavigate, onReturnToSource, onHostStateChanged, autoOpenDraft = false, hideDraftTitle = false }: Omit<AgentSessionWorkbenchProps, 'gateway' | 'host'>) {
    ...
    if (conversationDraft) {
      if (canBootstrap && !bootstrap.isPending) {
        ...
        setOptimisticBootstrapTurn({
          ...
          event: { id: `pending-bootstrap:${message.id}`, event_type: 'MESSAGE', payload: { source: 'user', content, attachments, conversation_references: references.map(item => ({ event_id: item.eventId, content: item.content })) } },
        });
        setRequestStartedAt(Date.now());
        setTurnState('running');
        bootstrap.mutate(message);
      }
      else { setDraft(content); setAttachments(attachments); setReferences(references); }
      return;
    }
    if (!canWrite) { setDraft(content); setAttachments(attachments); setReferences(references); return; }
    if (turnState === 'idle') {
  ...
  return <main className="agent-workbench-page">
    ...
    <section className="agent-workbench-main">
      <header className="agent-workbench-header"><div><span className="eyebrow">DIRECT AGENT SESSION</span>{editing ? <div className="agent-title-edit"><input ref={titleInput} aria-label="会话标题" value={title} onChange={event => setTitle(event.target.value)} onBlur={() => { if (!rename.isPending) { setTitle(selected ? conversationName(selected) : ''); setEditing(false); } }} onKeyDown={event => { if (event.key === 'Enter' && title.trim()) { event.preventDefault(); rename.mutate(); } if (event.key === 'Escape') { setTitle(selected ? conversationName(selected) : ''); setEditing(false); } }}/></div> : !(hideDraftTitle && conversationDraft) && <h2 className="agent-session-title" title={selected ? conversationName(selected) : undefined} aria-label={selected && canWrite ? '双击修改标题' : undefined} onDoubleClick={() => { if (!selected || !canWrite) return; setTitle(conversationName(selected)); setEditing(true); }}><span>{selected ? conversationName(selected) : conversationDraft ? '新会话' : '开始一个新的会话'}</span></h2>}{features.modelSelection && (selected || conversationDraft) && <small className="agent-session-provider">当前供应商：{selected ? boundProviderInfo?.name ?? '未配置' : draftProviderInfo?.name ?? '请选择模型供应商'}{conversationDraft ? ` · ${conversationDraft.displayName}` : ''}</small>}</div><div className="agent-header-actions">{features.conversationDeletion && selected && <button type="button" className="danger" aria-label="删除会话" disabled={!canWrite || remove.isPending} onClick={() => remove.mutate(selected.id)}><Trash2 size={14}/></button>}</div></header>
