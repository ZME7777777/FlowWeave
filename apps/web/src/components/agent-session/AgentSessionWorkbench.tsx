...
function AgentSessionWorkbenchContent({ onNavigate, onReturnToSource, onHostStateChanged, autoOpenDraft = false, hideDraftTitle = false }: Omit<AgentSessionWorkbenchProps, 'gateway' | 'host'>) {
  ...
  const liveSessionChanges = selected && (isGenerating || selectedConversationRunning)
    ? sessionFileChanges
    : [];
  const liveSessionChangeAdditions = liveSessionChanges.reduce((total, change) => total + change.additions, 0);
  const liveSessionChangeDeletions = liveSessionChanges.reduce((total, change) => total + change.deletions, 0);
  ...
  return <main className="agent-workbench-page">
    ...
    <section className="agent-workbench-main">
      ...
      {(selected || conversationDraft) && runtimeWritable && runtime?.state !== 'RECOVERING' && <div className="agent-composer-dock">
        {liveSessionChanges.length > 0 && <button type="button" className="agent-live-file-changes" aria-label={`审查本次会话已更改的 ${liveSessionChanges.length} 个文件，新增 ${liveSessionChangeAdditions} 行，删除 ${liveSessionChangeDeletions} 行`} onClick={() => openChangesReview(liveSessionChanges)}><FileText size={14}/><span>{`${liveSessionChanges.length} 个文件已更改`}</span><ins>{`+${liveSessionChangeAdditions}`}</ins><del>{`-${liveSessionChangeDeletions}`}</del></button>}
