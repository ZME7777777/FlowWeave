...
function AgentSessionWorkbenchContent({ onNavigate, onReturnToSource, onHostStateChanged, autoOpenDraft = false, hideDraftTitle = false }: Omit<AgentSessionWorkbenchProps, 'gateway' | 'host'>) {
  ...
  // makes the first visual state deterministic even when the Runtime answers
  // an older-page request from a warm connection.
  const historyPrefetchDelayMs = inputReadinessQuery.isError ? 5_000 : 100;
