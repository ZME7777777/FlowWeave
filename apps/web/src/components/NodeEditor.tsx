...
import type { CapabilityAsset, CapabilityRef, IOField, NodeAsset, NodeAssetWrite, SkillCollection } from '../types';
...
export function NodeEditor({ node, onSave, onClose }: Props) {
  ...
  const addSkillCollection = (collection: SkillCollection) => {
    ...
    if (unavailable.length) {
      setCollectionNotice(`组合“${collection.name}”包含依赖未就绪的 Skill：${unavailable.map(item => item.capability_key).join('、')}。`);
      return;
    }
    setForm(old => {
      const memberKeys = new Set(collection.members.map(item => item.capability_key));
      const currentByKey = new Map(old.capabilities.filter(item => item.capability_type === 'SKILL').map(item => [item.capability_key, item]));
      const added = collection.members.filter(item => !currentByKey.has(item.capability_key)).length;
      const replaced = collection.members.filter(item => {
        const current = currentByKey.get(item.capability_key);
      ...
      setCollectionNotice(`已展开“${collection.name}”：新增 ${added} 项、替换 ${replaced} 项、已有 ${unchanged} 项。节点保存的仍是 ${expanded.length} 个真实 Skill 引用。`);
      return {
        ...old,
        capabilities: [
          ...old.capabilities.filter(item => item.capability_type !== 'SKILL' || !memberKeys.has(item.capability_key)),
          ...expanded,
        ],
      };
    });
  ...
  return <div className="modal-backdrop"><form className="modal editor asset-editor" onSubmit={submit}>
    ...
    {tab === 2 && <section className="form-pane">
      <div className="capability-help"><b>从公共能力仓库选择当前节点可用的能力</b><span>Skill 通过 ZIP 导入；MCP 与 Hook 使用表单或 JSON 创建。节点只保存不可变能力引用。</span><span>命令型 MCP 与 Command Hook 使用节点所选终端环境中已安装并发布的 CLI；Hook 会在生命周期事件中自动执行。</span></div>
      <section className="node-skill-collections"><header><div><Layers3 size={16}/><span><b>按 Skill 组合批量添加</b><small>组合仅用于选择；点击后立即展开为固定版本的真实 Skill。</small></span></div><em>{skillCollections.length} 个组合</em></header>{collectionsLoading ? <div className="empty compact">加载 Skill 组合…</div> : skillCollections.length ? <div>{skillCollections.map(collection => { const unavailable = collection.members.some(item => item.dependency_build_state !== 'NOT_REQUIRED' && item.dependency_build_state !== 'READY'); return <article key={collection.id}><div><span>{collection.category || '未分类'}</span><b>{collection.name}</b><small>{collection.description || collection.members.map(item => item.capability_key).join('、')}</small><p>{collection.members.map(item => <code key={item.id}>{item.capability_key} · rev {item.revision_number}</code>)}</p></div><button type="button" className="secondary" disabled={unavailable} title={unavailable ? '组合中有依赖未就绪的 Skill' : `添加 ${collection.members.length} 个真实 Skill`} onClick={() => addSkillCollection(collection)}>添加 {collection.members.length} 项</button></article>; })}</div> : <p className="node-skill-collections-empty">尚未创建 Skill 组合；仍可在下方逐项选择。</p>}{collectionNotice && <div className="node-collection-notice" role="status">{collectionNotice}</div>}</section>
      {capabilitiesLoading ? <div className="empty compact">加载能力仓库…</div> : selectableCapabilities.length ? <div className="capability-picker">{(['SKILL', 'MCP', 'HOOK'] as const).map(type => { const items = selectableCapabilities.filter(item => item.capability_type === type); return <section key={type}><header><b>{type === 'SKILL' ? 'Skills' : type}</b><span>{items.filter(item => item.is_latest).length} 项能力</span></header>{items.length ? items.map(item => { const ready = item.dependency_build_state === 'NOT_REQUIRED' || item.dependency_build_state === 'READY'; const selected = selectedCapabilityIds.has(item.id); return <label key={item.id} className={`${selected ? 'selected' : ''} ${ready ? '' : 'unavailable'}`} title={ready ? '' : item.dependency_build_state === 'FAILED' ? item.dependency_build_error || '依赖构建失败' : '依赖正在隔离构建中'}><input type="checkbox" aria-label={`选择能力 ${item.capability_key}`} checked={selected} disabled={!ready && !selected} onChange={() => toggleCapability(item)}/><span><b data-testid="capability-key">{item.capability_key}</b><small>{item.description || item.filename}</small></span><em>{!ready ? item.dependency_build_state === 'FAILED' ? '依赖失败' : '依赖构建中' : `${item.reference_count} 个节点引用`}</em></label>; }) : <p>能力仓库中暂无 {type}</p>}</section>; })}</div> : <div className="capability-pool-empty"><b>能力仓库尚为空</b><span>请关闭编辑器，前往顶部“能力仓库”上传 Skill ZIP，或配置 MCP / Hook。</span></div>}
    </section>}
