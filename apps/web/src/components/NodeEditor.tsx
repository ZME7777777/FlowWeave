...
export function NodeEditor({ node, onSave, onClose }: Props) {
  ...
  return <div className="modal-backdrop"><form className="modal editor asset-editor" onSubmit={submit}>
    ...
    {tab === 0 && <section className="form-grid form-pane">
      ...
      <label>图标<input aria-label="图标" value={form.icon_value} onChange={event => setForm({ ...form, icon_value: event.target.value })}/></label>
    </section>}
    ...
    {tab === 2 && <section className="form-pane io-editor"><div className="io-intro"><b>定义输入输出</b><span>这里只定义数据槽位，不填写运行时的具体飞书文档。模板为可选项：有模板时仅参考其格式和结构；留空时不参考模板，按说明和任务要求处理。</span></div>{(['inputs', 'outputs'] as const).map(direction => {
      const input = direction === 'inputs';
      return <section className={`io-section ${input ? 'input' : 'output'}`} key={direction}><header><div><span>{input ? 'INPUT' : 'OUTPUT'}</span><h3>{input ? '输入定义' : '输出定义'}</h3><p>{input ? '运行时由人工指定实际输入，或自动接收上游输出。' : '运行时在本次运行目录创建文档；有模板则复制模板，无模板则创建空白文档。'}</p></div><button type="button" className="secondary" onClick={() => setForm(old => ({ ...old, [direction]: [...old[direction], emptyField(input ? 'input' : 'output', old[direction].length)] }))}><Plus size={13}/>添加{input ? '输入' : '输出'}</button></header>{form[direction].length > 0 && <div className="io-column-head lark-doc-head" aria-hidden="true"><span>字段标识</span><span>展示名称</span><span>模板（可选）</span><span>说明</span><span>操作</span></div>}{form[direction].map((field, index) => <div className="io-row lark-doc-row" key={index}><input required pattern="[A-Za-z][A-Za-z0-9_]{0,99}" aria-label={`${direction} key ${index}`} value={field.field_key} placeholder={input ? 'requirement' : 'result'} title="以字母开头，只能包含字母、数字和下划线" onChange={event => updateField(direction, index, { field_key: event.target.value })}/><input aria-label={`${direction} name ${index}`} value={field.display_name} onChange={event => updateField(direction, index, { display_name: event.target.value })}/><input type="url" pattern="https://.*/docx/[^/]+.*" aria-label={`${direction} template ${index}`} value={field.template_url} placeholder="可选：飞书 Docx 模板 URL" onChange={event => updateField(direction, index, { template_url: event.target.value })}/><input aria-label={`${direction} description ${index}`} value={field.description} placeholder={input ? '说明 Agent 如何使用输入' : '说明输出内容与验收要求'} onChange={event => updateField(direction, index, { description: event.target.value })}/><button type="button" className="ghost" aria-label={`移除 ${direction} 字段 ${field.field_key}`} onClick={() => removeField(direction, index)}><Trash2 size={14}/></button></div>)}{form[direction].length === 0 && <div className="io-empty">暂未定义{input ? '输入' : '输出'}。</div>}</section>;
    })}</section>}
    {error && <p className="error">{error}</p>}
    <footer><button type="button" className="ghost" onClick={onClose}>取消</button>{tab > 0 && <button type="button" className="secondary" onClick={() => setTab(tab - 1)}>上一步</button>}{tab < tabs.length - 1 ? <button type="button" className="primary" onClick={() => setTab(tab + 1)}>下一步</button> : <button type="submit" className="primary" disabled={saving}>{saving ? '保存中…' : '保存节点资产'}</button>}</footer>
  </form></div>;
}
