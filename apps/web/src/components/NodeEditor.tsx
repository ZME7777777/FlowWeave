...
export function NodeEditor({ node, onSave, onClose }: Props) {
  ...
  return <div className="modal-backdrop"><form className="modal editor asset-editor" onSubmit={submit}>
    ...
    {tab === 2 && <section className="form-pane io-editor"><div className="io-intro"><b>定义输入输出</b><span>这里只定义运行时表单的数据槽位和类型。URL 接受任意安全 HTTP(S) 链接，FILE 接受图片或其他文件附件。</span></div>{(['inputs', 'outputs'] as const).map(direction => {
      const input = direction === 'inputs';
      return <section className={`io-section ${input ? 'input' : 'output'}`} key={direction}><header><div><span>{input ? 'INPUT' : 'OUTPUT'}</span><h3>{input ? '输入定义' : '输出定义'}</h3><p>{input ? '运行时严格按这里的字段和类型生成表单。' : 'Agent 按类型返回 URL，或返回本次工作区内创建的文件。'}</p></div><button type="button" className="secondary" onClick={() => setForm(old => ({ ...old, [direction]: [...old[direction], emptyField(input ? 'input' : 'output', old[direction].length)] }))}><Plus size={13}/>添加{input ? '输入' : '输出'}</button></header>{form[direction].length > 0 && <div className="io-column-head lark-doc-head" aria-hidden="true"><span>字段标识</span><span>展示名称</span><span>类型</span><span>说明</span><span>操作</span></div>}{form[direction].map((field, index) => <div className="io-row lark-doc-row" key={index}><input required pattern="[A-Za-z][A-Za-z0-9_]{0,99}" aria-label={`${direction} key ${index}`} value={field.field_key} placeholder={input ? 'requirement' : 'result'} title="以字母开头，只能包含字母、数字和下划线" onChange={event => updateField(direction, index, { field_key: event.target.value })}/><input aria-label={`${direction} name ${index}`} value={field.display_name} onChange={event => updateField(direction, index, { display_name: event.target.value })}/><select aria-label={`${direction} type ${index}`} value={field.data_type} onChange={event => updateField(direction, index, { data_type: event.target.value as IOField['data_type'] })}><option value="URL">URL</option><option value="FILE">文件附件</option></select><input aria-label={`${direction} description ${index}`} value={field.description} placeholder={input ? '说明 Agent 如何使用输入' : '说明输出内容与验收要求'} onChange={event => updateField(direction, index, { description: event.target.value })}/><button type="button" className="ghost" aria-label={`移除 ${direction} 字段 ${field.field_key}`} onClick={() => removeField(direction, index)}><Trash2 size={14}/></button></div>)}{form[direction].length === 0 && <div className="io-empty">暂未定义{input ? '输入' : '输出'}。</div>}</section>;
    })}</section>}
