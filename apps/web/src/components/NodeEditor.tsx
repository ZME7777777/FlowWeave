...
import type { CapabilityAsset, CapabilityRef, IOField, NodeAsset, NodeAssetWrite } from '../types';
...
export function NodeEditor({ node, onSave, onClose }: Props) {
  ...
  const toggleCapability = (asset: CapabilityAsset) => setForm(old => {
    ...
    const next: CapabilityRef = {
      ...
      capability_type: asset.capability_type,
    ...
    return {
      ...old,
      capabilities: [
        ...old.capabilities.filter(item => !(item.capability_type === asset.capability_type && item.capability_key === asset.capability_key)),
        next,
      ],
    };
  });
