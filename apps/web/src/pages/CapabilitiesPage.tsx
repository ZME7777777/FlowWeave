...
function capabilityModuleDescription(type: CapabilityAssetType): string {
  ...
  if (type === 'TOOL_POLICY') return '定义 Agent 可用工具及其确认与访问边界。';
...
export function CapabilitiesPage() {
  ...
  return <section className="page capabilities-page">
    ...
    <section className="capability-module-head">
      <div><span className="eyebrow">{typeLabel(type).toUpperCase()}</span><h2>{typeLabel(type)}</h2><p>{capabilityModuleDescription(type)}</p></div>
      <div className="capability-module-actions">
        <label className="capability-repository-search"><span className="sr-only">搜索当前模块</span><input aria-label="搜索当前能力模块" value={repositoryQuery} onChange={event => setRepositoryQuery(event.target.value)} placeholder={`搜索 ${typeLabel(type)} 名称、说明或文件…`}/>{repositoryQuery && <button type="button" aria-label="清除搜索" onClick={() => setRepositoryQuery('')}>×</button>}</label>
        ...
        {type === 'MCP' && <button className="primary" disabled={importingSkill || busy} onClick={() => void openMcpEditor()}><Braces size={15}/>新建 MCP</button>}
    ...
    {isLoading ? <div className="empty">加载 {typeLabel(type)}…</div> : visible.length ? <div className="capability-card-grid">{visible.map(group => <CapabilityCard key={group.id} group={group} selected={selectedLineageIds.has(group.id)} onToggle={() => toggleLineage(group.id)} onEdit={() => group.latest.capability_type === 'MCP' ? void openMcpEditor(group.latest) : void openEditor(group.latest)} onViewPolicy={() => setViewingPolicy(group)} onProfileHistory={() => setProfileHistory(group)} onDelete={() => void remove([group])}/>)}</div> : <div className="empty"><FileArchive size={30}/><b>{repositoryQuery ? `没有匹配的 ${typeLabel(type)}` : `暂无 ${typeLabel(type)}`}</b><span>{repositoryQuery ? '调整搜索条件，或清除搜索查看全部能力。' : '使用本模块右上角的功能创建或导入。'}</span></div>}</div>
