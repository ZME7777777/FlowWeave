...
function MessageBubble({ message, onRetry, final }: { message: AgentMessage; onRetry: (id: string) => void; final: boolean }) {
  ...
  return <article className={`chat-message ${meta.className} ${final ? 'final-answer' : ''}`}>
    ...
    <div className="message-body"><header>{message.source !== 'HUMAN' && <b>{final ? '最终答复' : meta.label}</b>}<span>{new Date(message.created_at).toLocaleTimeString()}</span></header>{message.source === 'AGENT' ? <MarkdownMessage text={messageText(message)}/> : <p>{messageText(message)}</p>}
...
export function AgentChatPage() {
  ...
  const sendMutation = useMutation({
    mutationFn: async ({ conversation, content }: { conversation: AgentConversation; content: string }): Promise<AgentMessage | undefined> => {
      if (conversation.kind === 'AUTO' && attempt!.state === 'WAITING_HUMAN') {
        await api.humanInput(attempt!.id, content, attempt!.state_version);
        return undefined;
      }
      return api.sendConversationMessage(conversation.id, content, conversation.state_version);
    },
