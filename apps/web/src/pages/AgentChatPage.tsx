import { AlertTriangle, ArrowLeft, Bot, CheckCircle2, ChevronRight, Clock3, FileText, Plus, RefreshCw, Send, Sparkles, Workflow, XCircle } from 'lucide-react';
...
function MarkdownMessage({ text }: { text: string }) {
  return <div className="message-markdown"><ReactMarkdown>{text}</ReactMarkdown></div>;
}
...
function ActivityMessage({ message }: { message: AgentMessage }) {
  ...
  return <details className={`activity-message tool-activity ${completed ? 'completed' : 'running'}`}><summary><span>{completed ? <CheckCircle2 size={15}/> : <Sparkles size={15}/>}<b>{completed ? '工具执行结果' : '调用工具'} · {name}</b><small>{content.split('\n')[0] || (Object.keys(details).length ? `${Object.keys(details).length} 项参数` : '')}</small></span><ChevronRight size={14}/></summary><div className="activity-detail">{content && <MarkdownMessage text={content}/>} {!!Object.keys(details).length && <pre>{JSON.stringify(details, null, 2)}</pre>}</div></details>;
}

function MessageBubble({ message, onRetry, final }: { message: AgentMessage; onRetry: (id: string) => void; final: boolean }) {
...
function ConversationRail({ conversations, selectedId, attempt, onSelect, onCreate, creating }: {
  ...
  return <aside className="conversation-rail"><header><div><span className="eyebrow">CONVERSATIONS</span><h2>本轮协作会话</h2></div><button className="secondary icon-button" aria-label="新建会话" title={canCreate ? '新建会话' : '本轮开始执行后可新建'} disabled={!canCreate || creating} onClick={onCreate}><Plus size={16}/></button></header>
    <div className="conversation-list">{conversations.map(item => <button key={item.id} className={item.id === selectedId ? 'active' : ''} onClick={() => onSelect(item.id)}><span className={`conversation-kind ${item.kind === 'AUTO' ? 'auto' : 'human'}`}>{item.kind === 'AUTO' ? <Workflow size={13}/> : <HumanControlIcon size={14}/>}</span><span><b>{conversationTitle(item.title)}</b><small>{STATE_LABELS[item.state] ?? item.state} · {item.message_count} 条消息</small></span>{item.state === 'WAITING_HUMAN' && <i/>}</button>)}</div>
...
function ContextPanel({ attempt, nodeName, node, runName }: { attempt: NodeAttempt; nodeName: string; node?: { asset: { executor: { model_name?: string | null } | null; default_skill_ref?: string | null; workspace_ref?: string; capabilities: Array<{ capability_type: string; capability_key: string }> } }; runName: string }) {
  return <aside className="conversation-context"><header><span className="eyebrow">EXECUTION CONTEXT</span><h2>执行上下文</h2></header><section><h3>当前事实</h3><dl><dt>流程运行</dt><dd>{runName}</dd><dt>节点</dt><dd>{nodeName}</dd><dt>执行轮次</dt><dd>第 {attempt.attempt_no} 轮</dd><dt>状态 / 版本</dt><dd>{attempt.state} · v{attempt.state_version}</dd></dl></section><section><h3>模型与能力</h3><dl><dt>模型</dt><dd>{node?.asset.executor?.model_name || '服务默认'}</dd><dt>默认 Skill</dt><dd>{node?.asset.default_skill_ref || '—'}</dd><dt>宿主工作区</dt><dd>{node?.asset.workspace_ref ? `var/workspaces/${node.asset.workspace_ref}` : '—'}</dd></dl>{node?.asset.capabilities.map(item => <span className="context-capability" key={`${item.capability_type}-${item.capability_key}`}>{item.capability_type} · {item.capability_key}</span>)}</section><section><h3>输入与本轮产物</h3>{attempt.input_bindings.map(item => <span className="context-reference" key={item.id}><FileText size={13}/>{item.input_field_key}<small>{item.binding_source}</small></span>)}{attempt.artifacts.map(item => <span className="context-reference" key={item.id}><FileText size={13}/>{item.field_key} · v{item.version_no}<small>{item.artifact_type}</small></span>)}</section><footer>这些会话只属于当前执行轮次；退回后产生的新轮次会建立独立上下文并保留这里的历史。</footer></aside>;
}
...
export function AgentChatPage() {
  ...
  const sendMutation = useMutation({
    mutationFn: async ({ conversation, content, capabilityRefs }: { conversation: AgentConversation; content: string; capabilityRefs: Array<{ capability_type: 'SKILL' | 'MCP'; capability_key: string }> }): Promise<AgentMessage | undefined> => {
      if (conversation.kind === 'AUTO' && attempt!.state === 'WAITING_HUMAN') {
        await api.humanInput(attempt!.id, content, attempt!.state_version);
        return undefined;
      }
      return api.sendConversationMessage(conversation.id, content, conversation.state_version, capabilityRefs);
    },
  ...
  const timelineMessages = (messagesQuery.data ?? []).filter(message => !queuedMessages.some(queued => queued.id === message.id));
  const lastAgentTextId = [...timelineMessages].reverse().find(message => message.source === 'AGENT' && message.message_type === 'TEXT')?.id;
  const replyingToAttempt = selected?.kind === 'AUTO' && attempt.state === 'WAITING_HUMAN';
  const send = () => { const content = draft.trim(); if (selected && content && !readOnly) { const capabilityRefs = callableCapabilities.filter(item => selectedCapabilityKeys.includes(`${item.capability_type}:${item.capability_key}`)).map(item => ({ capability_type: item.capability_type as 'SKILL' | 'MCP', capability_key: item.capability_key })); sendMutation.mutate({ conversation: selected, content, capabilityRefs }); } };
  ...
  const toggleCapability = (type: string, key: string) => { if (!selected) return; const marker = capabilityMarker(type, key); const matcher = new RegExp(`(^|\\s)${escapeRegExp(marker)}(?=\\s|$)`, 'g'); if (matcher.test(draft)) { setDrafts(old => ({ ...old, [selected.id]: draft.replace(matcher, '$1').replace(/ {2,}/g, ' ').trimStart() })); } else { selectCapability(type, key); } };
  ...
  return <section className="agent-chat-page"><div className="conversation-topbar"><button onClick={returnToWorkbench}><ArrowLeft size={15}/>返回运行详情</button><div><span>{runQuery.data.name}</span><b>{nodeName} · 第 {attempt.attempt_no} 轮</b></div><span className={`conversation-status ${selected?.state.toLowerCase() ?? ''}`}>{selected ? STATE_LABELS[selected.state] : '等待会话'}</span></div>
    <div className="agent-chat-layout"><ConversationRail conversations={conversations} selectedId={selected?.id} attempt={attempt} onSelect={selectConversation} onCreate={() => createMutation.mutate()} creating={createMutation.isPending}/><main className="conversation-workspace"><header><div><span className="eyebrow">AGENT COLLABORATION</span><h1>{selected ? conversationTitle(selected.title) : 'Agent 协作空间'}</h1></div>{selected && <span>{selected.kind === 'AUTO' ? 'AUTO 默认会话' : `人工会话 #${selected.conversation_no}`}</span>}</header>
      <div className="message-timeline" aria-live="polite">{timelineMessages.map(message => <MessageBubble key={message.id} message={message} onRetry={id => retryMutation.mutate(id)} final={message.content.presentation === 'final' || (message.id === lastAgentTextId && (selected?.state === 'IDLE' || selected?.state === 'READ_ONLY'))}/>)}{selected && !messagesQuery.data?.length && <div className="conversation-empty"><Bot size={26}/><b>当前轮次上下文已挂载</b><span>发送第一条消息开始协作。</span></div>}{!selected && <div className="conversation-empty"><Workflow size={26}/><b>尚无可用会话</b><span>本轮开始执行后将自动创建默认会话。</span></div>}</div>
      {!!queuedMessages.length && <section className="queued-message-tray" aria-label="排队消息"><header><span><b>{queuedMessages.length} 条消息等待发送</b><small>默认在当前回合结束后发送，也可以立即引导正在运行的 Agent。</small></span></header>{queuedMessages.map(message => <article key={message.id}><p>{messageText(message)}</p><button className="secondary" disabled={steerMutation.isPending} onClick={() => steerMutation.mutate(message.id)}><Send size={13}/>{steerMutation.isPending && steerMutation.variables === message.id ? '正在引导…' : '立即引导'}</button></article>)}</section>}
      {selected && (readOnly ? <div className="read-only-composer"><b>历史会话，只读</b><span>验收、退回和重跑请返回运行详情操作。</span><button className="secondary" onClick={returnToWorkbench}>返回运行详情</button></div> : <div className="message-composer">{!!callableCapabilities.length && <section className="composer-capabilities"><header><b>引用运行能力</b><small>输入 <code>$</code>，统一引用当前节点的 Skill 或 MCP。</small></header><div>{callableCapabilities.map(item => { const value = `${item.capability_type}:${item.capability_key}`; const active = selectedCapabilityKeys.includes(value); return <button type="button" aria-pressed={active} className={active ? 'active' : ''} key={value} onClick={() => toggleCapability(item.capability_type, item.capability_key)}>{capabilityMarker(item.capability_type, item.capability_key)}</button>; })}</div></section>}{!!commandSuggestions.length && <div className="capability-command-menu" role="listbox" aria-label="能力引用候选">{commandSuggestions.map((item, index) => <button type="button" role="option" aria-selected={index === suggestionIndex} className={index === suggestionIndex ? 'active' : ''} key={`${item.capability_type}-${item.capability_key}`} onMouseDown={event => event.preventDefault()} onClick={() => selectCapability(item.capability_type, item.capability_key)}><b>{capabilityMarker(item.capability_type, item.capability_key)}</b><span>{item.capability_type === 'SKILL' ? 'Skill · 调用并遵循能力说明' : 'MCP · 使用 Server 暴露的工具'}</span></button>)}</div>}<textarea aria-label="发送给 Agent 的消息" value={draft} maxLength={20000} placeholder={selected.state === 'CREATING' ? '会话创建中…' : replyingToAttempt ? '回复 Agent 并继续执行。输入 $ 引用运行能力。' : '补充要求；输入 $ 引用 Skill 或 MCP。Enter 发送。'} disabled={selected.state === 'CREATING' || selected.state === 'FAILED'} onChange={event => { setDrafts(old => ({ ...old, [selected.id]: event.target.value })); setSuggestionIndex(0); }} onKeyDown={keyDown}/><div className="composer-actions"><span>{selected.state === 'GENERATING' ? '将在当前回合结束后发送' : `${draft.length} / 20,000`}</span><button className="primary" disabled={!draft.trim() || selected.state === 'CREATING' || selected.state === 'FAILED' || sendMutation.isPending} onClick={send}><Send size={15}/>{replyingToAttempt ? '提交并继续' : selected.state === 'GENERATING' ? '加入队列' : '发送'}</button></div></div>)}
      {(createMutation.error || sendMutation.error || retryMutation.error || steerMutation.error) && <p className="conversation-error"><AlertTriangle size={14}/>{(createMutation.error || sendMutation.error || retryMutation.error || steerMutation.error)?.message}</p>}
