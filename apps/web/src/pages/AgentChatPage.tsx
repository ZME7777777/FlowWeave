...
function AgentActivityStatus({ conversation, messages, retrying, onRetry }: { conversation: AgentConversation; messages: AgentMessage[]; retrying: boolean; onRetry: (id: string) => void }) {
  const activeMessages = messages.filter(message => message.delivery_state !== 'CANCELLED');
  const frozenRuntimeFailure = activeMessages
    .filter(message => message.message_type === 'ERROR')
    .some(message => {
      const detail = messageText(message);
      return detail.includes('SNAPSHOT_MANIFEST_INVALID') || detail.includes('RUNTIME_CONTRACT_INCOMPATIBLE');
    });
  ...
  if (conversation.state === 'CREATING') {
    ...
    tone = 'failed';
    title = 'Agent 连接中断';
    detail = frozenRuntimeFailure
      ? '本轮冻结的 Snapshot 或 Runtime 契约不兼容，无法在当前执行内重连。请返回运行详情并创建新的 Run。'
      : '消息仍然保留。可继续输入新消息自动重建，或重试失败消息。';
...
function ConversationRail({ conversations, selectedId, attempt, onSelect, onCreate, onDelete, creating, deleting, creationBlocked }: {
  conversations: AgentConversation[]; selectedId?: string; attempt: NodeAttempt;
  onSelect: (id: string) => void; onCreate: () => void; onDelete: (item: AgentConversation) => void; creating: boolean; deleting?: string; creationBlocked?: boolean;
}) {
  const canCreate = STARTED_STATES.has(attempt.state) && !creationBlocked;
  return <aside className="conversation-rail"><header><div><span className="eyebrow">CONVERSATIONS</span><h2>本轮协作会话</h2></div><button className="secondary icon-button" aria-label="新建会话" title={creationBlocked ? '当前执行的冻结配置不兼容，请新建运行' : canCreate ? '新建会话' : '本轮开始执行后可新建'} disabled={!canCreate || creating} onClick={onCreate}><Plus size={16}/></button></header>
  ...
  return <div className="runtime-selection-menu" ref={rootRef}>
    ...
    {open && <div className="runtime-selection-popover">
      <div className="runtime-selection-main" role="menu">
        <button type="button" className={submenu === 'model' ? 'active' : ''} role="menuitem" onClick={() => setSubmenu('model')}><span><b>模型</b><small>{modelName || '未配置'}</small></span><ChevronRight size={14}/></button>
        <button type="button" className={submenu === 'effort' ? 'active' : ''} role="menuitem" disabled={!efforts.length} onClick={() => efforts.length && setSubmenu('effort')}><span><b>推理强度</b><small>{effortLabel}</small></span><ChevronRight size={14}/></button>
      </div>
...
export function AgentChatPage() {
  ...
  const frozenRuntimeFailure = [attempt.error_code, attempt.error_detail, ...timelineMessages.filter(message => message.message_type === 'ERROR').map(messageText)].some(value => value?.includes('SNAPSHOT_MANIFEST_INVALID') || value?.includes('RUNTIME_CONTRACT_INCOMPATIBLE'));
  ...
  return <section className="agent-chat-page"><div className="conversation-topbar"><button onClick={returnToWorkbench}><ArrowLeft size={15}/>返回运行详情</button><div><span>{runQuery.data.name}</span><b>{nodeName} · 第 {attempt.attempt_no} 轮</b></div><span className={`conversation-status ${selected?.state.toLowerCase() ?? ''}`}>{selected ? STATE_LABELS[selected.state] : '等待会话'}</span></div>
    <div className={`agent-chat-layout ${runtimeSidebarCollapsed ? 'runtime-sidebar-collapsed' : ''}`}><ConversationRail conversations={conversations} selectedId={selected?.id} attempt={attempt} onSelect={selectConversation} onCreate={() => createMutation.mutate()} onDelete={item => { void dialog.confirm({ title: `删除“${conversationTitle(item.title)}”？`, message: '该会话的消息和临时附件都会被永久删除。', confirmLabel: '确认删除', tone: 'danger' }).then(confirmed => { if (confirmed) deleteMutation.mutate(item.id); }); }} creating={createMutation.isPending} deleting={deleteMutation.isPending ? deleteMutation.variables : undefined} creationBlocked={frozenRuntimeFailure}/><main className="conversation-workspace"><header><div><span className="eyebrow">AGENT COLLABORATION</span><h1>{selected ? conversationTitle(selected.title) : 'Agent 协作空间'}</h1></div>{selected && <span>{selected.kind === 'AUTO' ? 'AUTO 默认会话' : `人工会话 #${selected.conversation_no}`}</span>}</header>
      ...
      {selected && (frozenRuntimeFailure ? (
        <div className="read-only-composer"><b>当前执行无法重新连接</b><span>本轮冻结的 Snapshot 或 OpenHands Runtime 契约已不兼容；新建人工会话仍会复用同一配置。请返回运行详情，使用兼容的环境版本创建新的 Run。</span><button className="secondary" onClick={returnToWorkbench}>返回运行详情</button></div>
      ) : readOnly ? (
        ...
        <div className="message-composer">
          ...
          <div className="composer-actions">
            ...
            <div className="composer-runtime-actions">
              ...
              {selected.state === 'GENERATING' ? <button type="button" className="agent-stop-button" aria-label="停止当前 Agent" title="停止当前 Agent" disabled={stopMutation.isPending} onClick={stop}><Square size={11} fill="currentColor"/></button> : <button className="primary" title={selected.state === 'FAILED' && !draft.trim() && !attachments.length ? '先输入一条消息，发送时系统会自动重建 Agent 会话' : undefined} disabled={attempt.state === 'WAITING_CONFIRMATION' || (!draft.trim() && !attachments.length) || selected.state === 'CREATING' || selected.state === 'STOPPING' || sendMutation.isPending} onClick={send}><Send size={15}/>{replyingToAttempt ? '提交并继续' : selected.state === 'FAILED' ? (!draft.trim() && !attachments.length ? '输入消息后重连' : '重新连接并发送') : '发送'}</button>}
            </div>
          </div>
        </div>
      ))}
