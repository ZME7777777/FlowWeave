...
import { AlertTriangle, ArrowLeft, CheckCircle2, Download, Eye, FileText, Play, RefreshCw, Send, StopCircle, Wrench, X } from 'lucide-react';
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query';
import { useCallback, useEffect, useMemo, useState } from 'react';
import { api, artifactContentUrl, subscribeToRun } from '../api/client';
import { useWorkbenchStore } from '../store/workbench';
import type { ArtifactVersion, FlowRun, GateEvaluation, NodeAttempt, NodeRun, RunEvent, SnapshotFlowNode } from '../types';
...
function RunRail({ run, selected, onSelect }: { run: FlowRun; selected?: string; onSelect: (id: string) => void }) {
  return <aside className="run-rail"><span className="eyebrow">RUN HIERARCHY</span><h2>{run.name}</h2><span data-testid="flow-run-state" className={`run-state ${run.state.toLowerCase()}`}>{run.state}</span><div className="run-progress"><span>{run.progress.accepted} 已接受</span><span>{run.progress.terminal} 终态</span><span>{run.progress.active} 激活</span></div><div className="timeline">{run.node_runs.map(item => <button key={item.id} className={selected === item.id ? 'active' : ''} onClick={() => onSelect(item.id)}><i className={item.state.toLowerCase()}/><span><b>{item.flow_node_snapshot_key}</b><small>Node Run #{item.sequence_no} · {attemptState(item)}</small></span></button>)}</div></aside>;
}
...
function SnapshotNodeDialog({ node, visits, onClose, onSelect }: { node: SnapshotFlowNode; visits: NodeRun[]; onClose: () => void; onSelect: (id: string) => void }) {
  ...
  return <div className="modal-backdrop"><section className="modal run-node-dialog" role="dialog" aria-label={`运行节点详情 ${node.alias || node.asset.name}`}><header><div><span className="eyebrow">SNAPSHOT NODE</span><h2>{node.alias || node.asset.name}</h2><p>{node.asset.name} · {node.instance_key}</p></div><button className="ghost" aria-label="关闭运行节点详情" onClick={onClose}><X size={16}/></button></header>
    ...
    <section><h3>Node Run / Attempt 历史</h3>{visits.length ? visits.map(visit => <button className="run-node-visit" key={visit.id} onClick={() => { onSelect(visit.id); onClose(); }}><span><b>Node Run #{visit.sequence_no}</b><small>{new Date(visit.activated_at).toLocaleString()} · {visit.created_from}</small></span><strong>{attemptState(visit)}</strong><small>{visit.attempts.length} Attempts</small></button>) : <p>此快照节点尚未激活，可从右侧人工控制台选择“从任意节点重新运行”。</p>}</section>
...
function AttemptPanel({ run, nodeRun, attempt, refresh, navigate }: { run: FlowRun; nodeRun: NodeRun; attempt: NodeAttempt; refresh: () => void; navigate: (result: unknown, kind: string) => void }) {
  ...
  }, onSuccess: (result, variables) => { setText(''); setArtifact(''); navigate(result, variables.kind); refresh(); } });
  ...
  return <aside className="action-panel"><header><div><b>人工控制台</b><small>Attempt {attempt.attempt_no} · Snapshot {attemptSnapshot?.version}</small></div></header><div className="action-content"><div className="state-banner"><span>当前状态</span><b data-testid="attempt-state">{attempt.state}</b><small>state_version {attempt.state_version}</small></div>
    ...
    <button className="ghost full" onClick={() => act('complete')}>人工结束流程</button>{run.state !== 'COMPLETED' && <button className="danger full" onClick={() => act('cancel')}><StopCircle size={15}/>取消流程</button>}{mutation.error && <p className="error"><AlertTriangle size={14}/>{mutation.error.message}</p>}</div></aside>;
}
...
export function WorkbenchPage() {
  ...
  return <section className="workbench-page"><RunRail run={run} selected={nodeRun?.id} onSelect={id => { const node = run.node_runs.find(item => item.id === id); selectExecution(id, node?.attempts.at(-1)?.id); }}/><main className="run-main"><button className="back" onClick={() => setView('runs')}><ArrowLeft size={14}/>返回运行列表</button><header className="run-title"><div><span className="eyebrow">RUN #{run.run_no}</span><h1>{run.name}</h1><p>Active Snapshot v{run.active_snapshot_version} · {run.progress.accepted}/{run.progress.active} 节点已接受</p></div><span className={`run-state ${run.state.toLowerCase()}`}>{run.state}</span></header><SnapshotSync run={run} currentVersion={flow.data?.row_version} onSynced={updated => navigate(updated, 'sync')}/><SnapshotGraph run={run} onSelect={selectNodeRun}/>
    {nodeRun && attempt ? <><section className="attempt-tabs"><header><div><h3>{nodeRun.flow_node_snapshot_key}</h3><small>Node Run #{nodeRun.sequence_no} · {nodeRun.created_from}</small></div><div>{nodeRun.attempts.map(item => <button className={item.id === attempt.id ? 'active' : ''} key={item.id} onClick={() => selectAttempt(item.id)}>Attempt {item.attempt_no}</button>)}</div></header><div className="attempt-grid"><section><h4>输入绑定</h4>{attempt.input_bindings.map(item => <div className="binding-row" key={item.id}><b>{item.input_field_key}</b><span>{run.artifacts.find(artifactItem => artifactItem.id === item.artifact_version_id)?.field_key} v{run.artifacts.find(artifactItem => artifactItem.id === item.artifact_version_id)?.version_no}</span><small>{item.binding_source}</small></div>)}</section><section><h4>门禁结果</h4><GateList evaluations={attempt.gate_evaluations}/></section></div></section><ArtifactList artifacts={attempt.artifacts}/></> : <div className="empty compact">此运行尚未激活节点。</div>}<EventTimeline events={events.data ?? []}/></main>{nodeRun && attempt && <AttemptPanel run={run} nodeRun={nodeRun} attempt={attempt} refresh={refresh} navigate={navigate}/>}</section>;
}
