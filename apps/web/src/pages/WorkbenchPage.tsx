...
import type { AgentPreset, ArtifactVersion, AttemptState, AutomaticNodePlan, CapabilityAsset, CapabilityCollection, FlowRun, FlowRunAutomaticRecord, FlowRunAutomaticRecordSummary, GateAgentPreset, GateEvaluation, GatePolicy, NodeAttempt, NodeRun, OpenHandsConversationEvent, SnapshotFlowNode } from '../types';
...
function GateList({ evaluations, policies = [], onViewDetails }: { evaluations: GateEvaluation[]; policies?: GatePolicy[]; onViewDetails?: (evaluation: GateEvaluation) => void }) {
  ...
  const status = (evaluation: GateEvaluation) => evaluation.decision === 'ERROR' ? '执行异常' : evaluation.decision;
  ...
  return <div className="gate-results gate-overview-list">{groups.map(group => {
    ...
    return <button type="button" className="gate-overview-row" key={group.key} onClick={() => setSelected(group)}><span><b>{group.title}</b><small>{latest ? `${new Date(latest.created_at).toLocaleString()} · ${summary(latest.result.summary, latest.decision === 'ERROR' ? '门禁执行未完成' : '未记录审查摘要')}` : '尚无审查记录'}</small></span><strong className={latest?.decision === 'PASS' ? 'good' : latest ? 'bad' : 'neutral'}>{latest ? status(latest) : '未执行'}</strong><em>{group.evaluations.length} 条</em><ChevronRight size={15}/></button>;
...
function GateDetailDialog({ attemptId, attemptStateVersion, evaluation, policy, canRetryWithProvider, onClose, onRetried }: { attemptId: string; attemptStateVersion: number; evaluation: GateEvaluation; policy?: GatePolicy; canRetryWithProvider: boolean; onClose: () => void; onRetried: (attempt: NodeAttempt) => void }) {
  ...
  const review = evaluation.review_projection;
  const fallbackInstructions = String(policy?.config.prompt || '').trim();
  const criteria = review?.criteria.instructions || fallbackInstructions;
  const candidates = review?.review_context.candidate_outputs ?? [];
  const describeCandidate = (item: typeof candidates[number]) => `${item.filename || item.field_key || item.id || '未命名产物'} · ${item.artifact_type || '未知类型'}${item.byte_size != null ? ` · ${item.byte_size} B` : ''}${item.review_preview ? ` · ${item.review_preview.kind === 'TEXT' ? `已给 Agent ${item.review_preview.character_count || 0} 字文本预览` : item.review_preview.kind === 'BINARY' ? '二进制，仅元信息' : 'URL 引用'}` : ' · 未提供可读预览'}`;
  return <div className="modal-backdrop gate-conversation-backdrop" role="dialog" aria-modal="true" aria-label="门禁详情" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><section className="modal gate-detail-dialog"><header><div><h2>{platformReview ? '平台交付与映射校验详情' : '审查门禁详情'}</h2><small>只读 · {evaluation.decision} · {platformReview ? '确定性平台校验' : '门禁审查记录'} · 第 {evaluation.evaluation_attempt} 次执行</small></div><button className="ghost" aria-label="关闭门禁详情" onClick={onClose}><X size={17}/></button></header><div className="gate-detail-body">{platformReview ? <section className="platform-check-details"><h3>逐项校验结果</h3><h4>输出校验</h4><ul>{outputChecks.length ? outputChecks.map((item, index) => checkRow(item, `output-${index}`)) : <li className="fail"><span>未找到冻结产物或逐项校验记录</span></li>}</ul><h4>下游端口映射</h4><ul>{mappingChecks.length ? mappingChecks.map((item, index) => checkRow(item, `mapping-${index}`)) : <li className="pass"><span>当前节点没有冻结的下游端口映射</span></li>}</ul><p className="platform-review-note">平台确定性校验不会创建审查 Agent 会话。</p></section> : <><section className={`gate-detail-summary${technicalFailure ? ' gate-detail-failure' : ''}`}><b>{String(evaluation.result.summary ?? '')}</b>{Array.isArray(evaluation.result.reasons) && evaluation.result.reasons.length > 0 && <ul>{evaluation.result.reasons.map((reason, index) => <li key={index}>{reason}</li>)}</ul>}{technicalFailure && <dl className="gate-technical-details"><dt>错误代码</dt><dd>{evaluation.error_code || 'GATE_ERROR'}</dd>{evaluation.log_excerpt && <><dt>执行原因</dt><dd><pre>{evaluation.log_excerpt}</pre></dd></>}</dl>}</section><section className="gate-execution-config"><h3>本次门禁 Agent 配置</h3><p>{provider?.name || executionPreset.model_provider_id || '历史记录未保存供应商'} · {executionPreset.model_name || '未记录模型'}{executionPreset.reasoning_effort ? ` · ${executionPreset.reasoning_effort}` : ''}</p>{!evaluation.agent_preset && policy?.agent_preset && <small>该历史记录未保存单次配置，以上为本轮冻结门禁策略。</small>}</section><section className="gate-review-explanation"><h3>本次冻结标准（审计副本）</h3>{criteria ? <pre>{criteria}</pre> : <p>该历史记录未保存冻结的门禁提示词。</p>}{review?.criteria.script && <><h4>辅助判断脚本</h4><pre>{review.criteria.script}</pre></>}<h3>审查输入证据</h3>{review ? <><p>候选产物 {candidates.length} 个；仅显示安全元信息，不复制产物正文。</p>{candidates.length ? <ul>{candidates.map(item => <li key={item.id || `${item.field_key}-${item.version_no}`}>{describeCandidate(item)}</li>)}</ul> : <p>本次冻结上下文中没有候选输出。</p>}{review.review_context.downstream_consumers.length > 0 && <small>已同时提供 {review.review_context.downstream_consumers.length} 个下游节点的冻结输入合同。</small>}</> : <p>该历史记录生成于可解释性投影上线前；只能从本轮冻结门禁配置还原标准，未保存审查输入清单。</p>}</section>{technicalFailure && canRetryWithProvider && <section className="gate-provider-retry"><h3>修改本轮门禁后重试</h3><p>修改仅保存到当前执行记录的门禁副本，不会影响节点预设或以后复制的节点。</p><label>判定提示词<textarea aria-label="重试门禁判定提示词" value={retryPrompt} onChange={event => setRetryPrompt(event.target.value)}/></label><label>可选 Python 脚本<textarea aria-label="重试门禁 Python 脚本" className="code" value={retryCode} onChange={event => setRetryCode(event.target.value)}/></label><ModelPresetFields label="重试门禁" preset={retryPreset} onChange={setRetryPreset}/><button className="secondary full" disabled={!retryable || retry.isPending} onClick={() => retry.mutate()}>{retry.isPending ? '正在提交重试…' : '保存本轮配置并重试'}</button>{retry.error && <p className="error">{retry.error.message}</p>}</section>}<section className="gate-agent-transcript"><h3>完整审查对话</h3>{evaluation.conversation_available ? query.isLoading ? <div className="empty compact">正在读取完整对话…</div> : query.isError ? <p className="error">{query.error.message}</p> : events.length ? <div className="gate-conversation-body"><ConversationSurface events={events} liveText="" isGenerating={false}/></div> : <div className="empty compact">该历史评估使用不会写入事件的审查接口，因此没有可恢复的原生对话；重新执行门禁后会显示完整过程。</div> : <div className="empty compact">门禁 Agent 未能创建可恢复的审查会话；上方技术原因可用于定位。</div>}</section></>}</div><footer><span>该记录不能继续对话、修改、分叉或删除。</span><button className="secondary" onClick={onClose}>关闭</button></footer></section></div>;
}
...
function AttemptPanel({ run, nodeRun, attempt, refresh, navigate, sessionReturnContext, automaticArtifactScope }: { run: FlowRun; nodeRun: NodeRun; attempt: NodeAttempt; refresh: () => void; navigate: (result: unknown, kind: string) => void; sessionReturnContext?: { runId: string; mode: WorkbenchMode; automaticRecordId?: string }; automaticArtifactScope?: { parentRunId: string; recordId: string } }) {
  ...
  const automaticArtifacts = useQuery({
    queryKey: ['flow-run-automatic-artifacts', automaticArtifactScope?.parentRunId, automaticArtifactScope?.recordId, attempt.id, artifactPage],
    queryFn: () => api.automaticRecordArtifacts(automaticArtifactScope!.parentRunId, automaticArtifactScope!.recordId, artifactPage, attempt.id),
    enabled: Boolean(automaticArtifactScope && tab === 'outputs'),
  });
  ...
  const automaticGateRemediationPending = automaticAttempt
    && attempt.state === 'END_BLOCKED'
    && !attempt.error_code
    && automaticGateFailureRound > 0
    && automaticGateFailureRound <= 3;
  ...
  return <aside className="action-panel attempt-control"><header><div><b>{nodeRunName(run, nodeRun)}</b><small>第 {nodeVisitNumber(run, nodeRun)} 次执行 / 第 {attempt.attempt_no} 轮</small></div></header><nav className="attempt-detail-tabs" aria-label="执行详情"><button className={tab === 'overview' ? 'active' : ''} onClick={() => setTab('overview')}>概览</button><button className={tab === 'gates' ? 'active' : ''} onClick={() => setTab('gates')}>门禁结果</button><button className={tab === 'outputs' ? 'active' : ''} onClick={() => setTab('outputs')}>输出</button>{automaticArtifactScope && <button className={tab === 'audit' ? 'active' : ''} onClick={() => setTab('audit')}>历史审计</button>}</nav><div className="action-content">{tab === 'overview' && <><div className="state-banner"><span>当前轮次状态</span><b>{runtimeFailed ? '节点执行失败' : ATTEMPT_STATE_LABELS[attempt.state] ?? attempt.state}</b><small><span data-testid="attempt-state">{attempt.state}</span> · 状态版本 {attempt.state_version}</small></div>{automaticAttempt && attempt.automatic_progress && <AutomaticProgressPanel progress={attempt.automatic_progress}/>} {attemptNode && <NodeContextSummary node={attemptNode} contextIds={attempt.context_ids ?? null} frozenSessionContexts={attempt.frozen_session_contexts} frozenAgentCapabilities={attempt.frozen_agent_capabilities} agentPreset={attempt.agent_preset} mode={attempt.startup_mode === 'CHAT' ? 'CHAT' : 'PROMPT'}/>}<InputSummary fields={attemptNode?.asset.inputs ?? []} bindings={nodeInputBindings} artifacts={inputArtifacts}/>{editableInputs && <button className="secondary full" onClick={() => setInputDialogOpen(true)}><Play size={14}/>编辑本轮输入</button>}{!editableInputs && <p className="field-hint">输入已随本轮启动冻结，仅供查看。</p>}
    ...
    {nodeRun.attempts.length > 1 && <section className="attempt-switcher"><h4>修订轮次</h4><div>{nodeRun.attempts.map(item => <button key={item.id} className={item.id === attempt.id ? 'active' : ''} onClick={() => useWorkbenchStore.getState().selectAttempt(item.id)}>第 {item.attempt_no} 轮</button>)}</div></section>}
      ...
      {(attempt.state === 'START_BLOCKED' || attempt.state === 'END_BLOCKED') && <section className="terminal-run-panel"><h4>{automaticBlockedTitle}</h4><p>{automaticBlockedDescription || attempt.error_detail || (runtimeFailed ? '模型或运行时执行失败，尚未生成正式输出。可进入节点会话继续处理；OpenHands 接受继续或新消息后，后续正式事件会重新驱动本节点。' : automaticGateRemediationPending ? `已创建输出修订会话（第 ${automaticGateFailureRound}/3 次），原会话与历史输出保持不变。` : '请查看门禁结果，再选择根据门禁结果调整，或在人工风险确认后继续。')}</p>{runtimeFailed && <small>失败发生在 Runtime 执行阶段，不是完成门禁拒绝；当前没有可流转的正式 Artifact。会话可继续时，新的原生回合会恢复节点事件投影。</small>}{(!automaticAttempt || retryableAutomaticFailure) && !runtimeFailed && <button className="secondary full" onClick={() => act('retry')}>重试当前阶段</button>}{attempt.state === 'END_BLOCKED' && !attempt.error_code && !automaticGateRemediationPending && <><button className="primary full" disabled={mutation.isPending} onClick={() => void dialog.confirm({ title: '根据门禁结果调整？', message: '平台会从主执行会话的完成边界创建新的原生 Fork，并将未通过门禁的结论整理后发送给该会话。原会话、门禁结果和历史分支都会保留；修订完成后会重新执行完成门禁。', confirmLabel: '创建调整会话' }).then(ok => ok && act('remediate-gate-failure'))}>根据门禁结果调整</button><label>人工接受风险理由<textarea value={text} maxLength={4000} onChange={event => setText(event.target.value)} placeholder="说明为何在保留校验失败结论的情况下仍可验收和流转"/></label><button className="danger full" disabled={!text.trim() || mutation.isPending} onClick={() => void dialog.confirm({ title: '接受门禁风险并继续？', message: '校验 Agent 的 FAIL 结论会原样保留；本次操作会作为独立人工决定写入审计，并继续验收和流转。', confirmLabel: '接受风险并继续', tone: 'danger' }).then(ok => ok && act('accept-gate-risk', { reason: text.trim() }))}>接受风险并继续流转</button></>}</section>}
