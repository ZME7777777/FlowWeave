...
function RunRail({ run, mode, automaticRecords, automaticError, selected, selectedNodeDeletable, manualBusyId, selectedAutomaticId, automaticBusyId, onModeChange, onSelect, onDeleteNode, onSelectAutomatic, onClearSelection, onCreateAutomatic, onDeleteAutomatic, onStartAutomatic }: {
  ...
  const manualToolbar = <div className="automatic-record-toolbar manual-record-toolbar"><button type="button" className="danger" title={selected && !selectedNodeDeletable ? '运行中的记录请先在右侧取消本轮节点执行，待运行时停止后再删除' : undefined} disabled={!selectedNodeDeletable || Boolean(manualBusyId)} onClick={onDeleteNode}><Trash2 size={13}/>{manualBusyId ? '删除中…' : '删除'}</button></div>;
...
function GateList({ evaluations, policies = [], onViewConversation }: { evaluations: GateEvaluation[]; policies?: GatePolicy[]; onViewConversation?: (evaluation: GateEvaluation) => void }) {
  ...
  return <div className="gate-results">{platformReview && <section className="platform-output-review"><header><span><b>平台交付与映射校验</b><small>确定性校验 · 仅检查正式产物、类型和端口映射；不评估文件内容或业务质量</small></span><strong className={platformReview.decision === 'PASS' ? 'good' : 'bad'}>{platformReview.decision}</strong></header><p>{String(platformReview.result.summary ?? '')}</p><small className="platform-review-evidence">校验的冻结产物：{reviewArtifacts.length ? reviewArtifacts.map(item => item.filename).join('、') : '未找到冻结产物记录'}</small>{reviewConsumers.length > 0 && <div className="platform-review-consumers"><small>下游端口映射：</small>{reviewConsumers.flatMap(consumer => consumer.mappings.map(mapping => <span key={`${consumer.instance_key}:${mapping.source_output_key}:${mapping.target_input.field_key}`}>{mapping.source_output_key} → {consumer.alias || consumer.instance_key}.{mapping.target_input.display_name || mapping.target_input.field_key}{mapping.target_input.data_type ? `（${mapping.target_input.data_type}）` : ''}</span>))}</div>}{platformReview.conversation_available && onViewConversation && <div><button className="ghost gate-conversation-button" onClick={() => onViewConversation(platformReview)}><Eye size={13}/>查看历史审查会话</button></div>}</section>}<section className="custom-gate-results">{configured.length > 0 && <h5>自定义门禁（内容与质量）</h5>}{configured.map(policy => {
...
function GateConversationDialog({ attemptId, evaluation, onClose }: { attemptId: string; evaluation: GateEvaluation; onClose: () => void }) {
  ...
  return <div className="modal-backdrop gate-conversation-backdrop" role="dialog" aria-modal="true" aria-label={platformReview ? '平台历史输出审查会话' : '校验 Agent 记录'} onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><section className="modal gate-conversation-dialog"><header><div><h2>{platformReview ? '平台历史输出审查会话' : '校验 Agent 记录'}</h2><small>只读 · 原始 OpenHands 事件 · {evaluation.decision}</small>{platformReview && <p className="gate-conversation-explainer">这是旧版模型审查留下的历史记录。新运行改用确定性的交付与端口映射校验，不会评估文件内容或业务质量。</p>}</div><button className="ghost" aria-label="关闭校验记录" onClick={onClose}><X size={17}/></button></header><div className="gate-conversation-body">{query.isLoading ? <div className="empty compact">正在读取校验记录…</div> : query.isError ? <p className="error">{query.error.message}</p> : <ConversationSurface events={events} liveText="" isGenerating={false}/>}</div><footer><span>该记录不能继续对话、修改、分叉或删除。</span><button className="secondary" onClick={onClose}>关闭</button></footer></section></div>;
}
