...
function GateList({ evaluations, policies = [], onViewDetails }: { evaluations: GateEvaluation[]; policies?: GatePolicy[]; onViewDetails?: (evaluation: GateEvaluation) => void }) {
  ...
  const labelFor = (stage: GatePolicy['stage'], position: number) => `${stage === 'START' ? '启动' : '完成'}门禁 · #${position + 1}`;
  ...
  return <div className="gate-results"><section className="custom-gate-results">{configured.length > 0 && <h5>自定义门禁（内容与质量）</h5>}{configured.map(policy => {
    const policyKey = policy.id ?? `${policy.stage}-${policy.position}`;
    const evaluationsForPolicy = policy.id ? evaluations.filter(item => item.stage === policy.stage && item.policy_snapshot_key === policy.id) : [];
    const preset = policy.agent_preset ?? {};
    return <section className="custom-gate-history" key={policyKey}><header><span><b>{labelFor(policy.stage, policy.position)}</b><small>本轮冻结的门禁定义与完整审查记录</small></span></header><section className="gate-definition-list"><h5>门禁定义</h5><article><span><b>判定提示词</b><small>{summary(policy.config.prompt, '未配置')}</small></span><button type="button" className="secondary" onClick={() => setDefinition({ policy, kind: 'prompt' })}>查看</button></article><article><span><b>可选 Python 脚本</b><small>{summary(policy.config.code, '未配置')}</small></span><button type="button" className="secondary" onClick={() => setDefinition({ policy, kind: 'python' })}>查看</button></article><article><span><b>模型配置</b><small>{preset.model_provider_id || '未记录供应商'} · {preset.model_name || '未选择模型'}{preset.reasoning_effort ? ` · ${preset.reasoning_effort}` : ''}</small></span><button type="button" className="secondary" onClick={() => setDefinition({ policy, kind: 'model' })}>查看</button></article></section>{reviewRows('审查记录', evaluationsForPolicy, `policy:${policyKey}`)}</section>;
  })}{[...legacyGroups.entries()].map(([key, items]) => reviewRows(`${labelFor(items[0].stage, items[0].policy_position)}（历史定义不可用）`, items, `legacy:${key}`, '该记录所属的冻结门禁定义已不在当前详情中，但审查结果完整保留。'))}</section>{platformReviews.length > 0 && reviewRows('平台交付与映射校验记录', platformReviews, 'platform-output-review', '确定性校验，不使用门禁提示词、脚本或模型。')}{definition?.kind === 'prompt' && <GateContentDialog kind="prompt" initial={String(definition.policy.config.prompt || '')} readOnly onClose={() => setDefinition(undefined)} onSave={() => undefined}/>} {definition?.kind === 'python' && <GateContentDialog kind="python" initial={String(definition.policy.config.code || '')} readOnly onClose={() => setDefinition(undefined)} onSave={() => undefined}/>} {definition?.kind === 'model' && <GateModelDetailDialog policy={definition.policy} onClose={() => setDefinition(undefined)}/>}</div>;
}
...
function AttemptPanel({ run, nodeRun, attempt, refresh, navigate, sessionReturnContext, automaticArtifactScope }: { run: FlowRun; nodeRun: NodeRun; attempt: NodeAttempt; refresh: () => void; navigate: (result: unknown, kind: string) => void; sessionReturnContext?: { runId: string; mode: WorkbenchMode; automaticRecordId?: string }; automaticArtifactScope?: { parentRunId: string; recordId: string } }) {
  ...
  return <aside className="action-panel attempt-control"><header><div><b>{nodeRunName(run, nodeRun)}</b><small>第 {nodeVisitNumber(run, nodeRun)} 次执行 / 第 {attempt.attempt_no} 轮</small></div></header><nav className="attempt-detail-tabs" aria-label="执行详情"><button className={tab === 'overview' ? 'active' : ''} onClick={() => setTab('overview')}>概览</button><button className={tab === 'gates' ? 'active' : ''} onClick={() => setTab('gates')}>门禁结果</button><button className={tab === 'outputs' ? 'active' : ''} onClick={() => setTab('outputs')}>输出</button>{automaticArtifactScope && <button className={tab === 'audit' ? 'active' : ''} onClick={() => setTab('audit')}>历史审计</button>}</nav><div className="action-content">{tab === 'overview' && <><div className="state-banner"><span>当前轮次状态</span><b>{runtimeFailed ? '节点执行失败' : ATTEMPT_STATE_LABELS[attempt.state] ?? attempt.state}</b><small><span data-testid="attempt-state">{attempt.state}</span> · 状态版本 {attempt.state_version}</small></div>{automaticAttempt && attempt.automatic_progress && <AutomaticProgressPanel progress={attempt.automatic_progress}/>} {attemptNode && <NodeContextSummary node={attemptNode} contextIds={attempt.context_ids ?? null} frozenSessionContexts={attempt.frozen_session_contexts} frozenAgentCapabilities={attempt.frozen_agent_capabilities} agentPreset={attempt.agent_preset} mode={attempt.startup_mode === 'CHAT' ? 'CHAT' : 'PROMPT'}/>}<InputSummary fields={attemptNode?.asset.inputs ?? []} bindings={nodeInputBindings} artifacts={inputArtifacts}/>{editableInputs && <button className="secondary full" onClick={() => setInputDialogOpen(true)}><Play size={14}/>编辑本轮输入</button>}{!editableInputs && <p className="field-hint">输入已随本轮启动冻结，仅供查看。</p>}
    ...
    {nodeRun.attempts.length > 1 && <section className="attempt-switcher"><h4>修订轮次</h4><div>{nodeRun.attempts.map(item => <button key={item.id} className={item.id === attempt.id ? 'active' : ''} onClick={() => useWorkbenchStore.getState().selectAttempt(item.id)}>第 {item.attempt_no} 轮</button>)}</div></section>}
      ...
      {(attempt.state === 'START_BLOCKED' || attempt.state === 'END_BLOCKED') && <section className="terminal-run-panel"><h4>{automaticBlockedTitle}</h4><p>{automaticBlockedDescription || attempt.error_detail || (runtimeFailed ? '模型或运行时执行失败，尚未生成正式输出。可进入节点会话继续处理；OpenHands 接受继续或新消息后，后续正式事件会重新驱动本节点。' : automaticGateRemediationPending ? `已创建输出修订会话（第 ${automaticGateFailureRound}/3 次），原会话与历史输出保持不变。` : '请查看门禁结果，选择人工接受风险，或让平台 Fork 主会话后自动返工。')}</p>{runtimeFailed && <small>失败发生在 Runtime 执行阶段，不是完成门禁拒绝；当前没有可流转的正式 Artifact。会话可继续时，新的原生回合会恢复节点事件投影。</small>}{(!automaticAttempt || retryableAutomaticFailure) && !runtimeFailed && <button className="secondary full" onClick={() => act('retry')}>重试当前阶段</button>}{attempt.state === 'END_BLOCKED' && !attempt.error_code && !automaticGateRemediationPending && <><button className="primary full" disabled={mutation.isPending} onClick={() => void dialog.confirm({ title: 'Fork 会话并自动返工？', message: '平台会从主执行会话的完成边界创建新的原生 Fork，并把所有未通过门禁的结论发送给返工 Agent。原会话、门禁结果和历史分支都会保留；修订分支完成后会重新执行完成门禁。', confirmLabel: 'Fork 并开始返工' }).then(ok => ok && act('remediate-gate-failure'))}>Fork 会话并自动返工</button><label>人工接受风险理由<textarea value={text} maxLength={4000} onChange={event => setText(event.target.value)} placeholder="说明为何在保留校验失败结论的情况下仍可验收和流转"/></label><button className="danger full" disabled={!text.trim() || mutation.isPending} onClick={() => void dialog.confirm({ title: '接受门禁风险并继续？', message: '校验 Agent 的 FAIL 结论会原样保留；本次操作会作为独立人工决定写入审计，并继续验收和流转。', confirmLabel: '接受风险并继续', tone: 'danger' }).then(ok => ok && act('accept-gate-risk', { reason: text.trim() }))}>接受风险并继续流转</button></>}</section>}
