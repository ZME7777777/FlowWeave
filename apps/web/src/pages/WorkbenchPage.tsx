...
function CapabilityPresetDialog({ selectedIds, onClose, onSave }: { selectedIds: string[]; onClose: () => void; onSave: (ids: string[]) => void }) {
  ...
  useEscapeClose(onClose);
  const capabilities = useMemo(() => (catalog.data ?? []).filter(item => ['SKILL', 'MCP', 'PLUGIN', 'CONTEXT', 'AGENT_DEFINITION'].includes(item.capability_type) && item.is_latest), [catalog.data]);
