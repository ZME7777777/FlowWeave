...
function FallbackModelDialog({ initial, primary, onClose, onSave }: { initial: NonNullable<AgentPreset['fallback_models']>; primary: GateAgentPreset; onClose: () => void; onSave: (models: NonNullable<AgentPreset['fallback_models']>) => void }) {
  ...
  return <div className="modal-backdrop"><section className="modal agent-preset-model-dialog" role="dialog" aria-modal="true" aria-label="配置额度失败 fallback 模型"><header><div><span className="eyebrow">QUOTA FALLBACK</span><h2>硬额度失败时的备用模型</h2><p>仅在主模型返回不可恢复的额度／用量限制时，由 OpenHands 按顺序尝试。未配置即快速失败，不会静默换模。</p></div><button type="button" className="ghost" onClick={onClose}><X size={17}/></button></header><div className="agent-fallback-list">{models.length ? models.map((item, index) => <article key={`${item.model_provider_id}:${item.model_name}:${item.reasoning_effort ?? ''}`}><span><b>备用 {index + 1}</b><small>{item.model_provider_id} · {item.model_name}{item.reasoning_effort ? ` · ${item.reasoning_effort}` : ''}</small></span><button type="button" className="ghost gate-delete" onClick={() => setModels(current => current.filter((_, position) => position !== index))}><Trash2 size={14}/>移除</button></article>) : <p className="field-hint">未配置备用模型：硬额度失败会保持当前模型并返回可解释错误。</p>}</div>{models.length < 3 && <section className="agent-fallback-add"><h3>添加备用模型</h3><ModelPresetFields label="备用" preset={candidate} onChange={setCandidate}/>{sameAsPrimary && <p className="field-hint">备用模型必须不同于主模型 {primary.model_provider_id} · {primary.model_name}。</p>}<button type="button" className="secondary" disabled={!candidate.model_provider_id || !candidate.model_name || sameAsPrimary} onClick={add}><Plus size={14}/>加入顺序</button></section>}<footer><button type="button" className="ghost" onClick={onClose}>取消</button><button type="button" className="primary" onClick={() => onSave(models)}>冻结此配置</button></footer></section></div>;
}
