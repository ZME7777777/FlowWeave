...
function GateList({ evaluations, policies = [], onViewDetails }: { evaluations: GateEvaluation[]; policies?: GatePolicy[]; onViewDetails?: (evaluation: GateEvaluation) => void }) {
  ...
  return <div className="gate-results gate-overview-list">{groups.map(group => {
    ...
    return <button type="button" className="gate-overview-row" key={group.key} onClick={() => setSelected(group)}><span><b>{group.title}</b><small>{latest ? `${new Date(latest.created_at).toLocaleString()} · ${summary(latest.result.summary, latest.decision === 'ERROR' ? '门禁执行未完成' : '未记录审查摘要')}` : '尚无审查记录'}</small>{latest?.usage && <small>累计 {latest.usage.total_tokens.toLocaleString('zh-CN')} Token</small>}</span><strong className={latest?.decision === 'PASS' ? 'good' : latest ? 'bad' : 'neutral'}>{latest ? status(latest) : '未执行'}</strong><em>{group.evaluations.length} 条</em><ChevronRight size={15}/></button>;
