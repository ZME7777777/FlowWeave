...
import { AlertTriangle, ArrowDown, ArrowLeft, ArrowUp, Bot, Boxes, Check, ChevronDown, ChevronRight, Copy, Download, ExternalLink, Eye, FileText, FolderClosed, Layers3, Play, Plus, RefreshCw, Send, StopCircle, Trash2, Upload, X } from 'lucide-react';
...
function FallbackModelDialog({ initial, primary, onClose, onSave }: { initial: NonNullable<AgentPreset['fallback_models']>; primary: GateAgentPreset; onClose: () => void; onSave: (models: NonNullable<AgentPreset['fallback_models']>) => void }) {
  ...
  const move = (index: number, offset: number) => setModels(current => { const target = index + offset; if (target < 0 || target >= current.length) return current; const next = [...current]; [next[index], next[target]] = [next[target], next[index]]; return next; });
  const providerName = (providerId: string) => providers.data?.find(item => item.id === providerId)?.name ?? providerId;
  return <div className="modal-backdrop"><section className="modal agent-preset-model-dialog agent-fallback-dialog" role="dialog" aria-modal="true" aria-label="配置额度失败 fallback 模型"><header><div><span className="eyebrow">QUOTA FALLBACK</span><h2>硬额度失败时的备用模型</h2><p>仅在主模型返回不可恢复的额度／用量限制时，由 OpenHands 按以下顺序尝试。未配置即快速失败，不会静默换模。</p></div><button type="button" className="ghost" onClick={onClose}><X size={17}/></button></header>{models.length < 3 && <section className="agent-fallback-add" aria-labelledby="fallback-add-title"><div className="agent-fallback-section-heading"><span>1</span><div><h3 id="fallback-add-title">添加备用模型</h3><p>选择供应商和模型后添加到下方优先级列表。</p></div></div><ModelPresetFields label="备用" preset={candidate} showReasoningEffort={false} onChange={setCandidate}/>{sameAsPrimary && <p className="field-hint">备用模型必须不同于主模型 {providerName(primary.model_provider_id ?? '')} · {primary.model_name}。</p>}<button type="button" className="secondary agent-fallback-add-button" disabled={!candidate.model_provider_id || !candidate.model_name || sameAsPrimary} onClick={add}><Plus size={14}/>添加备用模型</button></section>}<section className="agent-fallback-priority" aria-labelledby="fallback-priority-title"><div className="agent-fallback-section-heading"><span>2</span><div><h3 id="fallback-priority-title">备用模型优先级</h3><p>额度失败时从上到下依次尝试；可使用上下箭头调整顺序。</p></div></div>{models.length ? <ol className="agent-fallback-list">{models.map((item, index) => <li key={`${item.model_provider_id}:${item.model_name}:${item.reasoning_effort ?? ''}`}><span className="agent-fallback-order">{index + 1}</span><span className="agent-fallback-model"><b>{providerName(item.model_provider_id)} · {item.model_name}</b><small>{item.reasoning_effort ? `默认思考程度：${item.reasoning_effort}` : '使用模型默认思考程度'}</small></span><div className="agent-fallback-actions"><button type="button" className="ghost" aria-label={`上移备用模型 ${index + 1}`} title="上移" disabled={index === 0} onClick={() => move(index, -1)}><ArrowUp size={14}/></button><button type="button" className="ghost" aria-label={`下移备用模型 ${index + 1}`} title="下移" disabled={index === models.length - 1} onClick={() => move(index, 1)}><ArrowDown size={14}/></button><button type="button" className="ghost gate-delete" aria-label={`移除备用模型 ${index + 1}`} onClick={() => setModels(current => current.filter((_, position) => position !== index))}><Trash2 size={14}/><span>移除</span></button></div></li>)}</ol> : <p className="agent-fallback-empty">还没有备用模型。添加后会按这里的优先级，在主模型额度失败时依次尝试。</p>}</section><footer><button type="button" className="ghost" onClick={onClose}>取消</button><button type="button" className="primary" onClick={() => onSave(models)}>保存</button></footer></section></div>;
}
