...
function GateList({ evaluations, policies = [], onViewConversation }: { evaluations: GateEvaluation[]; policies?: GatePolicy[]; onViewConversation?: (evaluation: GateEvaluation) => void }) {
  ...
  return <div className="gate-results">{platformReview && <section className="platform-output-review"><header><span><b>平台默认输出审查</b><small>独立审查会话 · 不共享执行 Agent 的对话、工具或工作区</small></span><strong className={platformReview.decision === 'PASS' ? 'good' : 'bad'}>{platformReview.decision}</strong></header><p>{String(platformReview.result.summary ?? '')}</p><small className="platform-review-evidence">审查的冻结产物：{reviewArtifacts.length ? reviewArtifacts.map(item => item.filename).join('、') : '未找到冻结产物记录'}</small>{reviewConsumers.length > 0 && <div className="platform-review-consumers"><small>下游输入合同：</small>{reviewConsumers.flatMap(consumer => consumer.mappings.map(mapping => <span key={`${consumer.instance_key}:${mapping.source_output_key}:${mapping.target_input.field_key}`}>{mapping.source_output_key} → {consumer.alias || consumer.instance_key}.{mapping.target_input.display_name || mapping.target_input.field_key}{mapping.target_input.data_type ? `（${mapping.target_input.data_type}）` : ''}</span>))}</div>}<div>{platformReview.conversation_available && onViewConversation ? <button className="ghost gate-conversation-button" onClick={() => onViewConversation(platformReview)}><Eye size={13}/>查看审查会话与依据</button> : <small>审查会话记录不可用。</small>}</div></section>}<section className="custom-gate-results">{configured.length > 0 && <h5>自定义门禁</h5>}{configured.map(policy => {
...
function GateConversationDialog({ attemptId, evaluation, onClose }: { attemptId: string; evaluation: GateEvaluation; onClose: () => void }) {
  ...
  return <div className="modal-backdrop gate-conversation-backdrop" role="dialog" aria-modal="true" aria-label={platformReview ? '平台输出审查会话' : '校验 Agent 记录'} onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><section className="modal gate-conversation-dialog"><header><div><h2>{platformReview ? '平台默认输出审查会话' : '校验 Agent 记录'}</h2><small>只读 · 原始 OpenHands 事件 · {evaluation.decision}</small>{platformReview && <p className="gate-conversation-explainer">以下完整记录包含审查 Agent 收到的输出合同、冻结产物证据及其最终判断；它与执行 Agent 的会话隔离。</p>}</div><button className="ghost" aria-label="关闭校验记录" onClick={onClose}><X size={17}/></button></header><div className="gate-conversation-body">{query.isLoading ? <div className="empty compact">正在读取校验记录…</div> : query.isError ? <p className="error">{query.error.message}</p> : <ConversationSurface events={events} liveText="" isGenerating={false}/>}</div><footer><span>该记录不能继续对话、修改、分叉或删除。</span><button className="secondary" onClick={onClose}>关闭</button></footer></section></div>;
}
