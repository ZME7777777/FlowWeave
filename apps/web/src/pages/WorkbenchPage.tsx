...
function GateList({ evaluations, policies = [], onViewConversation }: { evaluations: GateEvaluation[]; policies?: GatePolicy[]; onViewConversation?: (evaluation: GateEvaluation) => void }) {
  ...
  return <div className="gate-results">{platformReview && <section className="platform-output-review"><header><span><b>平台交付与映射校验</b><small>确定性校验 · 只核对正式产物、类型和端口映射；不评估文件内容或业务质量</small></span><strong className={platformReview.decision === 'PASS' ? 'good' : 'bad'}>{platformReview.decision}</strong></header><p>{String(platformReview.result.summary ?? '')}</p><div className="platform-check-details"><small>逐项校验结果</small><h6>声明输出</h6><ul>{renderedOutputChecks.length ? renderedOutputChecks.map((item, index) => checkRow(item, `output-${index}`)) : <li className="fail"><span>未找到输出校验记录</span></li>}</ul><h6>下游端口映射</h6><ul>{renderedMappingChecks.length ? renderedMappingChecks.map((item, index) => checkRow(item, `mapping-${index}`)) : <li className="pass"><span>当前节点没有冻结的下游端口映射</span></li>}</ul></div><small className="platform-review-note">这是平台确定性校验，不会创建审查 Agent 会话；内容和质量审查请查看下方自定义门禁。</small>{platformReview.conversation_available && onViewConversation && <div><button className="ghost gate-conversation-button" onClick={() => onViewConversation(platformReview)}><Eye size={13}/>查看旧版历史审查会话</button></div>}</section>}<section className="custom-gate-results">{configured.length > 0 && <h5>自定义门禁（内容与质量）</h5>}{configured.map(policy => {
