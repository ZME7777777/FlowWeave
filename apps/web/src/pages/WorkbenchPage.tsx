...
function NodeConsole({ run, node, refresh, onActivated, onSelectExecution }: { run: FlowRun; node: SnapshotFlowNode; refresh: () => void; onActivated: (nodeRun: NodeRun) => void; onSelectExecution: (nodeRun: NodeRun) => void }) {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (mode === 'CHAT') {
        const conversation = await api.createConversation(attempt.id, attempt.state_version, '启动协作会话');
    ...
    onSuccess: result => {
      ...
      if (result.conversationId) {
        ...
        if (attempt) useWorkbenchStore.getState().openAgentChat(run.id, result.created.id, attempt.id, result.conversationId);
      }
    },
  });
  ...
  return <aside className="action-panel node-console"><header><div><b>{node.alias || node.asset.name}</b><small>节点控制台 · 已执行 {visits} 次</small></div></header><div className="action-content">
    <section className="node-console-intro"><span className="eyebrow">ARTIFACT-DRIVEN</span><h3>创建一次独立执行</h3><p>先绑定本次实际输入，再选择启动方式。每次启动都会保留独立的执行记录和 Agent 对话。</p></section>
...
function AttemptPanel({ run, nodeRun, attempt, refresh, navigate, onCreateNew }: { run: FlowRun; nodeRun: NodeRun; attempt: NodeAttempt; refresh: () => void; navigate: (result: unknown, kind: string) => void; onCreateNew: () => void }) {
  ...
  }, onSuccess: (result, variables) => { setText(''); if (variables.kind === 'chat' && result && typeof result === 'object' && 'id' in result) useWorkbenchStore.getState().openAgentChat(run.id, nodeRun.id, attempt.id, String(result.id)); else navigate(result, variables.kind); refresh(); } });
  ...
  return <aside className="action-panel attempt-control"><header><div><b>{nodeRunName(run, nodeRun)}</b><small>第 {nodeVisitNumber(run, nodeRun)} 次执行 / 第 {attempt.attempt_no} 轮</small></div></header><div className="action-content">{!terminal && <button className="primary full create-another-run" onClick={onCreateNew}><Plus size={15}/>创建新的独立执行</button>}<div className="state-banner"><span>当前轮次状态</span><b>{ATTEMPT_STATE_LABELS[attempt.state] ?? attempt.state}</b><small><span data-testid="attempt-state">{attempt.state}</span> · 状态版本 {attempt.state_version}</small></div>
    {attempt.runtime_phase === 'CANCEL_FAILED' && <section className="terminal-run-panel"><h4>Agent 停止状态未确认</h4><p>{attempt.error_detail || '运行时停止失败，需要重新对账或清理受管 Runtime。'}</p>{attempt.runtime_cancel_recovery_modes.includes('RECONCILE_PARENT') && <button className="secondary full" disabled={mutation.isPending} onClick={() => act('retry-cancel')}>重新对账并重试父会话停止</button>}{attempt.runtime_cancel_recovery_modes.includes('DELETE_MANAGED_RUNTIME') && <button className="danger full" disabled={mutation.isPending} onClick={() => void dialog.confirm({ title: '清理整个受管 Runtime？', message: '这会删除本轮绑定的整个 Runtime 容器。只有物理删除完成后，系统才会确认执行已停止。', confirmLabel: '清理整个 Runtime', tone: 'danger' }).then(ok => ok && act('delete-runtime'))}>清理整个受管 Runtime</button>}</section>}
