...
function FallbackModelDialog({ initial, onClose, onSave }: { initial: NonNullable<AgentPreset['fallback_models']>; onClose: () => void; onSave: (models: NonNullable<AgentPreset['fallback_models']>) => void }) {
  useEscapeClose(onClose);
  const [models, setModels] = useState(initial);
  const [candidate, setCandidate] = useState<GateAgentPreset>({});
  const add = () => {
    if (!candidate.model_provider_id || !candidate.model_name || models.length >= 3) return;
