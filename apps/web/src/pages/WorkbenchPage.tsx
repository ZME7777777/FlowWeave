...
import type { AgentPreset, ArtifactVersion, AttemptState, AutomaticNodePlan, CapabilityAsset, CapabilityCollection, FlowRun, FlowRunAutomaticRecord, FlowRunAutomaticRecordSummary, GateAgentPreset, GateEvaluation, GatePolicy, NodeAttempt, NodeRun, OpenHandsConversationEvent, OpenHandsConversationEventBatch, SnapshotFlowNode, TokenUsageSummary } from '../types';
...
function AttemptPanel({ run, nodeRun, attempt, refresh, navigate, sessionReturnContext, automaticArtifactScope }: { run: FlowRun; nodeRun: NodeRun; attempt: NodeAttempt; refresh: () => void; navigate: (result: unknown, kind: string) => void; sessionReturnContext?: { runId: string; mode: WorkbenchMode; automaticRecordId?: string }; automaticArtifactScope?: { parentRunId: string; recordId: string } }) {
  ...
  const mutation = useMutation({ mutationFn: async ({ kind, body }: { kind: string; body?: unknown }) => {
    ...
    if (kind === 'reconcile-runtime-completion') return api.reconcileRuntimeCompletion(attempt.id, attempt.state_version, String((body as { reason: string }).reason));
  ...
  return <aside className="action-panel attempt-control"><header><div><b>{nodeRunName(run, nodeRun)}</b><small>第 {nodeVisitNumber(run, nodeRun)} 次执行 / 第 {attempt.attempt_no} 轮</small></div></header><nav className="attempt-detail-tabs" aria-label="执行详情"><button className={tab === 'overview' ? 'active' : ''} onClick={() => setTab('overview')}>概览</button><button className={tab === 'gates' ? 'active' : ''} onClick={() => setTab('gates')}>门禁结果</button><button className={tab === 'outputs' ? 'active' : ''} onClick={() => setTab('outputs')}>输出</button>{automaticArtifactScope && <button className={tab === 'audit' ? 'active' : ''} onClick={() => setTab('audit')}>历史审计</button>}</nav><div className="action-content">{tab === 'overview' && <><div className="state-banner"><span>当前轮次状态</span><b>{runtimeFailed ? '节点执行失败' : ATTEMPT_STATE_LABELS[attempt.state] ?? attempt.state}</b><small><span data-testid="attempt-state">{attempt.state}</span> · 状态版本 {attempt.state_version}</small></div>{automaticAttempt && attempt.automatic_progress && <AutomaticProgressPanel progress={attempt.automatic_progress}/>} {attemptNode && <NodeContextSummary node={attemptNode} contextIds={attempt.context_ids ?? null} frozenSessionContexts={attempt.frozen_session_contexts} frozenAgentCapabilities={attempt.frozen_agent_capabilities} agentPreset={attempt.agent_preset} mode={attempt.startup_mode === 'CHAT' ? 'CHAT' : 'PROMPT'}/>}<InputSummary fields={attemptNode?.asset.inputs ?? []} bindings={nodeInputBindings} artifacts={resolvedInputArtifacts}/>{automaticArtifactScope && automaticInputArtifacts.isError && <p className="error">读取本轮输入失败：{automaticInputArtifacts.error.message}</p>}{editableInputs && <button className="secondary full" onClick={() => setInputDialogOpen(true)}><Play size={14}/>编辑本轮输入</button>}{!editableInputs && <p className="field-hint">输入已随本轮启动冻结，仅供查看。</p>}
    ...
    {nodeRun.attempts.length > 1 && <section className="attempt-switcher"><h4>修订轮次</h4><div>{nodeRun.attempts.map(item => <button key={item.id} className={item.id === attempt.id ? 'active' : ''} onClick={() => useWorkbenchStore.getState().selectAttempt(item.id)}>第 {item.attempt_no} 轮</button>)}</div></section>}
      ...
      {(attempt.state === 'START_BLOCKED' || attempt.state === 'END_BLOCKED') && <section className="terminal-run-panel"><h4>{automaticBlockedTitle}</h4><p>{automaticBlockedDescription || attempt.error_detail || (runtimeFailed ? '模型或运行时执行失败，尚未生成正式输出。可进入节点会话继续处理；OpenHands 接受继续或新消息后，后续正式事件会重新驱动本节点。' : '请查看门禁结果：可直接重新执行门禁，或根据门禁结论调整交付物后重试。只有人工明确接受风险时才会继续流转。')}</p>{completionReconciliationAvailable && <><small>平台会重新验证当前 OpenHands 活跃分支的正式 FinishAction、冻结输出合同和受管工作区文件；不会接受手工指定的事件 ID、路径或产物。</small><label>对账原因<textarea value={text} maxLength={4000} onChange={event => setText(event.target.value)} placeholder="说明为什么需要重新投影这次 OpenHands 完成事件"/></label><button className="primary full" disabled={!text.trim() || mutation.isPending} onClick={() => void dialog.confirm({ title: '确认从 OpenHands 对账并补登？', message: '平台将依据正式 FinishAction ID 幂等登记候选产物，并重新执行完成门禁。', confirmLabel: '确认对账并补登' }).then(ok => ok && act('reconcile-runtime-completion', { reason: text.trim() }))}>从 OpenHands 对账并补登</button></>}{runtimeFailed && <small>失败发生在 Runtime 执行阶段，不是完成门禁拒绝；当前没有可流转的正式 Artifact。会话可继续时，新的原生回合会恢复节点事件投影。</small>}{(!automaticAttempt || retryableAutomaticFailure) && !runtimeFailed && !completionReconciliationAvailable && <button className="secondary full" onClick={() => act('retry')}>重新执行门禁</button>}{attempt.state === 'END_BLOCKED' && !attempt.error_code && <><button className="primary full" disabled={mutation.isPending} onClick={() => void dialog.confirm({ title: '根据门禁结果调整并重试？', message: '平台会从主执行会话的完成边界创建新的原生 Fork，并将未通过门禁的结论整理后发送给该会话。修订当前结果后会重新执行完成门禁；不会创建新的节点执行记录。', confirmLabel: '创建调整会话' }).then(ok => ok && act('remediate-gate-failure'))}>根据门禁结果调整并重试</button><label>人工接受风险理由<textarea value={text} maxLength={4000} onChange={event => setText(event.target.value)} placeholder="说明为何在保留校验失败结论的情况下仍可验收和流转"/></label><button className="danger full" disabled={!text.trim() || mutation.isPending} onClick={() => void dialog.confirm({ title: '接受门禁风险并继续？', message: '校验 Agent 的 FAIL 结论会原样保留；本次操作会作为独立人工决定写入审计，并继续验收和流转。', confirmLabel: '接受风险并继续', tone: 'danger' }).then(ok => ok && act('accept-gate-risk', { reason: text.trim() }))}>接受风险并继续流转</button></>}</section>}
...
export function WorkbenchPage() {
  ...
  const navigate = (result: unknown, kind: string) => {
    ...
    if (kind === 'remediate-gate-failure' && result && typeof result === 'object' && 'id' in result && nodeRun && result.id !== attempt?.id) {
