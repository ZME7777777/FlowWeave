...
function ScheduleDetailDialog({ schedule, onClose }: { schedule: FlowRunSchedule; onClose: () => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><section className="modal schedule-detail-dialog" role="dialog" aria-modal="true" aria-label={`定时任务 ${schedule.name} 的执行记录`}><header><div><span className="eyebrow">SCHEDULE RUNS</span><h2>{schedule.name}</h2><p>按触发时间倒序。点击 NodeRun 可直接进入该节点的执行记录。</p></div><button type="button" className="ghost product-dialog-close" aria-label="关闭" onClick={onClose}><X size={17}/></button></header><div className="schedule-occurrence-list">{isLoading ? <div className="empty compact">加载执行记录…</div> : data?.items.length ? data.items.map(occurrence => { const run = occurrence.flow_run; return <article className="schedule-occurrence-record" key={occurrence.id}><header><Clock3 size={15}/><span><b>{occurrence.trigger_kind === 'MANUAL' ? '手动触发' : '计划触发'} · {formatTime(occurrence.scheduled_for)}</b><small>{run ? `FlowRun #${run.run_no} · ${run.name}` : occurrence.state === 'FAILED' ? occurrence.error_detail || '启动失败' : '正在创建 FlowRun'}</small></span></header>{run?.node_runs.length ? <div className="schedule-occurrence-nodes">{run.node_runs.map(node => <button type="button" key={node.id} onClick={() => openRun(run.id, node.id)}><CircleDot size={13}/><span><b>{node.name || node.flow_node_snapshot_key}</b><small>{node.state} · {node.attempts.length} 次 Attempt</small></span><ChevronRight size={14}/></button>)}</div> : run ? <p>此 FlowRun 正在准备运行环境，尚未生成 NodeRun。</p> : null}</article>; }) : <div className="schedule-no-runs">尚未产生执行记录。</div>}</div>{data && <Pagination page={page} pageSize={pageSize} total={data.total} onPageChange={setPage}/>}</section></div>;
}
