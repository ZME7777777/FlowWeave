...
const runtimeEntryState = (run: FlowRunSummary) => {
  if (run.runtime_frozen) return { label: '已冻结', detail: run.runtime_freeze_reason || '运行环境镜像不兼容，需导出配置并在兼容的 FlowRun 中导入。', pending: true };
...
function RunResourceCell({ run }: { run: FlowRunSummary }) {
  const active = !run.runtime_frozen && run.runtime_status === 'ACTIVE';
  ...
  if (!active) return <span><b>{run.runtime_frozen ? '运行已冻结' : '暂无可用容器'}</b><small>{run.runtime_frozen ? '不兼容的旧运行环境不会再执行或恢复。' : '运行环境就绪后将显示当前绑定容器。'}</small></span>;
  if (resource.isPending) return <span className="run-resource-loading"><b>正在读取容器资源…</b><small>不影响运行列表加载</small></span>;
  const summary = resource.data?.resource;
  if (!summary) return <span><b>容器信息暂不可用</b><small>稍后将自动重试。</small></span>;
  return <span><b>容器 {summary.container_id} · #{summary.generation}</b><small>CPU 已用 {summary.cpu_usage_percent.toFixed(1)}%（实时）/ {summary.cpu_limit} · 内存 {formatResourceBytes(summary.memory_usage_bytes)} / {summary.memory_limit}</small><small>宿主机挂载 {summary.host_project_mount_path ?? '路径暂不可用'} → /runtime/workspace/project</small></span>;
}
...
export function RunsPage() {
  ...
  return <section className="page runs-page"><div className="page-action-row"><button className="primary" disabled={!flows.length} onClick={() => setStarting(flows[0])}><Plus size={16}/>启动流程</button></div>
    ...
    <div className="run-groups">{groups.map(group => { const isCollapsed = collapsed.has(group.id); return <section className="run-group" key={group.id}><header><button className="run-group-toggle" aria-label={`${isCollapsed ? '展开' : '收起'} ${group.name}`} aria-expanded={!isCollapsed} onClick={() => toggleGroup(group.id)}>{isCollapsed ? <ChevronRight size={15}/> : <ChevronDown size={15}/>}<span><b>{group.name}</b><small>{group.flow ? `流程编排 · 当前版本 v${group.rowVersion ?? '—'}` : `历史流程快照 · v${group.rowVersion ?? '—'}`}</small></span></button><span>{group.runs.length} 个运行</span>{group.flow ? <button className="secondary" onClick={() => setStarting(group.flow)}><Plus size={13}/>启动</button> : <span className="deleted-resource">流程已删除</span>}</header>{!isCollapsed && <div className="run-table"><div className="table-head"><span/><span>运行实例</span><span>运行状态</span><span>运行资源</span><span>时间</span><span/><span/></div>{group.runs.map(run => { const runtime = runtimeEntryState(run); const canToggle = !run.runtime_frozen && (run.runtime_status === 'ACTIVE' || run.runtime_status === 'STOPPED'); const isPaused = run.runtime_status === 'STOPPED'; const lifecyclePending = lifecycleRunId === run.id; return <div className={`run-row${runtime.pending ? ' runtime-pending' : ''}`} key={run.id}><label className="resource-check"><input type="checkbox" aria-label={`选择运行 ${run.name}`} checked={selectedIds.has(run.id)} onChange={() => toggleRun(run.id)}/></label><button className="run-open" disabled={runtime.pending} aria-label={runtime.pending ? `${run.name}：${runtime.label}` : undefined} onClick={() => openRun(run.id)}><span><b>Run #{run.run_no} · {run.name}</b><small>Snapshot v{run.active_snapshot_version ?? '—'} · {run.id.slice(0, 8)}</small>{run.usage && <small>累计 {run.usage.total_tokens.toLocaleString('zh-CN')} Token</small>}</span><span><span className={`run-state ${runtime.pending ? 'starting' : run.state.toLowerCase()}`}><CircleDot size={12}/>{runtime.label}</span><small>{runtime.detail}</small></span><RunResourceCell run={run}/><span><b>{new Date(run.started_at).toLocaleString()}</b><small>更新 {new Date(run.updated_at).toLocaleString()}</small></span><ArrowRight size={16}/></button><button className={`run-lifecycle${isPaused ? ' resume' : ''}`} disabled={!canToggle || lifecyclePending || deleting} aria-label={run.runtime_frozen ? `运行 ${run.name} 已冻结` : `${isPaused ? '启动' : '暂停'}运行 ${run.name}`} title={run.runtime_frozen ? '镜像不兼容，运行已冻结' : isPaused ? '启动运行容器' : '暂停并回收计算资源'} onClick={() => void toggleRuntime(run)}>{isPaused ? <Play size={15}/> : <Pause size={15}/>}<span>{run.runtime_frozen ? '已冻结' : lifecyclePending ? '处理中' : isPaused ? '启动' : '暂停'}</span></button><button className="run-delete" aria-label={`删除运行 ${run.name}`} title="删除运行" onClick={() => void removeMany([run.id], `运行“${run.name}”`)}><Trash2 size={15}/></button></div>; })}</div>}</section>; })}</div>
    </div>
