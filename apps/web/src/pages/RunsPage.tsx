...
const progressSummary = (run: FlowRunSummary) => {
  const { total, active, accepted, failed, cancelled, pending_action } = run.progress;
  if (!total) return { primary: '尚无执行记录', detail: '进入运行后可创建节点执行记录。', needsAttention: false };
  const detail = [`未结束 ${active}`, `已完成 ${accepted}`];
  if (failed) detail.push(`失败 ${failed}`);
  if (cancelled) detail.push(`已取消 ${cancelled}`);
  return { primary: `共 ${total} 条执行记录`, detail: detail.join(' · '), needsAttention: pending_action > 0 };
};
