...
export function AgentWorkbenchPage({ onNavigate, onOpenModels }: Props) {
  ...
  const canCreate = Boolean(runtimeWritable && workspace?.default_model_provider_id);
  const canWrite = Boolean(runtimeWritable && (selected || workspace?.default_model_provider_id));
  ...
  useEffect(() => { setEditing(false); setTitle(selected?.display_title ?? ''); setLiveText(''); setTurnState('idle'); setQueuedMessages([]); setPendingRewrite(undefined); setAttachments([]); setConversationModelName(''); setReasoningEffort(null); }, [selected?.display_title, selected?.id]);
  ...
  const switchModel = useMutation({ mutationFn: () => api.switchAgentConversationModel(workspace!.id, selected!.id, conversationModelName, reasoningEffort), onSuccess: refresh });
  ...
  const needsNewModelConfig = defaultMissing && !selected;
  ...
  const conversationProviderInfo = (providersQuery.data ?? []).find(item => item.id === selected?.model_provider_id);
