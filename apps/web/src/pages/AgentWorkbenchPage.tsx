...
import { Bot, ChevronRight, CircleDot, FileUp, LoaderCircle, Minimize2, PanelRightOpen, Pencil, Play, Plus, Send, Settings2, Square, Terminal, Trash2, X } from 'lucide-react';
...
export function AgentWorkbenchPage({ onNavigate, onOpenModels }: Props) {
  ...
  return <main className="agent-workbench-page">
    ...
    <section className="agent-workbench-main">
      ...
      {(create.error || saveSettings.error || rename.error || remove.error || send.error || upload.error || switchModel.error || condense.error || fork.error || interrupt.error || resume.error || rewrite.error || eventsQuery.error) && <p className="agent-workbench-error">{(create.error || saveSettings.error || rename.error || remove.error || send.error || upload.error || switchModel.error || condense.error || fork.error || interrupt.error || resume.error || rewrite.error || eventsQuery.error)?.message}</p>}
    </section>
