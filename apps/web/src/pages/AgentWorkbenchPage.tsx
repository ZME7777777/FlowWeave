...
export function AgentWorkbenchPage({ onNavigate, onOpenModels }: Props) {
  ...
  const send = useMutation({ mutationFn: ({ content, items, modelName, effort }: { content: string; items: AgentAttachment[]; modelName?: string; effort?: string | null }) => api.sendAgentMessage(workspace!.id, selected!.id, content, items, modelName, effort), onMutate: () => { setLiveText(''); setTurnState('running'); }, onSuccess: () => { setAttachments([]); setConversationModelName(''); setReasoningEffort(null); refresh(); }, onError: () => setTurnState('idle') });
