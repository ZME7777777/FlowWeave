...
import { Bot, ChevronRight, CircleDot, FileUp, LoaderCircle, PanelRightOpen, Pencil, Play, Plus, Send, Settings2, Square, Terminal, Trash2, X } from 'lucide-react';
...
import type { AgentAttachment, AgentConversation } from '../types';
...
export function AgentWorkbenchPage({ onNavigate, onOpenModels }: Props) {
  ...
  useEffect(() => { setEditing(false); setTitle(selected?.display_title ?? ''); setLiveText(''); setTurnState('idle'); setQueuedMessages([]); setPendingRewrite(undefined); setAttachments([]); setConversationProvider(workspace?.default_model_provider_id ?? ''); setConversationModelName(''); setReasoningEffort(null); }, [selected?.display_title, selected?.id, workspace?.default_model_provider_id]);
  ...
  const send = useMutation({ mutationFn: ({ content, items }: { content: string; items: AgentAttachment[] }) => api.sendAgentMessage(workspace!.id, selected!.id, content, items), onMutate: () => { setLiveText(''); setTurnState('running'); }, onSuccess: () => { setAttachments([]); refresh(); }, onError: () => setTurnState('idle') });
  ...
  const switchModel = useMutation({ mutationFn: () => api.switchAgentConversationModel(workspace!.id, selected!.id, conversationProvider, conversationModelName, reasoningEffort), onSuccess: refresh });
  ...
  const enqueueDraft = useCallback(() => {
    ...
    setDraft('');
    if (turnState === 'idle') send.mutate({ content, items: attachments });
    else setQueuedMessages(items => [...items, content]);
  }, [attachments, canWrite, draft, send, turnState]);
  useEffect(() => {
  ...
  const conversationProviderInfo = connectedProviders.find(item => item.id === conversationProvider);
  const conversationModel = conversationProviderInfo?.models.find(model => model.enabled && model.model_name === conversationModelName)
    ?? conversationProviderInfo?.models.find(model => model.enabled && model.is_default);
  const availableConversationModels = connectedProviders.flatMap(provider => provider.models.filter(model => model.enabled).map(model => ({ provider, model })));
