...
import type { BlockedNodeDelete, CapabilityAssetType, ModelProvider, NodeAsset, NodeAssetWrite, NodeDirectory } from '../types';
...
function NodeDetail({ node, directories, providers, onClose, onEdit }: { node: NodeAsset; directories: NodeDirectory[]; providers: ModelProvider[]; onClose: () => void; onEdit: () => void }) {
  ...
  const capabilities = (type: CapabilityAssetType) => node.capabilities.filter(item => item.capability_type === type);
  return <div className="detail-drawer-backdrop" onClick={onClose}><aside className="node-detail-drawer" role="dialog" aria-modal="true" aria-label={`节点详情 ${node.name}`} onClick={event => event.stopPropagation()}>
    ...
    <section><h3>Agent 能力</h3><dl><dt>模型</dt><dd>{provider?.name || '未配置'} · {node.executor?.model_name || '服务默认'}</dd><dt>启动方式</dt><dd>每次执行时选择能力或提示词</dd><dt>宿主工作区</dt><dd>{node.workspace_ref ? `var/workspaces/${node.workspace_ref}` : '创建后生成'}</dd></dl>
      {(['SKILL', 'PLUGIN', 'MCP', 'HOOK', 'TOOL_POLICY', 'AGENT_DEFINITION'] as const).map(type => <div className="detail-capability" key={type}><b>{type === 'SKILL' ? 'Skills' : type === 'AGENT_DEFINITION' ? 'Agent Definitions' : type}</b><span>{capabilities(type).length} 项</span><small>{capabilities(type).map(item => item.capability_key).join('、') || '无'}</small></div>)}
    </section>
