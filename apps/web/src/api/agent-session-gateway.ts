...
export interface AgentSessionApi {
  ...
  readonly bootstrapConversation: (hostId: AgentSessionHostId, conversationId: string, modelProviderId: string, modelName: string, reasoningEffort: string | null, content: string, messageKind: AgentMessageKind, attachments?: AgentAttachment[], references?: AgentConversationReference[], workDirectoryId?: AgentSessionWorkDirectoryId, capabilityVersionIds?: string[], idempotencyKey?: string) => Promise<{ conversation: AgentConversation; accepted: boolean; cursor?: string | null }>;
  ...
  readonly sendMessage: (hostId: AgentSessionHostId, bindingId: AgentSessionBindingId, content: string, messageKind: AgentMessageKind, attachments?: AgentAttachment[], references?: AgentConversationReference[]) => Promise<{ accepted: boolean; cursor?: string | null; compacted?: boolean; queued_during_turn?: boolean }>;
...
export function flowNodeSessionGateway(
  ...
  return {
    ...
    api: {
      ...
      bootstrapConversation: async (_hostId, conversationId, providerId, modelName, reasoningEffort, content, messageKind, attachments, references, workDirectoryId, _capabilityVersionIds, idempotencyKey) => {
        return nodeSessionApi.bootstrap(
          flowRunId, attemptId, content, messageKind, providerId, modelName, reasoningEffort, attachments, references, workDirectoryId, idempotencyKey ?? conversationId,
        );
      },
      ...
      sendMessage: (_hostId, bindingId, content, messageKind, attachments = [], references = []) =>
        nodeSessionApi.message(flowRunId, attemptId, bindingId, content, messageKind, attachments, references),
