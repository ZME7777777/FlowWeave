...
- FlowWeave 显式传入的 Runtime 能力必须可追溯到固定 version、digest、blob/hash 和
  Snapshot Runtime Manifest，明文 Secret 不得持久化进入 Runtime。OpenHands 1.47.0 原生的 HOME/项目
...
`30cf5832e42c71c24daa82a1a4fd5d25eb70d1b9`（`v1.47.0-4-g30cf5832e`），只修改 FlowWeave。
...
- 固定包版本：`openhands-agent-server==1.47.0`、`openhands-sdk==1.47.0`、
  `openhands-tools==1.47.0`、`openhands-workspace==1.47.0`
...
**默认目标（强制）：** 用户提到“部署”“发布”“更新远端镜像”或远端排障而未明确指定其他环境时，必须优先以
**本节的 `root@192.168.91.154` / `/opt/flowweave`** 为目标执行。开始任何部署操作前，先读取本节和
`docs/local-build-and-deploy.md`，并在操作输出中确认目标为 `192.168.91.154`；不得根据过期 SSH 别名、
其他主机或猜测自行改用部署环境。只有用户明确指定不同环境时才可例外，并应在执行前复述该例外。
